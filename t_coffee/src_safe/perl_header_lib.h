...
a]}\n    elsif ($ARGV[$a] eq \"-tcarg\") {$tcarg=f\
ile2string($ARGV[++$a]);}\n    else\n      {\n	add\
2tcenv($a++,@ARGV);\n      }\n  }\n\n\nif ($tree e\
q \"list\")\n  {\n    my $f=\"$tmpdir/f\";\n    # \
The command will create a dummy fasta to run tcoff\
ee\n    # and output the currently supported trees\
\n    open (F, \">$f\");\n    print F \">a\\nxxx\\\
n>b\\nyyyyy\\n\";\n    close (F);\n    print STDOU\
T (\"**** Supported Guide tree modes:\\n\");\n    \
my_system (\"t_coffee -other_pg seq_reformat -in $\
f -action +seq2dnd list \");\n    $do_exit=1;\n  }\
\n\nif ($method2use eq \"list\")\n  {\n    my %ml;\
\n    my $listfile=\"$tmpdir/list\";\n\n    $ml{tc\
offee}=1;\n    $ml{psicoffee}=1;\n    $ml{accurate\
}=1;\n    $ml{'3dcoffee'}=1;\n    $ml{expresso}=1;\
\n    $ml{clustalo}=1;\n    $ml{mafft}=1;\n    $ml\
{famsa}=1;\n    print STDOUT (\"**** Supported MSA\
 mode:\\n\");\n    my_system (\"t_coffee 2>/dev/nu\
ll | grep _msa > $listfile\");\n    open (F, $list\
file);\n    while (<F>)\n      {\n      	my $l=$_;\
\n      	$l=~/(.*_msa)\\s+(.*)/;\n      	my $m=$1;\
\n      	my $i=\"$2\\n\";\n      	if ($m=~/mafftsp\
arsescore/)\n        	  {\n        	   printf STDO\
...
$f\\n\";}\n	    ## $f=~/(\\W)+ (\\d)+/;\n	    @dyn\
amicFile = split ' ', $f;\n	    #if ($VERBOSE){pri\
nt \"\\n![dynamic.pl] --- -dynamic_config --- $dyn\
amicFile[0] :: $dynamicFile[1]\\n\";}\n	    $metho\
d{$dynamicFile[0]} = $dynamicFile[1];\n	  }\n	clos\
e(F);\n      }\n    else\n      {\n	$method{\"psic\
offee_msa\"}=50;\n	$method{\"famsa_msa\"}=10000000\
00;\n      }\n\n    foreach my $name (sort { $meth\
od{$a} <=> $method{$b} } keys %method)\n      {\n	\
if ($NSEQ<=$method{$name})\n	  {\n	    $method2use\
=$name;\n	    last;\n	  }\n      }\n  }\n\n\n\nif \
($tree)\n  {\n    ($h2,$treeF)=tempfile();\n    my\
 ($h2,$tmptree)=tempfile();\n    push (@tmpL,$tree\
F);\n    # If default tree is used\n    if ( $tree\
 eq \"default\"){$treeF=0;}\n    # If tree s provi\
ded as a file\n    elsif ( -e $tree)\n      {\n	  \
      my_system (\"cp $tree $tmptree\");\n      }\\
n    elsif ($tree eq \"master\" || $tree eq \"main\
\" || $tree eq \"parent\")\n      {\n      	if ($E\
NV{CHILD_TREEF_4_TCOFFEE} && -e $ENV{CHILD_TREEF_4\
_TCOFFEE})\n      	  {\n      	    my $ctree=$ENV{\
CHILD_TREEF_4_TCOFFEE};\n      	    my_system (\"m\
v $ctree $tmptree\");\n      	  }\n      	else\n  \
    	  {\n      	    my $master_tree=$ENV{CHILD_TR\
EE_4_TCOFFEE};\n      	    my_system (\"t_coffee -\
other_pg seq_reformat -in $master_tree -in2 $infil\
e -action +prune_tree -output newick > $tmptree\")\
;\n      	  }\n      }\n    else\n      {\n	      \
  my_system (\"t_coffee -other_pg seq_reformat -in\
 $infile -action +seq2dnd $tree -output newick> $t\
mptree\");\n      }\n\n    if ($method2use=~/mafft\
/)\n      {\n	       my_system (\"t_coffee -other_\
pg seq_reformat -in $tmptree -output mafftdndmatri\
x> $treeF\");\n      }\n    else\n      {\n	      \
 my_system (\"mv $tmptree $treeF\");\n      }\n  }\
\n\n\nchdir ($tmpdir);\n\nmy $CL4tc=get_cl4tc();#w\
ill collect from env every CLTCOFEE env variable\n\
\nif (!$treeF || $NSEQ<=2){$treeFlag=\"\";}\nelsif\
 ( $method2use=~/coffee/ || $method2use=~/accurate\
/){$treeFlag=\"-usetree $treeF \";}\nelsif ( $meth\
od2use=~/clustalo/){$treeFlag=\"--guidetree-in=$tr\
eeF \";}\nelsif ( $method2use=~/mafftsparsecore/){\
;}\nelsif ( $method2use=~/mafft/){$treeFlag=\"--tr\
eein $treeF \";}\nelsif ( $method2use=~/famsa/){$t\
reeFlag=\"-gt import $treeF \";}\n$CL4tc.=\" $tree\
Flag \";\n\n$threadFlag=($thread)?\"--thread $thre\
ad \":\"--thread 1 \";\n$threadFlag4tc=($thread)?\\
"-thread $thread \":\"-thread 1 \";\n$threadFlag4f\
amsa=($thread)?\"-t $thread \":\"-t 1 \";\n$CL4tc.\
=\" $threadFlag4tc \";\n\n\nmy $cmethod=$method2us\
e;\n$cmethod=~s/_pair/_msa/;\n$cmethod=~s/_msa//;\\
n\nif ($VERBOSE){print \"\\n![dynamic.pl] --- cmet\
hod == $cmethod\\n\";}\n\nif ($cmethod eq \"tcoffe\
e\"|| $cmethod eq \"t_coffee\" )\n  { my string=fi\
le2string($infile);\n    if ($VERBOSE){print \"\\n\
![dynamic.pl] --- RUNNING ALIGNMENT \\n $string \"\
...
e $threadFlag $QUIET\");\n  }\nelsif ($cmethod =~/\
sparsecore/)\n  {\n    my_system (\"mafft-sparseco\
re.rb -i $infile > $outfile $QUIET\");\n  }\nelsif\
 (($cmethod =~/mafft/))\n  {\n    my $mm;\n    my \
$retree;\n\n    if ( $cmethod eq \"mafft\" || $cme\
thod=~/\\-/ )\n      {\n	$mm=$cmethod;\n      }\n \
   elsif (($cmethod=~/mafft(.*)/))\n      {\n	$mm=\
\"mafft-\".$1;\n      }\n\n    if ($mm =~/1/)\n   \
   {\n	$mm=~s/1/i/;\n	$retree=\"--retree 1 \"\n   \
   };\n\n    my_system (\"$mm --anysymbol $threadF\
lag $treeFlag $retree $infile > $outfile $QUIET\")\
;\n  }\n\nelsif ($method2use=~/famsa/)\n  {\n    p\
rint \"\\n![dynamic.pl] --- FAMSA DEFAULT\\n\";\n \
   my_system (\"famsa $treeFlag $threadFlag4famsa \
$infile $outfile >/dev/null $QUIET\");\n  }\nelsif\
 ($method2use=~/famsaUpgma/)\n  {\n    print \"\\n\
![dynamic.pl] --- FAMSA Upgma\\n\";\n    print \"\\
\n![dynamic.pl] --- Command: famsa -gt upgma $tree\
Flag $threadFlag4famsa $infile $outfile >/dev/null\
 $QUIET\\n\";\n    my_system (\"famsa -gt upgma $t\
reeFlag $threadFlag4famsa $infile $outfile >/dev/n\
ull $QUIET\");\n  }\nelse\n  {\n    if ($treeF)\n \
     {\n	printf (STDERR \"WARNING: Method $method2\
use CANNOT use pre-sepecified guide tree [dynamic.\
pl]\\n\");\n      }\n    my_system (\"t_coffee -in\
 $infile -method $method2use -outfile $outfile -ou\
tput fasta_aln $tcarg -quiet $QUIET\");\n  }\n\nif\
 ( ! -e $outfile)\n  {\n    print \"ERROR - No MSA\
 computed - $LAST_COM -- [FATAL:dynamic.pl]\\n\";\\
n    my_exit ($CDIR,$EXIT_FAILURE);\n  }\nelsif ( \
$flush)\n {\n   open (F, $outfile);\n   while (<F>\
){print $_;}\n   close (F);\n }\nforeach my $f (@t\
mpL){unlink($f);}\n\n\nif ($VERBOSE!=-1)\n  {\n   \
 open (F, \"$stderrF\");\n    while (<F>)\n      {\
\n	my $l=$_;\n	if ( $VERBOSE || $l=~/WARNING/ || $\
l=~/ERROR/ || $l=~/INFORNATION/){print stderr \"$l\
\";}\n      }\n    close (F);\n  }\n\nmy_exit ($CD\
IR,$EXIT_SUCCESS);\n\nsub file2nseq\n  {\n    # In\
...
n\nsub file2string\n    {\n      my ($f)=@_;\n    \
 ...
 while (<F>)\n	{\n	  $s.=$_;\n	}\n      close (F);\
\n      chomp($s);\n      return $s;\n    }\n\nsub\
 get_psicl\n      {\n	my ($psitrim, $psitrim_mode,\
 $pisN);\n	my $cl;\n\n	if ($ENV{psitrim_tree_4_TCO\
FFEE}){$cl.=\" -psitrim_tree=\".$ENV{psitrim_tree_\
4_TCOFFEE}.\" \";}\n	if ($ENV{psitrim_mode_4_TCOFF\
EE}){$cl.=\" -psitrim_mode=\".$ENV{psitrim_mode_4_\
TCOFFEE}.\" \";}\n	if ($ENV{psitrim_4_TCOFFEE}){$c\
l.=\" -psitrim=\".$ENV{psitrim_4_TCOFFEE}.\" \";}\\
n	if ($ENV{psiJ_4_TCOFFEE}){$cl.=\" -psiJ=\".$ENV{\
psiJ_4_TCOFFEE}.\" \";}\n\n\n	return $cl;\n      }\
\n\nsub get_cl4tc\n	{\n	  my $cl;\n\n	  foreach my\
 $arg (keys(%ENV))\n	    {\n	      if ($arg=~/(.*)\
_4_CLTCOFFEE/)\n      		{\n      		  my $name=$1;\\
n      		  my $val=$ENV{$arg};\n      		  if (-e $\
val){$val=file2abs($val);}\n\n\n      		  if ($val\
 eq \"FLAGSET\"){$val=\"\";}\n      		  $cl.=\"-$n\
ame $val \";\n      		}\n	    }\n	  #if ($VERBOSE)\
...
\n      	  #print \"\\n![dynamic.pl] --- Processed\
 $NSEQ\\n\";\n      	  #print \"\\n![dynamic.pl] -\
-- \";\n      	  foreach my $arg (@ARGV)\n      	 \
   {\n      	      print \"$arg \";\n             \
 print(\"LUISAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\")\\
n      	    }\n\n      	  #print \"\\n![dynamic.pl\
] --- EXIT: $ec ($EXIT_SUCCESS:success, $EXIT_FAIL\
URE:failure)-- Verbose mode -- unset VERBOSE_4_DYN\
AMIC to turn verbose mode off\\n\";\n      	}\n\n\\
n      chdir ($dir);\n      exit ($ec);\n    }\n\n\
sub my_system\n  {\n    my ($com)=@_;\n    $LAST_C\
OM=$com;\n\n    if ($VERBOSE){print \"\\n![dynamic\
.pl] --- SysCall --- $com\\n\";}\n\n    system ($c\
om);\n  }\nprint(\"LUISAAAAAAAAAAAAAAAAAAAAAAAAAAA\
AAAAAA\")\nmy $program=\"T-COFFEE (Version_13.45.6\
0.cd84d2a)\";\n","use Env;\nuse FileHandle;\nuse C\
wd;\nuse File::Path;\nuse Sys::Hostname;\nmy $f = \
new FileHandle;\n\nopen ($f, $ARGV[1]);\n$atom=$AR\
GV[0];\n\n$atom=~s/PRIME/\\'/;\nwhile (<$f>)\n  {\\
...
adapt it to your need so that the method \n*you wa\
nt to use can be incorporated within T-Coffee\n***\
**************************************************\
**\n*                  USAGE                      \
        *\n***************************************\
****************\n*This file is passed to t_coffee\
 via -in:\n*\n*	t_coffee -in Mgeneric_method.metho\
d\n*\n*	The method is passed to the shell using th\
e following\n*call:\n*<EXECUTABLE><PARAM1><IN_FLAG\
><seq_file><PARAM2><OUT_FLAG><outname><PARAM>\n*\n\
*Conventions:\n*<FLAG_NAME> 	<TYPE>		<VALUE>\n*<VA\
LUE>:	no_name 	<=> Replaced with a space\n*<VALUE>\
:	&nbsp	<=> Replaced with a space\n*\n************\
*******************************************\n*    \
              ALN_MODE                           *\
\n************************************************\
*******\n*pairwise   ->all Vs all (no self )[(n2-n\
)/2aln]\n*m_pairwise ->all Vs all (no self)[n^2-n]\
...
****************************************\n* mode f\
or the output:\n*External methods: \n* aln -> alig\
nmnent File (Fasta or ClustalW Format)\n* lib-> Li\
b file (TC_LIB_FORMAT_01)\n*Internal Methods:\n* f\
L -> Internal Function returning a List (Librairie\
)\n* fA -> Internal Function returning an Alignmne\
nt\n*\nOUT_MODE		aln\n****************************\
***************************\n*                  SE\
Q_TYPE                           *\n**************\
*****************************************\n*G: Gen\
omic, S: Sequence, P: PDB, R: Profile\n*Examples:\\
n*SEQTYPE	S	sequences against sequences (default)\\
n*SEQTYPE	S_P	sequence against structure\n*SEQTYPE\
	P_P	structure against structure\n*SEQTYPE	PS	mix \
of sequences and structure	\n*\nSEQ_TYPE	S\n*\n\n*\
**************************************************\
****\n*                COMMAND LINE               \
          *\n*EXECUTABLE PARAM1 IN_FLAG OUT_FLAG P\
ARAM             *\n******************************\
*************************\n***********************\
********************************\n*               \
   EXECUTABLE                         *\n*********\
**********************************************\n*n\
ame of the executable\n*passed to the shell: execu\
table\n*	\nEXECUTABLE	tc_generic_method.pl\n*\n***\
**************************************************\
**\n*                  IN_FLAG                    \
         *\n**************************************\
*****************\n*IN_FLAG\n*flag indicating the \
name of the in coming sequences\n*IN_FLAG S no_nam\
e ->no flag\n*IN_FLAG S &bnsp-in&bnsp -> \" -in \"\
\n*\nIN_FLAG		-infile=\n*\n***********************\
********************************\n*               \
   OUT_FLAG                           *\n*********\
**********************************************\n*O\
UT_FLAG\n*flag indicating the name of the out-comi\
ng data\n*same conventions as IN_FLAG\n*OUT_FLAG	S\
 no_name ->no flag\n*if you want to redirect, pass\
 the parameters via PARAM1\n*set OUT_FLAG to >\n*\\
nOUT_FLAG		-outfile=\n*\n*************************\
******************************\n*                 \
 PARAM_1                              *\n*********\
**********************************************\n*<\
...
ECUTABLE and specified *before* IN_FLAG \n*If ther\
e is more than 1 PARAM line, the lines are\n*conca\
tenated\n*Command_line: @EP@PARAM@-gapopen%e10%s-g\
apext%e20\n*	%s white space\n*	%e equal sign\n*\n*\
PARAM1	\n*\n*\n*\n********************************\
***********************\n*                  PARAM_\
2                              *\n****************\
***************************************\n*<EXECUTA\
BLE><PARAM1><IN_FLAG><seq_file><PARAM2><OUT_FLAG><\
outname><PARAM>\n*Parameters sent to the EXECUTABL\
E and specified \n*after* IN_FLAG and *before* OUT\
_FLAG\n*If there is more than 1 PARAM line, the li\
nes are\n*concatenated\n*\n*PARAM1	\n*\n*\n*******\
************************************************\n\
*                  PARAM                          \
    *\n*******************************************\
...
s sent to the EXECUTABLE and specified *after* OUT\
_FLAG\n*If there is more than 1 PARAM line, the li\
nes are\n*concatenated\n*\nPARAM	-mode=seq_msa -me\
thod=clustalw2\nPARAM   -OUTORDER=INPUT -NEWTREE=c\
ore -align -gapopen=-15\n*\n**********************\
*********************************\n*              \
    END                                *\n********\
***********************************************\n"\
,"*TC_METHOD_FORMAT_01\n***************clustalw_me\
...
k=$ENV{\"DEBUG_LOCK\"};\nour $LOCKDIR=$ENV{\"LOCKD\
IR_4_TCOFFEE\"};\nif (!$LOCKDIR){$LOCKDIR=getcwd()\
;}\nour $ERRORDIR=$ENV{\"ERRORDIR_4_TCOFFEE\"};\no\
ur $ERRORFILE=$ENV{\"ERRORFILE_4_TCOFFEE\"};\n&set\
_lock ($$);\nif (isshellpid(getppid())){lock4tc(ge\
tppid(), \"LLOCK\", \"LSET\", \"$$\\n\");}\n      \
\nour $SILENT=\" >/dev/null 2>/dev/null\";\nour $I\
NTERNET=-1;\n\n\n\n\n\n\n\nour $BLAST_MAX_NRUNS=2;\
\nour $EXIT_SUCCESS=0;\nour $EXIT_FAILURE=1;\nour \
$CONFIGURATION=-1;\nour $REF_EMAIL=\"\";\nour $PRO\
GRAM=\"extract_from_pdb\";\n\n\nmy %onelett_prot=&\
fill_onelett_prot();\nmy %threelett_prot=&fill_thr\
eelett_prot();\nmy %onelett_RNA=&fill_onelett_RNA(\
);\nmy %threelett_RNA=&fill_threelett_RNA();\nmy %\
onelett_DNA=&fill_onelett_DNA();\nmy %threelett_DN\
A=&fill_threelett_DNA();\n\n\n\n\n\nmy %onelett = \
(\n'P' => \\%onelett_prot,\n'D' => \\%onelett_DNA,\
\n'R' => \\%onelett_RNA\n);\n\n\nmy %threelett = (\
\n'P' => \\%threelett_prot,\n'D' => \\%threelett_D\
NA,\n'R' => \\%threelett_RNA\n);\n\n\n\n\n\n\n\nif\
...
HELP/ || $ARGV[0]=~/Man/ || $ARGV[0] eq \"-h\"  ||\
 $ARGV[0] eq \"-H\"  )\n{die \"SYNTAX: extract_fro\
m_pdb Version $VersionTag	\n	Minimum:             \
[extract_from_pdb file] \n			   OR \n			     [... \
| extract_from_pdb]\n 	Flags (Default setting on t\
he first line)\n	   -version...................[Re\
turns the Version Number]\n           -force......\
...............[Forces the file to be treated like\
 a PDB file]\n                                    \
  [Regenerates the header and SEQRES fields]\n    \
       -force_name................[Forces the file\
 to be named after name]]\n           -infile.....\
file...........[Flag can be omited]\n			          \
    [File must be pdb or fro pgm]\n               \
                       [File can also be compresse\
d Z or gz]\n                                      \
[In the case of a compressed file, you can omit th\
e gz|Z extension]\n           -netfile............\
...
curl must be installed]\n                         \
             [ftp://ftp.gnu.org/pub/gnu/wget/]\n  \
                                    [http://curl.h\
axx.se/]\n                                      [M\
ust also be used to retrieve the file from a local\
 pdb copy (cf netaddress)]\n           -netaddress\
................[Address used for the retrieving t\
he netfile]\n                                     \
 [http://www.rcsb.org/pdb/cgi/export.cgi/%%.pdb.gz\
?format=PDB&pdbId=%%&compression=gz]\n            \
                          [http://www.expasy.ch/cg\
i-bin/get-pdb-entry.pl?%%]\n                      \
                [local -> will get the file from p\
db_dir (see pdb_dir)]\n           -netcompression.\
...........[Extension if the netfile comes compres\
sed]\n                                      [gz]\n\
           -pdb_dir...................[address of \
the repertory where the pdb is installed]\n       \
                               [Supports standard \
ftp style installation OR every stru in DIR]\n    \
                                  [Give the ..../p\
db/structure/ dir]\n                              \
        [If value omitted, the pg gets it from the\
 env variable PDB_DIR]\n           -netcompression\
_pg.........[gunzip]\n           -is_pdb_name.....\
.....name.[Returns 1 if the name is a PDB ID, 0 ot\
herwise]\n           -model_type...........name.[R\
eturns the model type if valid PDB name]\n        \
   -is_released_pdb_name name.[Returns 1 if the na\
me corresponds to a released PDB file]\n          \
 -get_pdb_chains.....name...[Returns the list of c\
hains corresponding to the entry]\n           -get\
_pdb_id.........name...[Returns the PDB id within \
the provided pdb file]\n           -get_fugue_name\
.....name...[Turns a name into a name valid for fu\
gue]\n                                      [Uses \
the netaddress to do so]\n	   -chain......FIRST...\
.......[Extract the first chain only]\n		       A \
B C..........[Extract Several chains if needed]\n	\
	       ALL............[Extract all the chains]	\n\
           -ligand.....ALL............[Extract the\
 ligands in the chain (HETATM)]\n                 \
      <name1>,<name2>[Extract All the named lignds\
]\n	   -ligand_only...............[Extract only th\
e ligands]\n           -ligand_list...............\
[Extract the list of ligands]\n	   -coor.......<st\
art>..<end>.[Coordinates of the fragment to extrac\
t]\n			              [Omit end to include the Cter\
]\n           -num........absolute.......[absolute\
: relative to the seq] \n                       fi\
le...........[file: relative to file]\n           \
-num_out....new............[new: start 1->L]\n    \
                   old............[old: keep the f\
ile coordinates]\n           -delete.....<start>..\
<end>.[Delete from residue start to residue end]\n\
	   -atom.......CA.............[Atoms to include, \
ALL for all of them]\n		       CA O N.........[Ind\
icate several atoms if needed]\n	   -code.......3.\
.............[Use the 1 letter code or the 3 lette\
rs code]\n	   -mode.......raw............[Output o\
riginal pdb file]\n                       pdb.....\
.......[Output something that looks like pdb]\n		 \
      fasta..........[Output the sequences in fast\
a format]\n		       simple.........[Output a forma\
t easy to parse in C ]\n            -seq_field..AT\
OM...........[Field used to extract the sequence]\\
n		       SEQRES.........[Use the complete sequenc\
e]\n	   -seq.......................[Equivalent to \
 -mode fasta]\n	   -model......1..............[Cho\
sen Model in an NMR file]\n           -nodiagnosti\
c..............[Switches Error Messages off]\n    \
       -debug.....................[Sets the DEBUG \
ON]\n           -no_remote_pdb_dir.........[Do not\
 look for a remote file]\n           -cache_pdb...\
..............[Cache Value, default is $HOME/.t_co\
ffee/cache, other values: NO<=> No cache]\n\n     \
 Environement Variables\n           These variable\
s can be set from the environement\n           Com\
mand line values with the corresponding flag super\
seed evironement value\n           NO_REMOTE_PDB_D\
IR..........[Prevents the program from searching r\
emote file: faster]\n           PDB_DIR...........\
.........[Indicates where PDB file must be fetched\
 (localy)]\n\n	 PROBLEMS: please contact cedric.no\
tredame\\@europe.com\\n\";\n	 exit ($EXIT_SUCCESS)\
;\n}\n\n$np=0;\n$n_para=$#ARGV;\n$model=1;\n$pdb_d\
ir=$ENV{'PDB_DIR'};if ($pdb_dir){$pdb_dir.=\"/\";}\
\n$debug=$ENV{'DEBUG_EXTRACT_FROM_PDB'};\n\n$no_re\
mote_pdb_dir=$ENV{NO_REMOTE_PDB_DIR};\n$HOME=$ENV{\
'HOME'};\nif ( $ENV{CACHE_4_TCOFFEE})\n{$cache=$EN\
V{CACHE_4_TCOFFEE};}\nelse\n{\n    $cache=\"$HOME/\
.t_coffee/cache/\";\n}\n\n   \n$netaddress=\"https\
://files.rcsb.org/download/%%.pdb.gz\";\n$netcompr\
ession_pg=\"gunzip\";\n$netcompression=\"gz\";\n\n\
...
ach $c (@c_chain)\n{\n\n	if    ( $seq_field eq \"S\
EQRES\"){@pdb_seq=@{$complete_seq{$c}};}\n	elsif (\
 $seq_field eq \"ATOM\")  {@pdb_seq=@{$atom_seq{$c\
}};}\n	\n\n	$full_length=$l=$#pdb_seq+1;\n		\n	if \
( $real_end{$c}==\"*\"){$real_end{$c}=$full_length\
;}\n	if ( $coor_set)\n{	   \n\n	   if ( $real_end{\
$c} < $l){splice @pdb_seq, $real_end{$c}, $l;}\n	 \
  if ( $real_start{$c} < $l){splice @pdb_seq, 0, $\
real_start{$c}-1;}	  	   \n	   $l=$#pdb_seq;\n}\n\\
n	elsif ( $delete_set)\n{\n	   splice @pdb_seq, $d\
elete_start, $delete_end-$delete_start+1;\n	   $l=\
$#pdb_seq;\n}\n	\n	$new_fasta_name=\"$pdb_id$c\";\\
n	if ( $coor_set)\n{\n	   if ( $n_pdb_chains==0){$\
new_fasta_name=\"$new_fasta_name$c\";}\n	   $new_f\
asta_name= $new_fasta_name.\"\\_$start\\_$end\";\n\
}\n	   \n	if ( $MODE eq \"pdb\")\n{\n	   $nl=1;\n	\
   $n=0;\n	   \n	   foreach $res ( @pdb_seq)\n		{\\
n		if ( !$n)\n		{\n		\n		 printf \"SEQRES %3d %1s \
%4d  \", $nl,$c, $l;\n		 $nl++;\n	}\n	     $res=~s\
/\\s//g;\n	     \n	     if ($code==1){ printf \"%3\
s \",$onelett{$molecule_type{$c}}->{$res};}\n	    \
...
ID} ||$ligand_list {'ALL'} || !&is_aa($RES_ID,$cha\
in)))\n{\n	    $line_header=\"HETATM\";\n}\n	elsif\
 ($line=~/^HETATM/ && (&is_aa($RES_ID,$chain) && !\
$no_hetatm))\n{\n	    $line_header=\"ATOM\";\n	   \
...
sif ( $already_read{$CHAIN}{$RES_ID}{$AT_ID}{$RES_\
NO}){next;}\n	else{$already_read{$CHAIN}{$RES_ID}{\
$AT_ID}{$RES_NO}=1;}\n	\n	$KEY=\"ALL\";\n\n      	\
if ( $RES_NO ==0){$start_at_zero=1;}\n\n	$RES_NO+=\
$start_at_zero;    \n	\n	if ( $current_chain ne $C\
HAIN)\n{\n	    $current_chain=$CHAIN;\n	    $pos=$\
current_residue=0;\n	    $offset=($coor_set)?($rea\
l_start{$CHAIN}-1):0;\n	    if    ( $seq_field eq \
...
= $RES_NO)\n{\n	    $current_residue=$RES_NO;\n	  \
  if    ( $seq_field eq \"SEQRES\"){$pos=$current_\
residue;}\n	    elsif ( $seq_field eq \"ATOM\"){$p\
os++;}\n}\n	\n	\n	if ($n_atom==0 || $atom_list{$AT\
_ID}==1 || $atom_list{$KEY}==1)\n{ 	\n	    \n	    \
$do_it=(!@c_chain || $hc_chain{$CHAIN} ||$hc_chain\
{'LIGAND'} );\n	    \n	    $do_it= ($do_it==1) && \
($coor_set==0 ||($pos>=$real_start{$CHAIN} && $pos\
<=$real_end{$CHAIN}));\n	    $do_it= ($do_it==1) &\
& ($delete_set==0 || $pos<$delete_start ||$pos>$de\
lete_end );\n	    if ($ligand==0 && $line_header e\
q \"HETATM\" ){$do_it=0;}\n	    if ($ligand_only==\
1 && $line_header eq \"ATOM\" ){$do_it=0;}\n	    i\
f ($ligand==1 && $line_header eq \"HETATM\" && $li\
gand_list{$RES_ID}==0 && $ligand_list{\"ALL\"}==0)\
{$do_it=0;} \n	    \n	    \n	    if ( $do_it)\n{\n\
		$n++;\n		$out_pos=$pos;\n		\n	       if ( $delet\
...
2f\\n\",$line_header, $n, $RAW_AT_ID,$RES_ID,$CHAI\
N,$out_pos, $X, $Y, $Z,$TEMP;		  \n}\n	       elsi\
f ( $MODE eq \"simple\")\n{\n		    if ( $RES_ID eq\
 \"\"){$RES_ID=\"X\";}\n		  printf \"%-6s %5s %s %\
2s %4d    %8.3f %8.3f %8.3f\\n\",$line_header, $AT\
_ID, $RES_ID,($CHAIN eq\"\" || $CHAIN eq \" \")?\"\
A\":$CHAIN,$out_pos, $X, $Y, $Z,$TEMP;\n}\n\n}\n}\\
n}\n}\nprint \"\\n\";\nclose($INFILE);\n\n\nif ( $\
error ne \"\") \n{$error=$error.\"\\nDiagnostic:  \
  SEQRES and the residues in ATOM are probably Inc\
ompatible\\n\";\n    $error=$error.  \"Recomendati\
on: Rerun with '-fix 1' in order to ignore the SEQ\
RES sequences\\n\";\n}\nif (!$nodiagnostic){print \
STDERR $error;}\n&myexit ( $EXIT_SUCCESS);\n\nsub \
get_pdb_entry_type_file\n  {\n    my $cache_file=\\
"$cache/pdb_entry_type.txt\";\n    my $env_file  =\
 $ENV{\"PDB_ENTRY_TYPE_FILE\"};\n    my $pdb_file \
 =\"$ENV{'PDB_DIR'}/derived_data/pdb_entry_type.tx\
t\";\n    \n    \n    if (-z $cache_file){unlink (\
$cache_file);}#will get updated\n    if (-z $env_f\
ile){$env_file=\"\";}    #cannot update\n    if (-\
z $pdb_file){$pdb_file=\"\";}    #cannot update\n \
   \n    if    (-e $env_file){return $env_file;} #\
env wins: user decides\n    elsif (-e $pdb_file){r\
eturn $pdb_file;} #local database wins: network fi\
le may be out of sync\n    elsif ($no_remote_pdb_d\
ir==1)\n      {\n	if (-e $cache_file){return $cach\
e_file;}\n	else\n	  {add_warning($$,$$,\"PDB_ENTRY\
_TYPE_FILE must be set to the location of <pdb>/de\
rived_data/pdb_entry_type.txt when using NO_REMOTE\
_PDB_DIR=1\");\n	   return \"\";\n	 }\n      }\n  \
  else #update can only take place if the file liv\
es in cache\n      {\n	my $new_file;\n	if (!-e $ca\
che_file || (-M $cache_file)>1)\n	  {\n	    $new_f\
ile=vtmpnam();\n	    &url2file(\"ftp://ftp.wwpdb.o\
rg/pub/pdb/derived_data/pdb_entry_type.txt\", $new\
_file);\n	    if ( !-z $new_file){system (\"mv $ne\
w_file $cache_file\"); unlink ($new_file); $new_fi\
le=$cache_file;}\n	    else {unlink($new_file);}\n\
	  }\n	else\n	  {\n	    $new_file=$cache_file;\n	 \
 }\n	\n	if (!-e $cache_file && !-e $new_file)\n	  \
{\n	    add_warning($$,$$,\"Could not download ftp\
://ftp.wwpdb.org/pub/pdb/derived_data/pdb_entry_ty\
pe.txt\");\n	    return \"\";\n	  }\n	elsif (-e $c\
ache_file && !-e $new_file)\n	  {\n	    my $m=(-M \
$cache_file);\n	    add_warning($$,$$,\"Could not \
update file ftp://ftp.wwpdb.org/pub/pdb/derived_da\
ta/pdb_entry_type.txt. Older Version [$cache_file]\
($m Month(s) old) will be used instead\");\n	    r\
eturn $cache_file;\n	  }\n	else\n	  {\n	    return\
 $new_file;\n	  }\n      }\n  }\n\n\n\nsub get_unr\
ealeased_file\n  {\n    my $cache_file=\"$cache/un\
released_entries.json.gz\";\n    my $env_file  = $\
ENV{\"PDB_UNREALEASED_FILE\"};\n    my $pdb_file  \
=\"$ENV{'PDB_DIR'}/derived_data/unreleased_entries\
.json.gz\";\n    \n    \n    if ($env_file eq \"NO\
\" || $env_file eq \"No\" || $env_file eq \"no\" |\
| $env_file eq \"0\"){return \"NO\";}\n\n    if (-\
z $cache_file){unlink ($cache_file);}#will get upd\
...
url2file(\"https://ftp.rcsb.org/pub/pdb/holdings/u\
nreleased_entries.json.gz\",$env_file);\n	    if (\
 -e $env_file && !-z $env_file){return $env_file;}\
\n	  }\n	else\n	  {\n	    add_warning($$,$$,\"UNRE\
ALEASED_FILE must be set to the location of your u\
nrealeased.xml file as downloaded from https://ftp\
.rcsb.org/pub/pdb/holdings/unreleased_entries.json\
.gz when using NO_REMOTE_PDB_DIR=1\");\n	    retur\
n \"\";\n	  }\n      }\n    else #update can only \
take place if the file lives in cache\n      {\n	m\
y $new_file=vtmpnam ();\n	if (!-e $cache_file || (\
-M $cache_file)>1)\n	  {\n	    &url2file(\"https:/\
/ftp.rcsb.org/pub/pdb/holdings/unreleased_entries.\
json.gz\",$new_file);\n	    if ( !-z $new_file){sy\
stem (\"mv $new_file $cache_file\"); unlink ($new_\
file); $new_file=$cache_file;}\n	    else {unlink(\
$new_file);}\n	  }\n	else\n	  {\n	    $new_file=$c\
ache_file;\n	  }\n	\n	if (!-e $cache_file && !-e $\
new_file)\n	  {\n	    add_warning($$,$$,\"Could no\
t download https://ftp.rcsb.org/pub/pdb/holdings/u\
nreleased_entries.json.gz\");\n	    return \"\";\n\
	  }\n	elsif (-e $cache_file && !-e $new_file)\n	 \
 {\n	    my $m=(-M $cache_file);\n	    add_warning\
($$,$$,\"Could not update file https://ftp.rcsb.or\
g/pub/pdb/holdings/unreleased_entries.json.gz. Old\
er Version [$cache_file]($m Month(s) ) will be use\
d\");\n	    return $cache_file;\n	  }\n	else\n	  {\
\n	    return $new_file;\n	  }\n      }\n  }\n\nsu\
b is_released \n  {\n    my ($r);\n    my $in=@_[0\
];\n    my $name=&remote_is_pdb_name ($in);\n    m\
y $hold=&remote_is_on_hold($in);\n    \n    $r=($n\
ame && !$hold)?1:0;\n    return $r;\n  }\n\nsub re\
mote_is_pdb_name \n  {\n    my $in=@_[0];\n    my \
...
db_entry_type_file();\n    \n    if ( $in=~/[^\\w\\
\d\\:\\_]/){return 0;}\n    elsif (!-e $ref_file)\\
n      {\n	add_warning ($$,$$,\"Cannot find pdb_en\
try_type.txt;  $in is assumed to be valid; add ftp\
://ftp.wwpdb.org/pub/pdb/derived_data/pdb_entry_ty\
pe.txt in $cache to automatically check name statu\
s\");\n	return 1;\n      }\n    else\n      {\n	$p\
db=substr ($in,0, 4);\n	chomp(($value1=`grep -c $p\
db $ref_file`));\n	$pdb=lc($pdb);\n	chomp(($value2\
=`grep -c $pdb $ref_file`));\n	$value=($value1 || \
$value2)?1:0;\n	$value=($value>0)?1:0;\n	\n	return\
 $value;\n      }\n  }\n\n\n\nsub pdb2model_type\n\
{\n    my $in=@_[0];\n    my ($ref_file, $pdb);\n \
   my ($value, $ret);\n\n    if ( $in=~/[^\\w\\d\\\
:\\_]/){return 0;}\n    $ref_file=&get_pdb_entry_t\
ype_file();\n    if (!-e $ref_file)\n      {\n	add\
_warning ($$,$$,\"Cannot find pdb_entry_type.txt; \
 $in is assumed to be diffraction; add ftp://ftp.w\
wpdb.org/pub/pdb/derived_data/pdb_entry_type.txt i\
n $cache to check name status\");\n	return \"diffr\
action\";\n      }\n    else\n      {\n	$pdb=subst\
r ($in,0, 4);\n	$pdb=lc($pdb);\n	\n	chomp(($value=\
`grep $pdb $ref_file`));\n	\n	$value=~/^\\S+\\s+\\\
S+\\s+(\\S+)/;\n	$ret=$1;\n	if ( $ret eq\"\"){retu\
...
e $ref_file)\n      {\n	add_warning ($$,$$,\"Canno\
t find https://ftp.rcsb.org/pub/pdb/holdings/unrel\
eased_entries.json.gz;  $in is assumed to be relea\
sed;\");\n	return 1;\n      }\n    \n    $pdb=subs\
...
n\n    if (&pg_is_installed (\"curl\")){$pg=\"curl\
\"; $flag=\"-o\";$arg=$curl_arg;}\n    elsif (&pg_\
is_installed (\"wget\"))   {$pg=\"wget\"; $flag=\"\
-O\";$arg=$wget_arg;}\n    \n    return safe_syste\
m (\"$pg $flag$out $address >/dev/null 2>/dev/null\
\");\n\n}\n\n\n\n\nsub pdbfile2chaintype\n  {\n   \
 my $file=@_[0];\n    my %ct;\n    my $F;\n    \n \
   $F=vfopen ($file, \"r\");\n    while (<$F>)\n  \
    {\n	my $line=$_;\n	if ($line =~/^ATOM/)\n	  {\\
n	    my $C=substr($line,21,1);\n	    if (!$ct{$C}\
)\n	      {	\n		my $r=substr($line,17,3);\n		$r=~s\
/\\s+//;\n		if (length ($r)==1){$ct{$C}=\"R\";}\n	\
	elsif (length ($r)==2){$ct{$C}=\"D\";}\n		elsif (\
length ($r)==3){$ct{$C}=\"P\";}\n		else \n		  {\n	\
	    myexit(flush_error(\"ERROR: Could not read RE\
S_ID field in file $file\"));\n		  }\n	      }\n	 \
 }\n      }\n    close ($F);\n    return %ct;\n  }\
...
\n	my $r1=\"$pdbdir/derived_data/pdb_entry_type.tx\
t\";\n	my $r2=$ref_file;\n	if    (-e $r1){$ref_fil\
e=$r1;}\n	elsif (-e $r2){$ref_file=$r2;}\n	else\n	\
  {\n	    my $p=substr ($in,0, 4);\n	    add_warni\
ng ($$, $$, \"Cannot find pdb_entry_type.txt;  $p \
is assumed to be valid; add ftp://ftp.wwpdb.org/pu\
b/pdb/derived_data/pdb_entry_type.txt in $cache to\
 check name status\");\n	  }\n      }\n    elsif (\
 !-e $ref_file || (-M $ref_file)>$max || -z $ref_f\
ile)\n      {\n	&url2file(\"ftp://ftp.wwpdb.org/pu\
b/pdb/derived_data/pdb_entry_type.txt\", $ref_file\
);\n      }\n    $pdb=substr ($in,0, 4);\n    chom\
p(($value1=`grep -c $pdb $ref_file`));\n    $pdb=l\
c($pdb);\n    chomp(($value2=`grep -c $pdb $ref_fi\
...
DATE_VERSION\n\nour $EXIT_SUCCESS=0;\nour $EXIT_FA\
ILURE=1;\nour $INTERNET=0;\n\nour $CP=\"cp \"; #wa\
s causing a crash on MacOSX\nour $SILENT=\">/dev/n\
ull 2>/dev/null\";\nour $WEB_BASE=\"http://www.tco\
ffee.org\";\nour $TCLINKDB_ADDRESS=\"$WEB_BASE/Res\
ources/tclinkdb.txt\";\nour $OS=get_os();\nour $RO\
OT=&get_root();\nour $CD=cwd();\nour $CDIR=$CD;\no\
...
rint \"!!!!!!! ./install [optional:target] -recomp\
ile            [forces the recompilation of T-Coff\
...
e installed - default ~/.t_coffee]\\n\";\n    prin\
t \"!!!!!!! ./install [optional:target] -repo=/pat\
h/to/repo   [binaries repository root directory]\\\
n\";\n    print \"!!!!!!! ./install [optional:targ\
et] -email=<your email>   [needed for remote BLAST\
]\\n\";\n    print \"!!!!!!! ./install [optional:t\
arget] -proxy=<proxy>   [may be needed to access r\
emote services]\\n\";\n    \n    print \"!!!!!!! m\
...
FLAGS=(.*)/)){$CFLAGS=$1;}\n    if ( ($cl=~/-CXXFL\
AGS=(.*)/)){$CXXFLAGS=$1;}\n  }\nour ($ROOT_INSTAL\
L, $NO_QUESTION, $default_update_action,$BINARIES_\
ONLY,$force, $default_update_action, $INSTALL_DIR,\
 $PLUGINS_DIR, $DISTRIBUTIONS,$tclinkdb, $proxy, $\
clean);\nif ( ($cl=~/-root/)){$ROOT_INSTALL=1;}\ni\
f ( ($cl=~/-no_question/)){$NO_QUESTION=1;}\nif ( \
...
. \\n\"; exit 1; } \n}\n\n\nif(!$TCDIR) { $TCDIR=\\
"$HOME/.t_coffee\"; }\n&add_dir ($TCDIR);\n&add_di\
r ($TCCACHE=\"$TCDIR/cache\");\n&add_dir ($TCTMP=\\
"$CDIR/tmp\");\n&add_dir ($TCM=\"$TCDIR/mcoffee\")\
;\n&add_dir ($TCMETHODS=\"$TCDIR/methods\");\n&add\
_dir ($TCPLUGINS=\"$TCDIR/plugins/$OS\");\n\n\nour\
 $BASE=\"$CD/bin\";\nour $BIN=\"$BASE/cache/binari\
es/$OS\";\nour $DOWNLOAD_DIR=\"$BASE/cache/downloa\
d\";\nour $DOWNLOAD_FILE=\"$DOWNLOAD_DIR/files\";\\
nour $TMP=\"$BASE/cache/tmp\";\n\n&add_dir($BASE);\
...
TALL_DIR && !$ROOT_INSTALL){$INSTALL_DIR=\"$TCDIR/\
bin/$OS\";mkpath ($INSTALL_DIR);}\nelsif (!$INSTAL\
L_DIR &&  $ROOT_INSTALL){$INSTALL_DIR=\"/usr/local\
/bin/\";}\n\nif (-d \"mcoffee\"){`cp mcoffee/* $TC\
M`;}\n\n\nour $ENV_FILE=\"$TCDIR/.t_coffee_env\";\\
nunlink ($ENV_FILE);\n&add2env_file ($ENV_FILE,\"E\
MAIL_4_TCOFFEE\", $email);\n&add2env_file ($ENV_FI\
LE,\"http_proxy_4_TCOFFEE\", $proxy);\n&env_file2p\
utenv ($ENV_FILE);\n&set_proxy($proxy);\n\n\n\nmy \
($target, $p, $r);\n$target=$p;\n\nforeach $p (  (\
(keys (%PG)),(keys(%MODE)),(@smode)) )\n  {\n    i\
f ($ARGV[0] eq $p && $target eq \"\"){$target=$p;}\
...
\n      }\n    if ($i==0)\n      {\n      print \"\
One of the following packages must be installed to\
 proceed: \";\n      foreach my $pg (@app_list)\n	\
{\n	  print (\"$pg \");\n	}\n      die;\n    }\n  \
}\n\n\n\n\n\n\n&sign_license_ni();\n\n\n$PG{C}{com\
piler}=get_C_compiler($CC);\n$PG{Fortran}{compiler\
}=get_F_compiler($FC);\n$PG{CXX}{compiler}=$PG{CPP\
}{compiler}=$PG{GPP}{compiler}=get_CXX_compiler($C\
XX);\nif ($CXXFLAGS){$PG{CPP}{options}=$PG{GPP}{op\
tions}=$PG{CXX}{options}=$CXXFLAGS;}\nif ($CFLAGS \
ne \"\" ){$PG{C}{options}=$CFLAGS;}\nforeach my $c\
 (keys(%PG))\n  {\n    my $arguments;\n    if ($PG\
{$c}{compiler})\n      {\n	$arguments=\"$PG{$c}{co\
mpiler_flag}=$PG{$c}{compiler} \";\n	if ($PG{$c}{o\
ptions})\n	  {\n	    $arguments.=\"$PG{$c}{options\
_flag}='\" . $PG{$c}{options} . \"' \";\n	  }\n	$P\
G{$c}{arguments}=$arguments;\n      }\n  }\n\nif (\
$PG{$target}){$PG{$target}{install}=1;}\nelse\n  {\
\n    foreach my $pg (keys(%PG))\n      {\n	if ( $\
target eq \"all\" || ($PG{$pg}{mode}=~/$target/))\\
n	  {\n	    $PG{$pg} {install}=1;\n	  }\n      }\n\
  }\n\nforeach my $pg (keys(%PG))\n  {\n    if (!$\
PG{$pg}{update_action}){$PG{$pg}{update_action}=$d\
efault_update_action;}\n    elsif ($PG{$pg}{update\
_action} eq \"never\"){$PG{$pg}{install}=0;}\n    \
if ( $force && $PG{$pg}{install})\n      {\n	`rm $\
BIN/$pg $BIN/$pg.exe $SILENT`;\n      }\n    if ($\
PG{$pg}{update_action} eq \"update\" && $PG{$pg}{i\
nstall}){$PG{$pg}{update}=1;}\n  }\n\nif (($target\
=~/clean/))\n  {\n    print \"------- cleaning exe\
cutables -----\\n\";\n    `rm bin/* $SILENT`;\n   \
 exit ($EXIT_SUCCESS);\n  }\n\nif ( !$PG{$target})\
{print \"------- Installing T-Coffee Modes\\n\";}\\
n\nforeach my $m (keys(%MODE))\n  {\n    if ( $tar\
get eq \"all\" || $target eq $m)\n      {\n	print \
\"\\n------- The installer will now install the $m\
 components $MODE{$m}{description}\\n\";\n	foreach\
 my $pg (keys(%PG))\n	  {\n	    if ( $PG{$pg}{mode\
} =~/$m/ && $PG{$pg}{install})\n	      {\n		if ($P\
G{$pg}{touched}){print \"------- $PG{$pg}{dname}: \
already processed\\n\";}\n		else {$PG{$pg}{success\
...
int \"\\n------- Install $pg\\n\";\n	$PG{$pg}{succ\
ess}=&install_pg($pg);$PG{$pg}{touched}=1;\n      \
}\n  }\nprint \"------- Finishing The installation\
\\n\";\nmy $final_report=&install ($INSTALL_DIR);\\
n\nprint \"\\n\";\nprint \"***********************\
**********************************************\\n\\
";\nprint \"********              INSTALLATION SUM\
MARY          *****************\\n\";\nprint \"***\
**************************************************\
****************\\n\";\nprint \"------- SUMMARY pa\
ckage Installation:\\n\";\nprint \"-------   Execu\
table Installed in: $PLUGINS_DIR\\n\";\n\nforeach \
my $pg (keys(%PG))\n  {\n    if ( $PG{$pg}{install\
...
if     ( $PG{$pg}{new} && !$PG{$pg}{old})         \
            {print \"*------        $PG{$pg}{dname\
                ...
                {print \"*------        $PG{$pg}{d\
name}: updated $bin_status\\n\"  ; $PG{$pg}{status\
}=1;} \n	elsif  (!$PG{$pg}{new} &&  $PG{$pg}{old} \
&& !$PG{$pg}{update}){print \"*------        $PG{$\
pg}{dname}: previous\\n\" ; $PG{$pg}{status}=1;}\n\
	elsif  (!$PG{$pg}{new} &&  $PG{$pg}{old} &&  $PG{\
$pg}{update}){print \"*------        $PG{$pg}{dnam\
e}: failed update (previous installation available\
)\\n\";$PG{$pg}{status}=0;}\n	else                \
                                          {print \\
"*------        $PG{$pg}{dname}: failed installati\
on\\n\";$PG{$pg}{status}=0;}\n      }\n  }\nmy $fa\
ilure;\n\nif ( !$PG{$target}){print \"*------ SUMM\
ARY mode Installation:\\n\";}\nforeach my $m (keys\
(%MODE))\n  {\n  \n    if ( $target eq \"all\" || \
$target eq $m)\n      {\n	my $succesful=1;\n	forea\
ch my $pg (keys(%PG))\n	  {\n	    if (($PG{$pg}{mo\
de}=~/$m/) && $PG{$pg}{install} && $PG{$pg}{status\
}==0)\n	      {\n		$succesful=0;\n		print \"*!!!!!\
!       $PG{$pg}{dname}: Missing\\n\";\n	      }\n\
	  }\n	if ( $succesful)\n	  {\n	    $MODE{$m}{stat\
us}=1;\n	    print \"*------       MODE $MODE{$m}{\
...
\n\n    \n      \nif ($clean==1 && ($BASE=~/instal\
l4tcoffee/) ){print \"*------ Clean Installation D\
irectory: $BASE\\n\";`rm -rf $BASE`;}\nforeach my \
$pg (keys(%PG)){if ($PG{$pg}{install} && $PG{$pg}{\
status}==0){exit ($EXIT_FAILURE);}}\n\nif ($failur\
e)\n  {\n    print \"*****************************\
****************************************\\n\";\n  \
  print \"********     SOME PACKAGES FAILED TO INS\
TALL        *****************\\n\";\n    print \"*\
**************************************************\
******************\\n\";\n    print \"\\nSome of t\
he reported failures may be due to connectivity pr\
oblems\";\n    print \"\\nRerun the installation a\
nd the installer will specifically try to install \
the missing packages\";\n    print \"\\nIf this Fa\
ils, go to the original website and install the pa\
ckage manually\";\n  }\n\nprint \"****************\
**************************************************\
***\\n\";\nprint \"********              FINALIZE \
YOUR INSTALLATION    *****************\\n\";\nprin\
t \"**********************************************\
***********************\\n\";\nprint \"------- You\
r third party executables are in:\\n\"; \nprint \"\
-------       $PLUGINS_DIR:\\n\";\nprint \"-------\
 Your t_coffee exccutable is in\\n\";\nprint \"---\
----       $INSTALL_DIR:\\n\";\nprint \"------- In\
 order to make your installation permanent add the\
se two lines\\n\";\nprint \"export PATH=$INSTALL_D\
IR:\\$PATH\\n\";\nprint \"export PLUGINS_4_TCOFFEE\
=$PLUGINS_DIR:\\n\";\nif ($OS eq \"linux\")\n  {\n\
    print \"-------       to the file: $HOME/.bash\
rc\\n\";\n  }\nelse \n  {\n    print \"-------    \
   to the file: $HOME/.profile\\ OR $HOME/.basrc\"\
;\n  }\nexit ($EXIT_SUCCESS);  \n  \nsub get_CXX_c\
ompiler\n  {\n    my $c=@_[0];\n    my (@clist)=(\\
...
\n	    print \"!!!!!!!! The $p utility must be ins\
talled for this installation to proceed [FATAL]\\n\
\";\n	    die;\n	  }\n      }\n    return 1;\n  }\\
nsub set_proxy\n  {\n    my ($proxy)=(@_);\n    my\
 (@list,$p);\n    \n    @list= (\"HTTP_proxy\", \"\
http_proxy\", \"HTTP_PROXY\", \"ALL_proxy\", \"all\
_proxy\",\"HTTP_proxy_4_TCOFFEE\",\"http_proxy_4_T\
COFFEE\");\n    \n    if (!$proxy)\n      {\n	fore\
ach my $p (@list)\n	  {\n	    if ( ($ENV_SET{$p}) \
|| $ENV{$p}){$proxy=$ENV{$p};}\n	  }\n      }\n   \
...
gle.com -ox >/dev/null 2>/dev/null`;}\n    else\n \
...
rg=\"--tries=2 --connect-timeout=10 --no-check-cer\
tificate $wget_arg\";}\n    elsif  (&pg_is_install\
ed    (\"curl\")){$pg=\"curl\"; $flag=\"-f -o\";$a\
rg=$curl_arg;}\n    else\n      {\n	printf stderr \
...
LL)\n      {\n	`$CP $BIN/* $PLUGINS_DIR`;\n	if (-e\
 \"$BIN/t_coffee\")\n	  {\n	    `$CP $BIN/t_coffee\
 $INSTALL_DIR`;\n	      unlink(\"$PLUGINS_DIR/t_co\
ffee\");\n	  }\n	$copied=1;\n      }\n    else\n  \
    {\n	$copied=&root_run (\"You must be root to f\
inalize the installation\", \"$CP $BIN/* $PLUGINS_\
DIR $SILENT\");\n	if (-e \"$BIN/t_coffee\")\n	  {\\
n	    &root_run (\"You must be root to finalize th\
e installation\", \"$CP $BIN/t_coffee $INSTALL_DIR\
\");\n	    &root_run (\"You must be root to finali\
ze the installation\", \"rm  $PLUGINS_DIR/t_coffee\
\");\n	  }\n      }\n    \n     \n  if ( !$copied)\
\n    {\n      $report=\"*!!!!!! Installation unsu\
ccesful. The executables have been left in $BASE/b\
in\\n\";\n    }\n  elsif ( $copied && $ROOT)\n    \
{\n      $report=\"*------ Installation succesful.\
 Your executables have been copied in $new_bin and\
 are on your PATH\\n\";\n    }\n  elsif ( $copied \
&& !$ROOT)\n    {\n      $report= \"*!!!!!! T-Coff\
ee has been installed in $INSTALL_DIR\\n\";\n     \
 $report= \"*!!!!!! T-Coffee and associated packag\
es have been copied in: $PLUGINS_DIR\\n\";\n      \
$report.=\"*!!!!!! This T-Coffee location is NOT o\
n your PATH sytem variable\\n\";\n      if ( $OS e\
q \"linux\")\n	{\n	  $report.=\"*!!!!!! You can do\
 so by adding the following line in your ~/.bashrc\
 file:\\n\";\n	}\n      else\n	{\n	  $report.=\"*!\
!!!!! You can do so by adding the following line i\
n your ~/.profile file:\\n\";\n	}\n      $report.=\
...
);\n    if ( $default_update_action ne \"update\" \
&& (-e \"$BIN/$pg\" || -e \"$BIN/$pg.exe\" )  ){re\
turn 1;}\n    \n    #\n    # check if the module e\
xists in the repository cache \n    #\n	if( repo_l\
oad($pg) ) {\n		return 1;\n	}\n    \n    if ($pg e\
q \"t_coffee\")  {return   &install_t_coffee_sourc\
...
s/int errno = 0;/int errno; errno = 0;/' Main.cc`;\
\n	&add_C_libraries(\"AlignedFragment.h\", \"vecto\
r\", \"iostream\", \"cstring\",\"cstdlib\");\n	&ad\
d_C_libraries(\"Main.cc\", \"vector\", \"climits\"\
...
s all\");\n	&check_cp(\"./Progs/RNAplfold\", \"$BI\
N\");\n	&check_cp(\"./Progs/RNAalifold\", \"$BIN\"\
);\n	&check_cp(\"./Progs/RNAfold\", \"$BIN\");\n	\\
n	repo_store(\"./Progs/RNAplfold\", \"./Progs/RNAa\
lifold\", \"./Progs/RNAfold\");\n      }\n      \n\
    #\n    # !!! RETREE module\n    #\n    elsif (\
 $pg eq \"retree\")\n      {\n	chdir \"src\";\n	&f\
lush_command (\"cp Makefile.unx Makefile\");\n	&fl\
ush_command (\"make $arguments all\");\n	&flush_co\
mmand (\"make put\");\n	system \"cp ../exe/* $BIN\\
";\n	\n	repo_store(\"retree\", \"../exe\");\n     \
 }\n	\n    chdir $CDIR;\n    return &pg_is_install\
ed ($pg, $BIN);\n  }\n\nsub install_t_coffee_sourc\
e\n  {\n    my ($pg)=(@_);\n    my ($report,$cflag\
s, $arguments, $language, $compiler) ;\n\n    #1-I\
nstall T-Coffee\n    chdir \"t_coffee_source\";\n \
   &flush_command (\"make clean\");\n    print \"\\
\n------- Compiling T-Coffee\\n\";\n    $language=\
$PG{$pg} {language2};\n    $arguments=$PG{$languag\
e}{arguments};\n    \n    if ( $CC ne \"\")\n     \
...
int \"!!!!!!! Compilation of $pg impossible. Will \
...
return 1;}\n    return 0;\n  }\nsub install_binary\
_package\n  {\n    my ($pg)=(@_);\n    my ($base,$\
report,$name, $download, $arguments, $language, $d\
ir);\n    my $isdir;\n    &input_os();\n    \n    \
#\n    # - paolodt - Check if the module exists in\
 the repository cache \n    #\n	if( repo_load($pg)\
 ) {\n	    $PG{$pg}{from_binary}=1;\n		return 1;\n\
	}\n    # - paolodt - end \n    \n    if (!&suppor\
ted_os($OS)){return 0;}\n    if ( $PG{$pg}{binary}\
){$name=$PG{$pg}{binary};}\n    else {$name=$pg;}\\
n    if ($name eq \"t_coffee\")\n      {\n	#check \
if local bin is there\n	if (-e \"./bin/$OS/t_coffe\
e\")\n	  {\n	    print \"\\n------- Installing  T-\
Coffee from Pre-Compiled/Pre-Downloaded $OS binary\
\\n\";\n	    print \"\\n------- If you want to tri\
gger a fresh compilation use -recompile\\n\";\n	  \
  &check_cp (\"./bin/$OS/t_coffee\", $BIN);\n	    \
return &pg_is_installed ($pg, $BIN);\n	  }\n	#try \
to get precompiled binary -- available from MAC is\
 distribution from MAC\n	else\n	  {\n	    $downloa\
d=\"$WEB_BASE/Packages/Binaries/tcoffee/$OS/$name.\
$VERSION\";\n	  }\n      }\n    else\n      {\n	$d\
ownload=\"$WEB_BASE/Packages/Binaries/plugins/$OS/\
$name\";\n      }\n    \n    $base=cwd();\n    chd\
ir $TMP;\n    \n    if (!-e $name)\n      {\n	`rm \
x $SILENT`;\n	if ( url2file(\"$download\",\"x\")==\
$EXIT_SUCCESS)\n	  {\n	    `mv x $name`;\n	  }\n  \
    }\n    \n    if (!-e $name)\n      {\n	print \\
"!!!!!!! $PG{$pg}{dname}: Download of $pg binary f\
ailed\\n\";\n	print \"!!!!!!! $PG{$pg}{dname}: Che\
ck Address: $download\\n\";\n	chdir $base;\n	retur\
n 0;\n      }\n    print \"\\n------- Installing $\
pg\\n\";\n    \n    if ($name =~/tar\\.gz/)\n     \
 {\n	`gunzip  -f $name`;\n	`tar -xvf $pg.tar`;\n	c\
...
CHE = \"$REPO_ROOT/$pg/$VER/$OSNAME-$OSARCH\"; \n \
   if( !-e \"$CACHE/$pg\" ) {\n   	 	print \"-----\
--- Module \\\"$pg\\\" NOT found on repository cac\
he.\\n\";\n    	return 0;\n    }\n    \n    print \
...
;}\n    close ($F);\n  }    \n\nsub input_installa\
tion_directory\n  {\n    my $dir=@_[0];\n    my $n\
ew;\n    \n    print \"------- The current install\
ation directory is: [$dir]\\n\";\n    print \"????\
??? Return to keep the default or new value:\";\n \
  \n    if ($NO_QUESTION==0)\n      {\n	chomp ($ne\
w=<stdin>);\n	while ( $new ne \"\" && !input_yes (\
\"You have entered $new. Is this correct? ([y]/n):\
\"))\n	  {\n	    print \"???????New installation d\
irectory:\";\n	    chomp ($new=<stdin>);\n	  }\n	$\
dir=($new eq \"\")?$dir:$new;\n	$dir=~s/\\/$//;\n \
     }\n    \n    if ( -d $dir){return $dir;}\n   \
 elsif (&root_run (\"You must be root to create $d\
ir\",\"mkdir $dir\")==$EXIT_SUCCESS){return $dir;}\
\n    else\n      {\n	print \"!!!!!!! $dir could n\
ot be created\\n\";\n	if ( $NO_QUESTION)\n	  {\n	 \
   return \"\";\n	  }\n	elsif ( &input_yes (\"????\
??? Do you want to provide a new directory([y]/n)?\
:\"))\n	  {\n	    return input_installation_direct\
ory ($dir);\n	  }\n	else\n	  {\n	    return \"\";\\
n	  }\n      }\n    \n  }\nsub input_yes\n  {\n   \
 my $question =@_[0];\n    my $answer;\n\n    if (\
$NO_QUESTION==1){return 1;}\n    \n    if ($questi\
on eq \"\"){$question=\"??????? Do you wish to pro\
ceed ([y]/n)?:\";}\n    print $question;\n    chom\
p($answer=lc(<STDIN>));\n    if (($answer=~/^y/) |\
| $answer eq \"\"){return 1;}\n    elsif ( ($answe\
r=~/^n/)){return 0;}\n    else\n      {\n	return i\
nput_yes($question);\n      }\n  }\nsub root_run\n\
  {\n    my ($txt, $cmd)=(@_);\n    \n    if ( sys\
tem ($cmd)==$EXIT_SUCCESS){return $EXIT_SUCCESS;}\\
n    else \n      {\n	print \"------- $txt\\n\";\n\
	if ( $ROOT eq \"sudo\"){return system (\"sudo $cm\
d\");}\n	else {return system (\"su root -c \\\"$cm\
d\\\"\");}\n      }\n  }\nsub get_root\n  {\n    i\
f (&pg_is_installed (\"sudo\")){return \"sudo\";}\\
n    else {return \"su\";}\n  }\n\nsub get_os\n  {\
\n    my $raw_os=`uname`;\n    my $os;\n\n    $raw\
_os=lc ($raw_os);\n    \n    if ($raw_os =~/cygwin\
/){$os=\"windows\";}\n    elsif ($raw_os =~/linux/\
...
, $value)=(@_);\n    my $F = new FileHandle;\n    \
my $t;\n    if (!$value){return;}\n    #make sure \
new variables do not get duplicated\n    if ( -e $\
env)\n      {\n	open ($F, \"$env\");\n	while (<$F>\
)\n	  {\n	    my $line=$_;\n	    if (!($line=~/$va\
r/)){$t.=$line;}\n	  }\n	close ($F);\n      }\n   \
 $t.=\"$var=$value\\n\";\n    open ($F, \">$env\")\
...
ffee\"}{\"language\"}=\"C++\";\n$PG{\"t_coffee\"}{\
\"language2\"}=\"CXX\";\n$PG{\"t_coffee\"}{\"sourc\
e\"}=\"http://www.tcoffee.org/Packages/sources/tco\
ffee/stable/T-COFFEE_distribution.tar.gz\";\n$PG{\\
"t_coffee\"}{\"update_action\"}=\"always\";\n$PG{\\
"t_coffee\"}{\"binary\"}=\"t_coffee\";\n$PG{\"t_co\
ffee\"}{\"mode\"}=\"tcoffee,mcoffee,rcoffee,expres\
so,3dcoffee\";\n$PG{\"clustalo\"}{\"4_TCOFFEE\"}=\\
"CLUSTALO\";\n$PG{\"clustalo\"}{\"type\"}=\"sequen\
ce_multiple_aligner\";\n$PG{\"clustalo\"}{\"ADDRES\
S\"}=\"http://www.clustal.org/omega/\";\n$PG{\"clu\
stalo\"}{\"language\"}=\"C++\";\n$PG{\"clustalo\"}\
{\"language2\"}=\"C++\";\n$PG{\"clustalo\"}{\"sour\
ce\"}=\"http://www.clustal.org/omega/clustal-omega\
-1.2.4.tar.gz\";\n$PG{\"clustalo\"}{\"mode\"}=\"mc\
offee\";\n$PG{\"clustalo\"}{\"binary\"}=\"clustalo\
\";\n$PG{\"clustalo\"}{\"version\"}=\"1.2.4\";\n$P\
G{\"strike\"}{\"4_TCOFFEE\"}=\"STRIKE\";\n$PG{\"st\
rike\"}{\"type\"}=\"sequence_alignment_scoring\";\\
n$PG{\"strike\"}{\"ADDRESS\"}=\"http://www.tcoffee\
.org/Projects/strike/index.html\";\n$PG{\"strike\"\
}{\"language\"}=\"C++\";\n$PG{\"strike\"}{\"langua\
ge2\"}=\"CXX\";\n$PG{\"strike\"}{\"source\"}=\"htt\
p://www.tcoffee.org/Projects/strike/strike_v1.2.ta\
r.bz2\";\n$PG{\"strike\"}{\"mode\"}=\"tcoffee,expr\
esso\";\n$PG{\"strike\"}{\"version\"}=\"1.2\";\n$P\
G{\"strike\"}{\"binary\"}=\"strike\";\n$PG{\"clust\
alw2\"}{\"4_TCOFFEE\"}=\"CLUSTALW2\";\n$PG{\"clust\
alw2\"}{\"type\"}=\"sequence_multiple_aligner\";\n\
$PG{\"clustalw2\"}{\"ADDRESS\"}=\"http://www.clust\
al.org\";\n$PG{\"clustalw2\"}{\"language\"}=\"C++\\
";\n$PG{\"clustalw2\"}{\"language2\"}=\"CXX\";\n$P\
G{\"clustalw2\"}{\"source\"}=\"http://www.clustal.\
org/download/2.0.10/clustalw-2.0.10-src.tar.gz\";\\
n$PG{\"clustalw2\"}{\"mode\"}=\"mcoffee,rcoffee\";\
...
\"}=\"mcoffee,rcoffee\";\n$PG{\"clustalw\"}{\"vers\
ion\"}=\"1.82\";\n$PG{\"clustalw\"}{\"binary\"}=\"\
clustalw\";\n$PG{\"dialign-t\"}{\"4_TCOFFEE\"}=\"D\
IALIGNT\";\n$PG{\"dialign-t\"}{\"type\"}=\"sequenc\
e_multiple_aligner\";\n$PG{\"dialign-t\"}{\"ADDRES\
S\"}=\"http://dialign-tx.gobics.de/\";\n$PG{\"dial\
ign-t\"}{\"DIR\"}=\"/usr/share/dialign-tx/\";\n$PG\
{\"dialign-t\"}{\"language\"}=\"C\";\n$PG{\"dialig\
n-t\"}{\"language2\"}=\"C\";\n$PG{\"dialign-t\"}{\\
"source\"}=\"http://dialign-tx.gobics.de/DIALIGN-T\
X_1.0.2.tar.gz\";\n$PG{\"dialign-t\"}{\"mode\"}=\"\
mcoffee\";\n$PG{\"dialign-t\"}{\"binary\"}=\"diali\
gn-t\";\n$PG{\"dialign-t\"}{\"version\"}=\"1.0.2\"\
;\n$PG{\"dialign-tx\"}{\"4_TCOFFEE\"}=\"DIALIGNTX\\
";\n$PG{\"dialign-tx\"}{\"type\"}=\"sequence_multi\
ple_aligner\";\n$PG{\"dialign-tx\"}{\"ADDRESS\"}=\\
"http://dialign-tx.gobics.de/\";\n$PG{\"dialign-tx\
\"}{\"DIR\"}=\"/usr/share/dialign-tx/\";\n$PG{\"di\
align-tx\"}{\"language\"}=\"C\";\n$PG{\"dialign-tx\
\"}{\"language2\"}=\"C\";\n$PG{\"dialign-tx\"}{\"s\
ource\"}=\"http://dialign-tx.gobics.de/DIALIGN-TX_\
1.0.2.tar.gz\";\n$PG{\"dialign-tx\"}{\"mode\"}=\"m\
coffee\";\n$PG{\"dialign-tx\"}{\"binary\"}=\"diali\
gn-tx\";\n$PG{\"dialign-tx\"}{\"version\"}=\"1.0.2\
\";\n$PG{\"poa\"}{\"4_TCOFFEE\"}=\"POA\";\n$PG{\"p\
oa\"}{\"type\"}=\"sequence_multiple_aligner\";\n$P\
G{\"poa\"}{\"ADDRESS\"}=\"http://www.bioinformatic\
s.ucla.edu/poa/\";\n$PG{\"poa\"}{\"language\"}=\"C\
\";\n$PG{\"poa\"}{\"language2\"}=\"C\";\n$PG{\"poa\
...
"PROBCONS\";\n$PG{\"probcons\"}{\"type\"}=\"sequen\
ce_multiple_aligner\";\n$PG{\"probcons\"}{\"ADDRES\
S\"}=\"http://probcons.stanford.edu/\";\n$PG{\"pro\
bcons\"}{\"language2\"}=\"CXX\";\n$PG{\"probcons\"\
}{\"language\"}=\"C++\";\n$PG{\"probcons\"}{\"sour\
ce\"}=\"http://probcons.stanford.edu/probcons_v1_1\
2.tar.gz\";\n$PG{\"probcons\"}{\"mode\"}=\"mcoffee\
\";\n$PG{\"probcons\"}{\"binary\"}=\"probcons\";\n\
$PG{\"probcons\"}{\"version\"}=\"1.12\";\n$PG{\"ms\
aprobs\"}{\"4_TCOFFEE\"}=\"MSAPROBS\";\n$PG{\"msap\
robs\"}{\"type\"}=\"sequence_multiple_aligner\";\n\
$PG{\"msaprobs\"}{\"ADDRESS\"}=\"http://msaprobs.s\
ourceforge.net/homepage.htm#latest\";\n$PG{\"msapr\
obs\"}{\"language2\"}=\"CXX\";\n$PG{\"msaprobs\"}{\
\"language\"}=\"C++\";\n$PG{\"msaprobs\"}{\"source\
\"}=\"https://sourceforge.net/projects/msaprobs/fi\
les/MSAProbs-MPI/MSAProbs-MPI_rel1.0.5.tar.gz\";\n\
$PG{\"msaprobs\"}{\"mode\"}=\"mcoffee\";\n$PG{\"ms\
aprobs\"}{\"binary\"}=\"msaprobs\";\n$PG{\"msaprob\
s\"}{\"version\"}=\"1.05\";\n$PG{\"msaprobs\"}{\"u\
pdate_action\"}=\"never\";\n$PG{\"upp\"}{\"4_TCOFF\
EE\"}=\"UPP\";\n$PG{\"upp\"}{\"type\"}=\"sequence_\
multiple_aligner\";\n$PG{\"upp\"}{\"ADDRESS\"}=\"h\
ttp://www.cs.utexas.edu/users/phylo/software/upp/\\
";\n$PG{\"upp\"}{\"language2\"}=\"CXX\";\n$PG{\"up\
p\"}{\"language\"}=\"C++\";\n$PG{\"upp\"}{\"source\
\"}=\"https://github.com/smirarab/pasta/archive/up\
p.zip\";\n$PG{\"upp\"}{\"mode\"}=\"mcoffee\";\n$PG\
{\"upp\"}{\"binary\"}=\"upp\";\n$PG{\"upp\"}{\"ver\
sion\"}=\"1\";\n$PG{\"upp\"}{\"update_action\"}=\"\
never\";\n$PG{\"famsa\"}{\"4_TCOFFEE\"}=\"FAMSA\";\
\n$PG{\"famsa\"}{\"type\"}=\"sequence_multiple_ali\
gner\";\n$PG{\"famsa\"}{\"ADDRESS\"}=\"https://git\
hub.com/refresh-bio/FAMSA\";\n$PG{\"famsa\"}{\"lan\
guage\"}=\"C++\";\n$PG{\"famsa\"}{\"language\"}=\"\
C++\";\n$PG{\"famsa\"}{\"source\"}=\"https://githu\
b.com/refresh-bio/FAMSA.git\";\n$PG{\"famsa\"}{\"m\
ode\"}=\"mcoffee,rcoffee\";\n$PG{\"famsa\"}{\"bina\
ry\"}=\"famsa\";\n$PG{\"famsa\"}{\"version\"}=\"1.\
...
//mafft.cbrc.jp/alignment/software/mafft-7.310-wit\
h-extensions-src.tgz\";\n$PG{\"mafft\"}{\"mode\"}=\
\"mcoffee,rcoffee\";\n$PG{\"mafft\"}{\"binary\"}=\\
"mafft.tar.gz\";\n$PG{\"mafft\"}{\"version\"}=\"7.\
310\";\n$PG{\"msa\"}{\"4_TCOFFEE\"}=\"MSA\";\n$PG{\
\"msa\"}{\"type\"}=\"sequence_multiple_aligner\";\\
n$PG{\"msa\"}{\"ADDRESS\"}=\"https://www.ncbi.nlm.\
nih.gov/CBBresearch/Schaffer/msa.html\";\n$PG{\"ms\
a\"}{\"language\"}=\"C\";\n$PG{\"msa\"}{\"language\
\"}=\"C\";\n$PG{\"msa\"}{\"source\"}=\"ftp://ftp.n\
cbi.nih.gov/pub/msa/msa.tar.Z\";\n$PG{\"msa\"}{\"m\
ode\"}=\"mcoffee\";\n$PG{\"msa\"}{\"binary\"}=\"ms\
a.pl\";\n$PG{\"msa\"}{\"version\"}=\"1.0\";\n$PG{\\
"msa\"}{\"update_action\"}=\"never\";\n$PG{\"dca\"\
}{\"4_TCOFFEE\"}=\"DCA\";\n$PG{\"dca\"}{\"type\"}=\
\"sequence_multiple_aligner\";\n$PG{\"dca\"}{\"ADD\
RESS\"}=\"https://bibiserv2.cebitec.uni-bielefeld.\
de/dca\";\n$PG{\"dca\"}{\"language\"}=\"C\";\n$PG{\
\"dca\"}{\"language\"}=\"C\";\n$PG{\"dca\"}{\"sour\
ce\"}=\"https://bibiserv2.cebitec.uni-bielefeld.de\
/applications/dca/resources/downloads/dca-1.1-src.\
tar.gz\";\n$PG{\"dca\"}{\"mode\"}=\"mcoffee\";\n$P\
G{\"dca\"}{\"binary\"}=\"dca.pl\";\n$PG{\"dca\"}{\\
"version\"}=\"1.1\";\n$PG{\"dca\"}{\"update_action\
\"}=\"never\";\n$PG{\"muscle\"}{\"4_TCOFFEE\"}=\"M\
USCLE\";\n$PG{\"muscle\"}{\"type\"}=\"sequence_mul\
tiple_aligner\";\n$PG{\"muscle\"}{\"ADDRESS\"}=\"h\
ttp://www.drive5.com/muscle/\";\n$PG{\"muscle\"}{\\
"language\"}=\"C++\";\n$PG{\"muscle\"}{\"language2\
\"}=\"GPP\";\n$PG{\"muscle\"}{\"source\"}=\"http:/\
/www.drive5.com/muscle/downloads3.7/muscle3.7_src.\
tar.gz\";\n$PG{\"muscle\"}{\"windows\"}=\"http://w\
ww.drive5.com/muscle/downloads3.7/muscle3.7_win32.\
zip\";\n$PG{\"muscle\"}{\"linux\"}=\"http://www.dr\
ive5.com/muscle/downloads3.7/muscle3.7_linux_ia32.\
tar.gz\";\n$PG{\"muscle\"}{\"mode\"}=\"mcoffee,rco\
ffee\";\n$PG{\"muscle\"}{\"version\"}=\"3.7\";\n$P\
G{\"pcma\"}{\"4_TCOFFEE\"}=\"PCMA\";\n$PG{\"pcma\"\
}{\"type\"}=\"sequence_multiple_aligner\";\n$PG{\"\
pcma\"}{\"ADDRESS\"}=\"http://prodata.swmed.edu/pc\
ma/pcma.php\";\n$PG{\"pcma\"}{\"language\"}=\"C\";\
\n$PG{\"pcma\"}{\"language2\"}=\"C\";\n$PG{\"pcma\\
"}{\"source\"}=\"http://prodata.swmed.edu/download\
/pub/PCMA/pcma.tar.gz\";\n$PG{\"pcma\"}{\"mode\"}=\
\"mcoffee\";\n$PG{\"pcma\"}{\"version\"}=\"1.0\";\\
n$PG{\"kalign\"}{\"4_TCOFFEE\"}=\"KALIGN\";\n$PG{\\
"kalign\"}{\"type\"}=\"sequence_multiple_aligner\"\
;\n$PG{\"kalign\"}{\"ADDRESS\"}=\"http://msa.cgb.k\
i.se\";\n$PG{\"kalign\"}{\"language\"}=\"C\";\n$PG\
{\"kalign\"}{\"language2\"}=\"C\";\n$PG{\"kalign\"\
}{\"source\"}=\"http://msa.cgb.ki.se/downloads/kal\
ign/current.tar.gz\";\n$PG{\"kalign\"}{\"mode\"}=\\
"mcoffee\";\n$PG{\"kalign\"}{\"version\"}=\"1.0\";\
\n$PG{\"amap\"}{\"4_TCOFFEE\"}=\"AMAP\";\n$PG{\"am\
ap\"}{\"type\"}=\"sequence_multiple_aligner\";\n$P\
G{\"amap\"}{\"ADDRESS\"}=\"http://bio.math.berkele\
y.edu/amap/\";\n$PG{\"amap\"}{\"language\"}=\"C++\\
";\n$PG{\"amap\"}{\"language2\"}=\"CXX\";\n$PG{\"a\
map\"}{\"source\"}=\"https://github.com/mes5k/amap\
-align/archive/amap.zip\";\n$PG{\"amap\"}{\"mode\"\
}=\"mcoffee\";\n$PG{\"amap\"}{\"version\"}=\"2.0\"\
;\n$PG{\"amap\"}{\"update_action\"}=\"never\";\n$P\
G{\"proda\"}{\"4_TCOFFEE\"}=\"PRODA\";\n$PG{\"prod\
a\"}{\"type\"}=\"sequence_multiple_aligner\";\n$PG\
{\"proda\"}{\"ADDRESS\"}=\"http://proda.stanford.e\
du\";\n$PG{\"proda\"}{\"language\"}=\"C++\";\n$PG{\
\"proda\"}{\"language2\"}=\"CXX\";\n$PG{\"proda\"}\
{\"source\"}=\"http://proda.stanford.edu/proda_1_0\
.tar.gz\";\n$PG{\"proda\"}{\"mode\"}=\"mcoffee\";\\
n$PG{\"proda\"}{\"version\"}=\"1.0\";\n$PG{\"prank\
\"}{\"4_TCOFFEE\"}=\"PRANK\";\n$PG{\"prank\"}{\"ty\
pe\"}=\"sequence_multiple_aligner\";\n$PG{\"prank\\
"}{\"ADDRESS\"}=\"http://www.ebi.ac.uk/goldman-srv\
/prank/\";\n$PG{\"prank\"}{\"language\"}=\"C++\";\\
n$PG{\"prank\"}{\"language2\"}=\"CXX\";\n$PG{\"pra\
nk\"}{\"source\"}=\"http://www.ebi.ac.uk/goldman-s\
rv/prank/src/prank/prank.src.100802.tgz\";\n$PG{\"\
prank\"}{\"mode\"}=\"mcoffee\";\n$PG{\"prank\"}{\"\
version\"}=\"100303\";\n$PG{\"sap\"}{\"4_TCOFFEE\"\
}=\"SAP\";\n$PG{\"sap\"}{\"type\"}=\"structure_pai\
rwise_aligner\";\n$PG{\"sap\"}{\"ADDRESS\"}=\"http\
s://mathbio.crick.ac.uk/wiki/Software#SAP\";\n$PG{\
\"sap\"}{\"language\"}=\"C\";\n$PG{\"sap\"}{\"lang\
uage2\"}=\"C\";\n$PG{\"sap\"}{\"source\"}=\"https:\
//github.com/jkleinj/SAP/archive/v.1.1.3.tar.gz\";\
\n$PG{\"sap\"}{\"mode\"}=\"expresso,3dcoffee\";\n$\
PG{\"sap\"}{\"version\"}=\"1.1.3\";\n$PG{\"sap\"}{\
\"binary\"}=\"sap\";\n$PG{\"TMalign\"}{\"4_TCOFFEE\
\"}=\"TMALIGN\";\n$PG{\"TMalign\"}{\"type\"}=\"str\
ucture_pairwise_aligner\";\n$PG{\"TMalign\"}{\"ADD\
RESS\"}=\"http://zhanglab.ccmb.med.umich.edu/TM-al\
ign/TMalign.f\";\n$PG{\"TMalign\"}{\"language\"}=\\
"Fortran\";\n$PG{\"TMalign\"}{\"language2\"}=\"For\
tran\";\n$PG{\"TMalign\"}{\"source\"}=\"http://zha\
nglab.ccmb.med.umich.edu/TM-align/TMalign.f\";\n$P\
G{\"TMalign\"}{\"linux\"}=\"http://zhanglab.ccmb.m\
ed.umich.edu/TM-align/TMalign_32.gz\";\n$PG{\"TMal\
ign\"}{\"mode\"}=\"expresso,3dcoffee\";\n$PG{\"TMa\
lign\"}{\"version\"}=\"2013.05.11\";\n$PG{\"mustan\
g\"}{\"4_TCOFFEE\"}=\"MUSTANG\";\n$PG{\"mustang\"}\
{\"type\"}=\"structure_pairwise_aligner\";\n$PG{\"\
mustang\"}{\"ADDRESS\"}=\"http://lcb.infotech.mona\
sh.edu.au/mustang/\";\n$PG{\"mustang\"}{\"language\
\"}=\"C++\";\n$PG{\"mustang\"}{\"language2\"}=\"CX\
X\";\n$PG{\"mustang\"}{\"source\"}=\"http://lcb.in\
fotech.monash.edu.au/mustang/mustang_v3.2.3.tgz\";\
\n$PG{\"mustang\"}{\"mode\"}=\"expresso,3dcoffee\"\
;\n$PG{\"mustang\"}{\"version\"}=\"3.2.3\";\n$PG{\\
"lsqman\"}{\"4_TCOFFEE\"}=\"LSQMAN\";\n$PG{\"lsqma\
n\"}{\"type\"}=\"structure_pairwise_aligner\";\n$P\
G{\"lsqman\"}{\"ADDRESS\"}=\"empty\";\n$PG{\"lsqma\
n\"}{\"language\"}=\"empty\";\n$PG{\"lsqman\"}{\"l\
anguage2\"}=\"empty\";\n$PG{\"lsqman\"}{\"source\"\
}=\"empty\";\n$PG{\"lsqman\"}{\"update_action\"}=\\
"never\";\n$PG{\"lsqman\"}{\"mode\"}=\"expresso,3d\
coffee\";\n$PG{\"align_pdb\"}{\"4_TCOFFEE\"}=\"ALI\
GN_PDB\";\n$PG{\"align_pdb\"}{\"type\"}=\"structur\
e_pairwise_aligner\";\n$PG{\"align_pdb\"}{\"ADDRES\
S\"}=\"empty\";\n$PG{\"align_pdb\"}{\"language\"}=\
\"empty\";\n$PG{\"align_pdb\"}{\"language2\"}=\"em\
pty\";\n$PG{\"align_pdb\"}{\"source\"}=\"empty\";\\
n$PG{\"align_pdb\"}{\"update_action\"}=\"never\";\\
n$PG{\"align_pdb\"}{\"mode\"}=\"expresso,3dcoffee\\
";\n$PG{\"fugueali\"}{\"4_TCOFFEE\"}=\"FUGUE\";\n$\
PG{\"fugueali\"}{\"type\"}=\"structure_pairwise_al\
igner\";\n$PG{\"fugueali\"}{\"ADDRESS\"}=\"http://\
mizuguchilab.org/fugue/\";\n$PG{\"fugueali\"}{\"la\
...
ce\"}=\"http://www.tbi.univie.ac.at/RNA/packages/s\
ource/ViennaRNA-2.1.9.tar.gz\";\n$PG{\"RNAplfold\"\
}{\"mode\"}=\"rcoffee,\";\n$PG{\"RNAplfold\"}{\"bi\
nary\"}=\"RNAplfold.tar.gz\";\n$PG{\"RNAplfold\"}{\
\"version\"}=\"2.1.9\";\n$PG{\"retree\"}{\"4_TCOFF\
EE\"}=\"PHYLIP\";\n$PG{\"retree\"}{\"type\"}=\"Phy\
...
://www.tcoffee.org/Packages/mirrors/source/phylip-\
3.66.tar.gz\";\n$PG{\"retree\"}{\"mode\"}=\"trmsd,\
\";\n$PG{\"retree\"}{\"binary\"}=\"retree.tar.gz\"\
;\n$PG{\"retree\"}{\"version\"}=\"3.66\";\n$PG{\"h\
mmtop\"}{\"4_TCOFFEE\"}=\"HMMTOP\";\n$PG{\"hmmtop\\
"}{\"type\"}=\"protein_secondarystructure_predicto\
r\";\n$PG{\"hmmtop\"}{\"ADDRESS\"}=\"www.enzim.hu/\
hmmtop/\";\n$PG{\"hmmtop\"}{\"language\"}=\"C\";\n\
$PG{\"hmmtop\"}{\"language2\"}=\"C\";\n$PG{\"hmmto\
p\"}{\"source\"}=\"http://www.tcoffee.org/Packages\
/mirrors/hmmtop2.1.tgz\";\n$PG{\"hmmtop\"}{\"binar\
y\"}=\"hmmtop\";\n$PG{\"hmmtop\"}{\"update_action\\
"}=\"never\";\n$PG{\"hmmtop\"}{\"mode\"}=\"psicoff\
ee\";\n$PG{\"hmmtop\"}{\"version\"}=\"2.1\";\n$PG{\
\"gorIV\"}{\"4_TCOFFEE\"}=\"GOR4\";\n$PG{\"gorIV\"\
}{\"type\"}=\"protein_secondarystructure_predictor\
\";\n$PG{\"gorIV\"}{\"ADDRESS\"}=\"http://mig.jouy\
.inra.fr/logiciels/gorIV/\";\n$PG{\"gorIV\"}{\"lan\
guage\"}=\"C\";\n$PG{\"gorIV\"}{\"language2\"}=\"C\
\";\n$PG{\"gorIV\"}{\"source\"}=\"http://www.tcoff\
ee.org/Packages/mirrors/GOR_IV.tar.gz\";\n$PG{\"go\
rIV\"}{\"update_action\"}=\"never\";\n$PG{\"gorIV\\
"}{\"mode\"}=\"tcoffee\";\n$PG{\"wublast.pl\"}{\"4\
_TCOFFEE\"}=\"EBIWUBLASTc\";\n$PG{\"wublast.pl\"}{\
\"type\"}=\"protein_homology_predictor\";\n$PG{\"w\
ublast.pl\"}{\"ADDRESS\"}=\"built_in\";\n$PG{\"wub\
last.pl\"}{\"ADDRESS2\"}=\"http://www.ebi.ac.uk/To\
ols/webservices/services/wublast\";\n$PG{\"wublast\
.pl\"}{\"language\"}=\"Perl\";\n$PG{\"wublast.pl\"\
}{\"language2\"}=\"Perl\";\n$PG{\"wublast.pl\"}{\"\
source\"}=\"empty\";\n$PG{\"wublast.pl\"}{\"update\
_action\"}=\"never\";\n$PG{\"wublast.pl\"}{\"mode\\
"}=\"psicoffee,expresso,accurate\";\n$PG{\"blastpg\
p.pl\"}{\"4_TCOFFEE\"}=\"EBIBLASTPGPc\";\n$PG{\"bl\
astpgp.pl\"}{\"type\"}=\"protein_homology_predicto\
r\";\n$PG{\"blastpgp.pl\"}{\"ADDRESS\"}=\"built_in\
\";\n$PG{\"blastpgp.pl\"}{\"ADDRESS2\"}=\"http://w\
ww.ebi.ac.uk/Tools/webservices/services/blastpgp\"\
;\n$PG{\"blastpgp.pl\"}{\"language\"}=\"Perl\";\n$\
PG{\"blastpgp.pl\"}{\"language2\"}=\"Perl\";\n$PG{\
\"blastpgp.pl\"}{\"source\"}=\"empty\";\n$PG{\"bla\
stpgp.pl\"}{\"update_action\"}=\"never\";\n$PG{\"b\
lastpgp.pl\"}{\"mode\"}=\"psicoffee,expresso,accur\
ate\";\n$PG{\"blastall\"}{\"4_TCOFFEE\"}=\"blastal\
l\";\n$PG{\"blastall\"}{\"type\"}=\"protein_homolo\
gy_predictor\";\n$PG{\"blastall\"}{\"ADDRESS\"}=\"\
ftp://ftp.ncbi.nih.gov/blast/executables/LATEST\";\
\n$PG{\"blastall\"}{\"language\"}=\"C\";\n$PG{\"bl\
astall\"}{\"language2\"}=\"C\";\n$PG{\"blastall\"}\
{\"source\"}=\"ftp://ftp.ncbi.nlm.nih.gov/blast/ex\
ecutables/blast+/2.6.0/ncbi-blast-2.6.0+-src.tar.g\
z\";\n$PG{\"blastall\"}{\"update_action\"}=\"never\
\";\n$PG{\"blastall\"}{\"mode\"}=\"psicoffee,expre\
sso,3dcoffee\";\n$PG{\"legacy_blast.pl\"}{\"4_TCOF\
FEE\"}=\"NCBIBLAST\";\n$PG{\"legacy_blast.pl\"}{\"\
type\"}=\"protein_homology_predictor\";\n$PG{\"leg\
acy_blast.pl\"}{\"ADDRESS\"}=\"ftp://ftp.ncbi.nih.\
gov/blast/executables/LATEST\";\n$PG{\"legacy_blas\
t.pl\"}{\"language\"}=\"C\";\n$PG{\"legacy_blast.p\
l\"}{\"language2\"}=\"C\";\n$PG{\"legacy_blast.pl\\
"}{\"source\"}=\"ftp://ftp.ncbi.nlm.nih.gov/blast/\
executables/blast+/2.6.0/ncbi-blast-2.6.0+-src.tar\
.gz\";\n$PG{\"legacy_blast.pl\"}{\"update_action\"\
}=\"never\";\n$PG{\"legacy_blast.pl\"}{\"mode\"}=\\
"psicoffee,expresso,3dcoffee\";\n$PG{\"SOAP::Lite\\
"}{\"4_TCOFFEE\"}=\"SOAPLITE\";\n$PG{\"SOAP::Lite\\
"}{\"type\"}=\"library\";\n$PG{\"SOAP::Lite\"}{\"A\
DDRESS\"}=\"http://cpansearch.perl.org/src/MKUTTER\
/SOAP-Lite-0.710.08/Makefile.PL\";\n$PG{\"SOAP::Li\
te\"}{\"language\"}=\"Perl\";\n$PG{\"SOAP::Lite\"}\
{\"language2\"}=\"Perl\";\n$PG{\"SOAP::Lite\"}{\"s\
ource\"}=\"empty\";\n$PG{\"SOAP::Lite\"}{\"update_\
action\"}=\"never\";\n$PG{\"SOAP::Lite\"}{\"mode\"\
}=\"none\";\n$PG{\"XML::Simple\"}{\"4_TCOFFEE\"}=\\
"XMLSIMPLE\";\n$PG{\"XML::Simple\"}{\"type\"}=\"li\
brary\";\n$PG{\"XML::Simple\"}{\"ADDRESS\"}=\"http\
://search.cpan.org/~grantm/XML-Simple-2.18/lib/XML\
/Simple.pm\";\n$PG{\"XML::Simple\"}{\"language\"}=\
\"Perl\";\n$PG{\"XML::Simple\"}{\"language2\"}=\"P\
erl\";\n$PG{\"XML::Simple\"}{\"source\"}=\"empty\"\
;\n$PG{\"XML::Simple\"}{\"mode\"}=\"psicoffee,expr\
esso,accurate\";\n$PG{\"x3dna\"}{\"4_TCOFFEE\"}=\"\
x3dna\";\n$PG{\"x3dna\"}{\"type\"}=\"RNA_secondary\
structure_predictor\";\n$PG{\"x3dna\"}{\"ADDRESS\"\
}=\"http://x3dna.bio.columbia.edu/\";\n$PG{\"x3dna\
\"}{\"source\"}=\"http://www.tcoffee.org/Packages/\
mirrors/source/x3dna-v2.3-linux-64bit.tar.gz\";\n$\
PG{\"x3dna\"}{\"mode\"}=\"saracoffee\";\n$PG{\"x3d\
na\"}{\"update_action\"}=\"never\";\n$PG{\"fsa\"}{\
\"4_TCOFFEE\"}=\"FSA\";\n$PG{\"fsa\"}{\"type\"}=\"\
sequence_multiple_aligner\";\n$PG{\"fsa\"}{\"ADDRE\
SS\"}=\"http://fsa.sourceforge.net/\";\n$PG{\"fsa\\
"}{\"language\"}=\"C++\";\n$PG{\"fsa\"}{\"language\
2\"}=\"CXX\";\n$PG{\"fsa\"}{\"source\"}=\"http://s\
ourceforge.net/projects/fsa/files/fsa-1.15.3.tar.g\
z/download/\";\n$PG{\"fsa\"}{\"mode\"}=\"mcoffee\"\
;\n$PG{\"fsa\"}{\"version\"}=\"1.15.3\";\n$PG{\"fs\
a\"}{\"update_action\"}=\"never\";\n$PG{\"mus4\"}{\
\"4_TCOFFEE\"}=\"MUS4\";\n$PG{\"mus4\"}{\"type\"}=\
\"sequence_multiple_aligner\";\n$PG{\"mus4\"}{\"AD\
DRESS\"}=\"http://www.drive5.com/muscle/\";\n$PG{\\
"mus4\"}{\"language\"}=\"C++\";\n$PG{\"mus4\"}{\"l\
anguage2\"}=\"GPP\";\n$PG{\"mus4\"}{\"source\"}=\"\
http://www.drive5.com/muscle/muscle4.0_src.tar.gz\\
...
curate\"}{\"name\"}=\"accurate\";\n$MODE{\"seq_ref\
ormat\"}{\"name\"}=\"seq_reformat\";\n\n\n$PG{C}{c\
ompiler}=\"gcc\";\n$PG{C}{compiler_flag}=\"CC\";\n\
$PG{C}{options}=\"\";\n$PG{C}{options_flag}=\"CFLA\
GS\";\n$PG{C}{type}=\"compiler\";\n\n$PG{\"CXX\"}{\
...
offee}{description}=\" for regular multiple sequen\
ce alignments\";\n$MODE{rcoffee} {description}=\" \
for RNA multiple sequence alignments\";\n\n$MODE{p\
sicoffee} {description}=\" for Homology Extended m\
ultiple sequence alignments\";\n$MODE{expresso}{de\
scription}=\" for very accurate structure based mu\
ltiple sequence alignments\";\n$MODE{\"3dcoffee\"}\
{description}=\" for multiple structure alignments\
\";\n$MODE{mcoffee} {description}=\" for combining\
 alternative multiple sequence alignment packages\\
\n------- into a unique meta-package. The installe\
r will upload several MSA packages and compile the\
m\\n\n\";\n\n\n&post_process_PG();\nreturn;\n}\n\n\
sub post_process_PG\n  {\n    my $p;\n    \n    %P\
G=&name2dname (%PG);\n    %MODE=&name2dname(%MODE)\
;\n    foreach $p (keys(%PG)){if ( $PG{$p}{type} e\
q \"compiler\"){$PG{$p}{update_action}=\"never\";}\
}\n    \n  }\n\nsub name2dname\n  {\n    my (%L)=(\
@_);\n    my ($l, $ml);\n    \n    foreach my $pg \
(keys(%L))\n      {\n	$l=length ($pg);\n	if ( $l>$\
ml){$ml=$l;}\n      }\n    $ml+=1;\n    foreach my\
...
");\n    while (<$I>)\n      {\n	$l=$_;\n	if (!($l\
=~/$wordin/)){print $O \"$l\";}\n	elsif ( $wordout\
 ne \"\"){$l=~s/$wordin/$wordout/g;print $O \"$l\"\
;}\n      }\n    close ($O);\n    close ($I);\n   \
 return;\n  }\n\nsub add_C_libraries\n  {\n   my (\
$file,$first,@list)=@_;\n   \n    my $O=new FileHa\
ndle;\n    my $I=new FileHandle;\n    my ($l,$anch\
...
\n\nif ($#ARGV==-1)\n  {\n    print \"clean_cache.\
pl -file <file to add in -dir> -dir=<dir> -size=<v\
...
oceed=1;}\n    $s=uc ($s);\n    if (($dir=~/$s/)){\
$proceed=1;}\n  }\nif ( $proceed==0)\n  {\n    pri\
nt STDERR \"Clean_cache.pl can only clean director\
ies whose absolute path name contains the followin\
g strings:\";\n    foreach $w (@suffix) {print STD\
ERR \"$w \";$w=lc($w); print STDERR \"$w \";}\n   \
 print STDERR \"\\nCannot process $dir\\n\";\n    \
exit (EXIT_FAILURE);\n  }\n\n$name_file=\"$dir/nam\
e_file.txt\";\n$size_file=\"$dir/size_file.txt\";\\
nif ( $force){&create_ref_file ($dir,$name_file,$s\
ize_file);}\nif ($file){&add_file ($dir, $name_fil\
e, $size_file, $file);}\n&clean_dir ($dir, $name_f\
ile, $size_file, $max_size,$max_age);\nexit (EXIT_\
SUCCESS);\n\nsub clean_dir \n  {\n    my ($dir, $n\
ame_file, $size_file, $max_size, $max_age)=@_;\n  \
  my ($tot_size, $size, $f, $s);\n\n  \n    $tot_s\
ize=&get_tot_size ($dir, $name_file, $size_file);\\
n\n    if ( $tot_size<=$max_size){return ;}\n    e\
lse {$max_size/=2;}\n    \n    #recreate the name \
file in case some temprary files have not been pro\
perly registered\n    &create_ref_file ($dir, $nam\
e_file, $size_file, $max_age);\n  \n    $new_name_\
file=&vtmpnam();\n    open (R, \"$name_file\");\n \
   open (W, \">$new_name_file\");\n    while (<R>)\
\n      {\n	my $line=$_;\n	\n	($f, $s)=($line=~/(\\
\S+) (\\S+)/);\n	if ( !($f=~/\\S/)){next;}\n	\n	el\
sif ($max_size && $tot_size>=$max_size && !($f=~/n\
ame_file/))\n	  {\n	    remove ( \"$dir/$f\");\n	 \
   $tot_size-=$s;\n	  }\n	elsif ( $max_age && -M(\\
"$dir/$f\")>=$max_age)\n	  {\n	    remove ( \"$dir\
/$f\");\n	    $tot_size-=$s;\n	  }\n	else\n	  {\n	\
...
e (R);\n    close (W);\n    open (F, \">$size_file\
\");\n    print F \"$tot_size\";\n    if ( -e $new\
_name_file){`mv $new_name_file $name_file`;}\n    \
close (F);\n  }\nsub get_tot_size\n  {\n    my ($d\
ir, $name_file, $size_file)=@_;\n    my $size;\n  \
  \n    if ( !-d $dir){return 0;}\n    if ( !-e $n\
ame_file)\n      {\n	\n	&create_ref_file ($dir, $n\
ame_file, $size_file);\n      }\n    open (F, \"$s\
ize_file\");\n    $size=<F>;\n    close (F);\n    \
chomp ($size);\n    return $size;\n  }\nsub size \\
n  {\n    my $f=@_[0];\n\n    if ( !-d $f){return \
...
ile\");\n    foreach $f (@l)\n      {\n	$s=&size(\\
"$dir/$f\");\n	$tot_size+=$s;\n	print F \"$f $s\\n\
\";\n      }\n    &myecho ($tot_size, \">$size_fil\
e\");\n    close (F);\n  }\nsub add_file \n  {\n  \
  my ($dir,$name_file,$size_file,$file)=@_;\n    m\
y ($s, $tot_size);\n    \n    if ( !-d $dir)   {re\
turn;}\n    if ( !-e \"$dir/$file\" ) {return;}\n \
   if ( !-e $name_file){&create_ref_file ($dir,$na\
me_file,$size_file);}\n					    \n    $s=&size(\"$\
dir/$file\");\n    open (F, \">>$name_file\");\n  \
  print F \"$file\\n\";\n    close (F);\n\n    $to\
t_size=&get_tot_size ($dir,$name_file,$size_file);\
\n    $tot_size+=$s;\n    &myecho ($tot_size, \">$\
size_file\");\n    \n  }\n	\nsub myecho\n  {\n    \
my ($string, $file)=@_;\n    open (ECHO, $file) ||\
 die;\n    print ECHO \"$string\";\n    close (ECH\
O);\n  }\n    \n		\n	\nsub vtmpnam\n  {\n    my $t\
mp_file_name;\n    $tmp_name_counter++;\n    $tmp_\
file_name=\"tmp_file_for_clean_cache_pdb$$.$tmp_na\
me_counter\";\n    $tmp_file_list[$ntmp_file++]=$t\
mp_file_name;\n    if ( -e $tmp_file_name) {return\
 &vtmpnam ();}\n    else {return $tmp_file_name;}\\
n  }\n","\nmy $address=\"http://www.tcoffee.org/Da\
ta/Datasets/NatureProtocolsDataset.tar.gz\";\nmy $\
out=\"NatureProtocolsDataset.tar.gz\";\n&url2file \
($address,$out);\n\nif ( -e $out)\n  {\n    \n    \
system (\"gunzip NatureProtocolsDataset.tar.gz\");\
\n    system (\"tar -xvf NatureProtocolsDataset.ta\
r\");\n  	system (\"rm -rf NatureProtocolsDataset.\
tar\");  \n    print \"Your Data Set is in the Fol\
der 'NatureProtocolsDataset'\\n\";\n  }\nelse \n  \
...
")){$pg=\"curl\"; $flag=\"-o\";$arg=$curl_arg;}\n \
   return system (\"$pg $address $flag $out>/dev/n\
ull 2>/dev/null\");\n\n}\n\nsub pg_is_installed\n \
 {\n    my @ml=@_;\n    my $r, $p, $m;\n    my $su\
pported=0;\n    \n    my $p=shift (@ml);\n    if (\
...
nchop ($type);\n\nif ( $type eq \"PROTEIN\"){$lib_\
mode=\"lalign_rs_s_pair -lalign_n_top 20\";}\nelsi\
f ( $type eq\"DNA\"){$lib_mode=\"lalign_rs_s_dna_p\
air -lalign_n_top 40\";}\n\nif ( !(-e $lib_name))\\
...
y=0 -mocca_interactive -in $lib_name -infile $seq_\
file\";\n  \n  }\n\nsystem ($command);\n\nexit;\n\\
n","my $WSDL = 'http://www.ebi.ac.uk/Tools/webserv\
ices/wsdl/WSDaliLite.wsdl';\n\nuse SOAP::Lite;\nus\
e Data::Dumper;\nuse Getopt::Long qw(:config no_ig\
nore_case bundling);\nuse File::Basename;\n\nmy $c\
heckInterval = 5;\n\nmy %params=(\n	    'async' =>\
 '1', # Use async mode and simulate sync mode in c\
lient\n	    );\nGetOptions(\n    'pdb1=s'     => \\
\$params{'sequence1'},\n    'chainid1=s' => \\$par\
ams{'chainid1'},\n    'pdb2=s'     => \\$params{'s\
equence2'},\n    'chainid2=s' => \\$params{'chaini\
d2'},\n    \"help|h\"	 => \\$help, # Usage info\n \
   \"async|a\"	 => \\$async, # Asynchronous submis\
sion\n    \"polljob\"	 => \\$polljob, # Get result\
...
tus) &&\n    !( defined($params{'sequence1'}) && d\
efined($params{'sequence2'}) )\n    ) {\n    print\
...
E') {\n	print STDERR \"To get results: $scriptName\
 --polljob --jobid $jobid\\n\";\n    }\n}\nelse {\\
n    if(-f $params{'sequence1'}) {\n	$params{'sequ\
ence1'} = read_file($params{'sequence1'});\n    }\\
n    if(-f $params{'sequence2'}) {\n	$params{'sequ\
ence2'} = read_file($params{'sequence2'});\n    }\\
...
s(open (FILE, \">$tmp\")) {\n	return 0;\n    }\n  \
 ...
 return 1;\n}\n\nsub usage {\n    print STDERR <<E\
OF\nDaliLite\n========\n\nPairwise comparison of p\
rotein structures\n\n[Required]\n\n  --pdb1       \
         : str  : PDB ID for structure 1\n  --pdb2\
                : str  : PDB ID for structure 2\n\\
n[Optional]\n\n  --chain1              : str  : Ch\
ain identifer in structure 1\n  --chain2          \
...
-trace	        :      : show SOAP messages being i\
nterchanged \n\nSynchronous job:\n\n  The results/\
errors are returned as soon as the job is finished\
.\n  Usage: $scriptName --email <your\\@email> [op\
tions] pdbFile [--outfile string]\n  Returns: save\
s the results to disk\n\nAsynchronous job:\n\n  Us\
e this if you want to retrieve the results at a la\
ter time. The results \n  are stored for up to 24 \
hours. \n  The asynchronous submission mode is rec\
ommended when users are submitting \n  batch jobs \
or large database searches	\n  Usage: $scriptName \
--email <your\\@email> --async [options] pdbFile\n\
  Returns: jobid\n\n  Use the jobid to query for t\
he status of the job. \n  Usage: $scriptName --sta\
tus --jobid <jobId>\n  Returns: string indicating \
the status of the job:\n    DONE - job has finishe\
d\n    RUNNING - job is running\n    NOT_FOUND - j\
ob cannot be found\n    ERROR - the jobs has encou\
ntered an error\n\n  When done, use the jobid to r\
etrieve the status of the job. \n  Usage: $scriptN\
ame --polljob --jobid <jobId> [--outfile string]\n\
\n[Help]\n\n  For more detailed help information r\
efer to\n  http://www.ebi.ac.uk/DaliLite/\nEOF\n;\\
n}\n","my $WSDL = 'http://www.ebi.ac.uk/Tools/webs\
ervices/wsdl/WSWUBlast.wsdl';\n\nuse strict;\nuse \
...
ams= ( # Defaults\n	      'async' => 1, # Force in\
to async mode\n	      'exp' => 10.0, # E-value thr\
eshold\n	      'numal' => 50, # Maximum number of \
alignments\n	      'scores' => 100, # Maximum numb\
er of scores\n            );\nGetOptions( # Map th\
e options into variables\n    \"program|p=s\"     \
=> \\$params{program}, # BLAST program\n    \"data\
base|D=s\"    => \\$params{database}, # Search dat\
abase\n    \"matrix|m=s\"      => \\$params{matrix\
}, # Scoring matrix\n    \"exp|E=f\"         => \\\
$params{exp}, # E-value threshold\n    \"echofilte\
r|e\"    => \\$params{echofilter}, # Display filte\
red sequence\n    \"filter|f=s\"      => \\$params\
{filter}, # Low complexity filter name\n    \"alig\
nments|b=i\"  => \\$params{numal}, # Number of ali\
gnments\n    \"scores|s=i\"      => \\$params{scor\
es}, # Number of scores\n    \"sensitivity|S=s\" =\
> \\$params{sensitivity}, # Search sensitivity\n  \
  \"sort|t=s\"	      => \\$params{sort}, # Sort hi\
ts by...\n    \"stats|T=s\"       => \\$params{sta\
ts}, # Scoring statistic to use\n    \"strand|d=s\\
"      => \\$params{strand}, # Strand to use in DN\
A vs. DNA search\n    \"topcombon|c=i\"   => \\$pa\
rams{topcombon}, # Consistent sets of HSPs\n    \"\
outfile=s\"       => \\$outfile, # Output file\n  \
  \"outformat|o=s\"   => \\$outformat, # Output fo\
rmat\n    \"help|h\"	      => \\$help, # Usage inf\
...
tp' => 'http://your.proxy.server/'], # HTTP proxy\\
n    timeout => 600, # HTTP connection timeout\n  \
...
}\n\n    # Asynchronous mode: output jobid and exi\
t.\n    if (defined($async)) {\n	print STDOUT $job\
id, \"\\n\";\n        print STDERR \"To check stat\
...
\n	    write_file($outfile.'.'.$selResultType->{ex\
...
ta);\n	close(FILE);\n    }\n}\n\nsub usage {\n    \
print STDERR <<EOF\nWU-BLAST\n========\n\nRapid se\
quence database search programs utilizing the BLAS\
T algorithm.\n   \n[Required]\n\n      --email    \
...
e data file (\"-\" for STDIN)\n\n[Optional]\n\n  -\
m, --matrix	    : str  : scoring matrix\n  -E, --e\
xp	    : real : 0<E<= 1000. Statistical significan\
ce threshold\n                             for rep\
orting database sequence matches.\n  -e, --echofil\
ter  :      : display the filtered query sequence \
in the output\n  -f, --filter	    : str  : activat\
es filtering of the query sequence\n  -b, --alignm\
ents  : int  : number of alignments to be reported\
...
was returned when an asynchronous job \n          \
                  was submitted.\n  -O, --outfile \
   : str  : name of the file results should be wri\
tten to \n                            (default is \
based on the jobid; \"-\" for STDOUT)\n  -o, --out\
format  : str  : txt or xml output (no file is wri\
...
r more detailed help information refer to \nhttp:/\
...
t\n	    );\nGetOptions(\n    \"mode=s\"           \
=> \\$params{mode}, # Search mode: PSI-Blast or PH\
I-Blast\n    \"database|d=s\"     => \\$params{dat\
abase}, # Database to search\n    \"matrix|M=s\"  \
     => \\$params{matrix},# Scoring maxtrix\n    \\
"exp|e=f\"          => \\$params{exp}, # E-value\n\
    \"expmulti|h=f\"     => \\$params{expmulti}, #\
 E-value\n    \"filter|F=s\"       => \\$params{fi\
...
align=i\"          => \\$params{align}, # Alignmen\
t view\n    \"startregion|S=i\"  => \\$params{star\
tregion}, # Start of region in query\n    \"endreg\
ion|H=i\"    => \\$params{endregion}, # End of reg\
ion in query\n    \"maxpasses|j=i\"    => \\$param\
s{maxpasses}, # Number of PSI iterations\n    \"op\
engap|G=i\"      => \\$params{opengap}, # Gap open\
 penalty\n    \"extendgap|E=i\"    => \\$params{ex\
tendgap}, # Gap extension penalty\n    \"pattern=s\
\"        => \\$params{pattern}, # PHI-BLAST patte\
rn\n    \"usagemode|p=s\"    => \\$params{usagemod\
e}, # PHI-BLAST program\n    \"appxml=s\"         \
...
result\n    \"jobid=s\"          => \\$jobid, # Jo\
bId\n    \"outfile=s\"        => \\$outfile, # Out\
put filename\n    \"outformat|o=s\"    => \\$outfo\
rmat, # Output file format\n    \"async|a\"	      \
...
urn new SOAP::SOM;\n    }\n               );\n\nif\
( !($polljob || $status || $ids) &&\n    !( (defin\
ed($ARGV[0]) && -f $ARGV[0]) || defined($sequence)\
...
sults: $scriptName --polljob --jobid $jobid\\n\";\\
n    }\n}  \nelsif($ids && defined($jobid)) {\n   \
...
);\n    # For SOAP::Lite 0.60 and earlier paramete\
rs are passed directly\n    if($SOAP::Lite::VERSIO\
N eq '0.60' || $SOAP::Lite::VERSION =~ /0\\.[1-5]/\
) {\n        $jobid = $soap->runBlastpgp($paramsDa\
ta, $contentData);\n    }\n    # For SOAP::Lite 0.\
69 and later parameter handling is different, so p\
ass\n    # undef's for templated params, and then \
pass the formatted args.\n    else {\n        $job\
id = $soap->runBlastpgp(undef, undef,\n				    $pa\
ramsData, $contentData);\n    }\n\n    if (defined\
($async)) {\n	print STDOUT $jobid, \"\\n\";\n     \
...
ffer;\n    while(sysread(FILE, $buffer, 1024)) {\n\
   ...
   return $content;\n}\n\nsub write_file($$) {\n  \
  my ($tmp,$entity) = @_;\n    print STDERR \"Crea\
ting result file: \".$tmp.\"\\n\";\n    unless(ope\
n (FILE, \">$tmp\")) {\n	return 0;\n    }\n    sys\
write(FILE, $entity);\n    close (FILE);\n    retu\
rn 1;\n}\n\nsub usage {\n    print STDERR <<EOF\nB\
lastpgp\n========\n   \nThe blastpgp program imple\
ments the PSI-BLAST and PHI-BLAST variations\nof N\
CBI BLAST.\n\nFor more detailed help information r\
efer to\nhttp://www.ebi.ac.uk/blastpgp/blastpsi_he\
lp_frame.html\n \nBlastpgp specific options:\n\n[R\
equired]\n\n      --mode            : str  : searc\
h mode to use: PSI-Blast or PHI-Blast\n  -d, --dat\
abase        : str  : protein database to search\n\
  seqFile               : file : query sequence\n\\
n[Optional]\n\n  -M, --matrix          : str  : sc\
oring matrix\n  -e, --exp             : real : Exp\
ectation value\n  -h, --expmulti        : real : t\
hreshold (multipass model)\n  -F, --filter        \
  : str  : filter query sequence with SEG [T,F]\n \
 -m, --align           : int  : alignment view opt\
ion:\n                                 0 - pairwis\
e, 1 - M/S identities,\n                          \
       2 - M/S non-identities, 3 - Flat identities\
,\n                                 4 - Flat non-i\
dentities\n  -G, --opengap         : int  : cost t\
o open a gap\n  -E, --extendgap       : int  : cos\
t to extend a gap\n  -g, --gapalign        : str  \
: Gapped [T,F]\n  -v, --scores          : int  : n\
umber of scores to be reported\n  -j, --maxpasses \
      : int  : number of iterations\n  -X, --dropo\
ff         : int  : Dropoff score\n  -Z, --finaldr\
opoff    : int  : Dropoff for final alignment\n  -\
S, --startregion     : int  : Start of required re\
gion in query\n  -H, --endregion       : int  : En\
d of required region in query\n  -k, --pattern    \
     : str  : Hit File (PHI-BLAST only)\n  -p, --u\
sagemode       : str  : Program option (PHI-BLAST \
only):\n                                 blastpgp,\
 patseedp, seedp\n\n[General]\n\n      --help     \
       :      : prints this help text\n  -a, --asy\
nc           :      : forces to make an asynchrono\
us query\n      --status          :      : poll fo\
r the status of a job\n      --polljob         :  \
    : poll for the results of a job\n      --jobid\
           : str  : jobid of an asynchronous job\n\
      --ids             :      : get hit identifie\
rs for result \n  -O, --outfile         : str  : n\
...
h:\n\n=over\n\n=item *\nL<LWP> 6.35, L<XML::Simple\
> 2.25 and Perl 5.22.0 (MacOS 10.13.6)\n\n=back\n\\
nFor further information see:\n\n=over\n\n=item *\\
nL<https://www.ebi.ac.uk/Tools/webservices/>\n\n=b\
ack\n\n=head1 LICENSE\n\nCopyright 2012-2018 EMBL \
...
tations under the License.\n\nPerl Client Automati\
cally generated with:\nhttps://github.com/ebi-wp/w\
ebservice-clients-generator\n\n=cut\n\nuse strict;\
\nuse warnings;\n\nuse English;\nuse LWP;\nuse XML\
::Simple;\nuse Getopt::Long qw(:config no_ignore_c\
ase bundling);\nuse File::Basename;\nuse Data::Dum\
per;\nuse Time::HiRes qw(usleep);\n\nmy $baseUrl =\
 'https://www.ebi.ac.uk/Tools/services/rest/ncbibl\
...
ze'},       # Word size for wordfinder algorithm\n\
    'compstats=s'     => \\$params{'compstats'},  \
    # Use composition-based statistics.\n    'alig\
n=i'         => \\$params{'align'},          # For\
mating for the alignments\n    'transltable=i'   =\
> \\$params{'transltable'},    # Query Genetic cod\
e to use in translation\n    'stype=s'         => \
\\$params{'stype'},          # Indicates if the se\
quence is protein or DNA/RNA.\n    'sequence=s'   \
   => \\$params{'sequence'},       # The query seq\
uence can be entered directly into this form. The \
sequence can be in GCG, FASTA, EMBL (Nucleotide on\
ly), GenBank, PIR, NBRF, PHYLIP or UniProtKB/Swiss\
-Prot (Protein only) format. A partially formatted\
 sequence is not accepted. Adding a return to the \
end of the sequence may help certain applications \
understand the input. Note that directly using dat\
a from word processors may yield unpredictable res\
ults as hidden/control characters may be present.\\
n    'database=s'      => \\$params{'database'},  \
     # Database\n    # Generic options\n    'email\
=s'         => \\$params{'email'},          # User\
 e-mail address\n    'title=s'         => \\$param\
s{'title'},          # Job title\n    'outfile=s' \
      => \\$params{'outfile'},        # Output fil\
e name\n    'outformat=s'     => \\$params{'outfor\
mat'},      # Output file type\n    'jobid=s'     \
    => \\$params{'jobid'},          # JobId\n    '\
help|h'          => \\$params{'help'},           #\
 Usage help\n    'asyncjob'        => \\$params{'a\
syncjob'},       # Asynchronous submission\n    'p\
olljob'         => \\$params{'polljob'},        # \
Get results\n    'pollFreq=f'      => \\$params{'p\
ollFreq'},       # Poll Frequency\n    'resultType\
s'     => \\$params{'resultTypes'},    # Get resul\
t types\n    'status'          => \\$params{'statu\
s'},         # Get status\n    'params'          =\
> \\$params{'params'},         # List input parame\
ters\n    'paramDetail=s'   => \\$params{'paramDet\
ail'},    # Get details for parameter\n    'multif\
asta'      => \\$params{'multifasta'},     # Multi\
ple fasta input\n    'useSeqId'        => \\$param\
s{'useSeqId'},       # Seq Id file name\n    'maxJ\
obs=i'       => \\$params{'maxJobs'},        # Max\
. parallel jobs\n\n    'verbose'         => \\$par\
ams{'verbose'},        # Increase output level\n  \
  'version'         => \\$params{'version'},      \
  # Prints out the version of the Client and exit.\
\n    'quiet'           => \\$params{'quiet'},    \
      # Decrease output level\n    'debugLevel=i' \
   => \\$params{'debugLevel'},     # Debugging lev\
el\n    'baseUrl=s'       => \\$baseUrl,          \
        # Base URL for service.\n);\nif ($params{'\
verbose'}) {$outputLevel++}\nif ($params{'quiet'})\
 {$outputLevel--}\nif ($params{'pollFreq'}) {$chec\
kInterval = $params{'pollFreq'} * 1000 * 1000}\nif\
...
elsif ($params{'resultTypes'} && defined($params{'\
jobid'})) {\n    &print_result_types($params{'jobi\
d'});\n}\n\nelsif ($params{'polljob'} && defined($\
params{'jobid'})) {\n    &get_results($params{'job\
id'});\n}\n\nelse {\n    # Multiple input sequence\
 mode, assume fasta format.\n    if (defined($para\
ms{'multifasta'}) && $params{'multifasta'}) {\n   \
     &multi_submit_job();\n    }\n\n    # Entry id\
entifier list file.\n    elsif ((defined($params{'\
sequence'}) && $params{'sequence'} =~ m/^\\@/)\n  \
      || (defined($ARGV[0]) && $ARGV[0] =~ m/^\\@/\
)) {\n        my $list_filename = $params{'sequenc\
e'} || $ARGV[0];\n        $list_filename =~ s/^\\@\
//;\n        &list_file_submit_job($list_filename)\
;\n    }\n    # Default: single sequence/identifie\
r.\n    else {\n        # Warn for invalid batch o\
nly option use.\n        if (defined($params{'useS\
eqId'}) && $params{'useSeqId'}) {\n            pri\
nt STDERR \"Warning: --useSeqId option ignored.\\n\
\";\n            delete $params{'useSeqId'};\n    \
    }\n        if (defined($params{'maxJobs'}) && \
$params{'maxJobs'} > 1) {\n            print STDER\
R \"Warning: --maxJobs option ignored.\\n\";\n    \
        $params{'maxJobs'} = 1;\n        }\n      \
  # Load the sequence data and submit.\n        &s\
ubmit_job(&load_data());\n    }\n}\n\n\n\n=head1 F\
UNCTIONS\n\n=cut\n\n\n=head2 rest_user_agent()\n\n\
Get a LWP UserAgent to use to perform REST request\
...
umber = 0;\n    $revisionNumber = \"Revision: \" .\
 $version;\n    $ua->agent(\"EBI-Sample-Client/$re\
visionNumber ($scriptName; $OSNAME) \" . $ua->agen\
t());\n    # Configure HTTP proxy support from env\
ironment.\n    $ua->env_proxy;\n    print_debug_me\
ssage('rest_user_agent', 'End', 21);\n    return $\
ua;\n}\n\n=head2 rest_error()\n\nCheck a REST resp\
onse for an error condition. An error is mapped to\
...
sage . '  ' . $error_message;\n    }\n    print_de\
bug_message('rest_error', 'End', 21);\n}\n\n=head2\
 rest_request()\n\nPerform a REST request (HTTP GE\
...
list_xml = XMLin($param_list_xml_str);\n    my (@p\
aram_list) = @{$param_list_xml->{'id'}};\n    prin\
t_debug_message('rest_get_parameters', 'End', 1);\\
n    return(@param_list);\n}\n\n=head2 rest_get_pa\
rameter_details()\n\nGet details of a tool paramet\
er.\n\n  my $paramDetail = &rest_get_parameter_det\
ails($param_name);\n\n=cut\n\nsub rest_get_paramet\
er_details {\n    print_debug_message('rest_get_pa\
rameter_details', 'Begin', 1);\n    my $parameterI\
d = shift;\n    print_debug_message('rest_get_para\
meter_details',\n        'parameterId: ' . $parame\
terId, 1);\n    my $url = $baseUrl . '/parameterde\
tails/' . $parameterId;\n    my $param_detail_xml_\
str = rest_request($url);\n    my $param_detail_xm\
l = XMLin($param_detail_xml_str);\n    print_debug\
_message('rest_get_parameter_details', 'End', 1);\\
n    return($param_detail_xml);\n}\n\n=head2 rest_\
run()\n\nSubmit a job.\n\n  my $job_id = &rest_run\
($email, $title, \\%params );\n\n=cut\n\nsub rest_\
run {\n    print_debug_message('rest_run', 'Begin'\
, 1);\n    my $email = shift;\n    my $title = shi\
ft;\n    my $params = shift;\n    $email = '' if (\
!$email);\n    print_debug_message('rest_run', 'em\
ail: ' . $email, 1);\n    if (defined($title)) {\n\
        print_debug_message('rest_run', 'title: ' \
. $title, 1);\n    }\n    print_debug_message('res\
t_run', 'params: ' . Dumper($params), 1);\n\n    #\
...
, 'jobid: ' . $job_id, 2);\n    my (@resultTypes);\
\n    my $url = $baseUrl . '/resulttypes/' . $job_\
...
. $type;\n    my $result = &rest_request($url);\n \
   print_debug_message('rest_get_result', length($\
result) . ' characters',\n        1);\n    print_d\
ebug_message('rest_get_result', 'End', 1);\n    re\
...
e('print_param_details', 'Begin', 1);\n    my $par\
amName = shift;\n    print_debug_message('print_pa\
ram_details', 'paramName: ' . $paramName, 2);\n   \
 my $paramDetail = &rest_get_parameter_details($pa\
ramName);\n    print $paramDetail->{'name'}, \"\\t\
\", $paramDetail->{'type'}, \"\\n\";\n    print $p\
aramDetail->{'description'}, \"\\n\";\n    if (def\
ined($paramDetail->{'values'}->{'value'})) {\n    \
    if (ref($paramDetail->{'values'}->{'value'}) e\
q 'ARRAY') {\n            foreach my $value (@{$pa\
ramDetail->{'values'}->{'value'}}) {\n            \
    &print_param_value($value);\n            }\n  \
      }\n        else {\n            &print_param_\
value($paramDetail->{'values'}->{'value'});\n     \
   }\n    }\n    print_debug_message('print_param_\
details', 'End', 1);\n}\n\n=head2 print_param_valu\
e()\n\nPrint details of a tool parameter value.\n\\
n  &print_param_details($param_value);\n\nUsed by \
print_param_details() to handle both singluar and \
...
, \"\\t\",\n                    $value->{'properti\
es'}{'property'}{$key}{'value'}, \"\\n\";\n       \
     }\n            else {\n                print \
\"\\t\", $value->{'properties'}{'property'}{'key'}\
,\n                    \"\\t\", $value->{'properti\
es'}{'property'}{'value'}, \"\\n\";\n             \
   last;\n            }\n        }\n    }\n}\n\n=h\
ead2 print_job_status()\n\nPrint status of a job.\\
n\n  &print_job_status($job_id);\n\n=cut\n\nsub pr\
int_job_status {\n    print_debug_message('print_j\
ob_status', 'Begin', 1);\n    my $jobid = shift;\n\
...
$result eq 'FINISHED' && $outputLevel > 0) {\n    \
    print STDERR \"To get results: perl $scriptNam\
...
types', 'Begin', 1);\n    my $jobid = shift;\n    \
print_debug_message('result_types', 'jobid: ' . $j\
obid, 1);\n    if ($outputLevel > 0) {\n        pr\
...
vel > 0) {\n            print STDOUT 'Available re\
sult types:', \"\\n\";\n        }\n        foreach\
 my $resultType (@resultTypes) {\n            prin\
t STDOUT $resultType->{'identifier'}, \"\\n\";\n  \
          if (defined($resultType->{'label'})) {\n\
                print STDOUT \"\\t\", $resultType-\
>{'label'}, \"\\n\";\n            }\n            i\
f (defined($resultType->{'description'})) {\n     \
...
, 'Begin', 1);\n\n    # Set input sequence\n    $p\
arams{'sequence'} = shift;\n    my $seq_id = shift\
;\n\n    # Load parameters\n    &load_params();\n\\
n    # Submit the job\n    my $jobid = &rest_run($\
params{'email'}, $params{'title'}, \\%params);\n\n\
    # Asynchronous submission.\n    if (defined($p\
arams{'asyncjob'})) {\n        print STDOUT $jobid\
, \"\\n\";\n        if ($outputLevel > 0) {\n     \
       print STDERR\n                \"To check st\
atus: perl $scriptName --status --jobid $jobid\\n\\
";\n        }\n    }\n\n    # Simulate synchronous\
 submission serial mode.\n    else {\n        if (\
$outputLevel > 0) {\n            print STDERR \"Jo\
bId: $jobid\\n\";\n        } else {\n            p\
rint STDERR \"$jobid\\n\";\n        }\n        usl\
eep($checkInterval);\n        # Get results.\n    \
    &get_results($jobid, $seq_id);\n\n    }\n    p\
rint_debug_message('submit_job', 'End', 1);\n    r\
...
rning: Input file \"'\n                . $params{'\
sequence'}\n                . '\" does not exist';\
\n        }\n    }\n\n    # Job identifier trackin\
g for parallel execution.\n    my @jobid_list = ()\
;\n    my $job_number = 0;\n    $/ = '>';\n    for\
each my $filename (@filename_list) {\n        my $\
INFILE;\n        if ($filename eq '-') { # STDIN.\\
n            open($INFILE, '<-')\n                \
or die 'Error: unable to STDIN (' . $! . ')';\n   \
     }\n        else { # File.\n            open($\
INFILE, '<', $filename)\n                or die 'E\
rror: unable to open file '\n                . $fi\
lename . ' ('\n                . $! . ')';\n      \
  }\n        while (<$INFILE>) {\n            my $\
seq = $_;\n            $seq =~ s/>$//;\n          \
  if ($seq =~ m/(\\S+)/) {\n                my $se\
q_id = $1;\n                print STDERR \"Submitt\
ing job for: $seq_id\\n\"\n                    if \
($outputLevel > 0);\n                $seq = '>' . \
$seq;\n                &print_debug_message('multi\
_submit_job', $seq, 11);\n                $job_num\
ber++;\n                my $job_id = &submit_job($\
seq, $seq_id);\n\n                my $job_info_str\
 = sprintf('%s %d %d', $job_id, 0, $job_number);\n\
\n                push(@jobid_list, $job_info_str)\
;\n            }\n\n            # Parallel mode, w\
ait for job(s) to finish to free slots.\n         \
...
params{'maxJobs'} > 1 && scalar(@jobid_list) > 0) \
{\n        &_job_list_poll(\\@jobid_list);\n      \
  print_debug_message('multi_submit_job',\n       \
     'Remaining jobs: ' . scalar(@jobid_list), 1);\
\n    }\n    print_debug_message('multi_submit_job\
', 'End', 1);\n}\n\n\n=head2 _job_list_poll()\n\nP\
oll the status of a list of jobs and fetch results\
 for finished jobs.\n\n  while(scalar(@jobid_list)\
 > 0) {\n    &_job_list_poll(\\@jobid_list);\n  }\\
n\n=cut\n\nsub _job_list_poll {\n    print_debug_m\
essage('_job_list_poll', 'Begin', 1);\n    my $job\
id_list = shift;\n    print_debug_message('_job_li\
st_poll', 'Num jobs: ' . scalar(@$jobid_list),\n  \
      11);\n\n    # Loop though job Id list pollin\
g job status.\n    for (my $jobNum = (scalar(@$job\
id_list) - 1); $jobNum > -1; $jobNum--) {\n       \
 my ($jobid, $seq_id, $error_count, $job_number) =\
\n            split(/\\s+/, $jobid_list->[$jobNum]\
);\n        print_debug_message('_job_list_poll', \
'jobNum: ' . $jobNum, 12);\n        print_debug_me\
ssage('_job_list_poll',\n            'Job info: ' \
. $jobid_list->[$jobNum], 12);\n\n        # Get jo\
b status.\n        my $job_status = &rest_get_stat\
us($jobid);\n        print_debug_message('_job_lis\
t_poll', 'Status: ' . $job_status, 12);\n\n       \
 # Fetch results and remove finished/failed jobs f\
rom list.\n        if (\n            !(\n         \
...
'ERROR' || $job_status eq 'FAILED') {\n           \
     print STDERR\n                    \"Warning: \
job $jobid failed for sequence $job_number: $seq_i\
d\\n\";\n            }\n			# Duplicated getting re\
sults.\n            #&get_results($jobid, $seq_id)\
;\n            splice(@$jobid_list, $jobNum, 1);\n\
        }\n        else {\n\n            # Update \
error count, increment for new error or clear old \
errors.\n            if ($job_status eq 'ERROR') {\
\n                $error_count++;\n            }\n\
            elsif ($error_count > 0) {\n          \
      $error_count--;\n            }\n\n          \
  # Update job tracking info.\n            my $job\
_info_str = sprintf('%s %s %d %d',\n              \
  $jobid, $seq_id, $error_count, $job_number);\n  \
          $jobid_list->[$jobNum] = $job_info_str;\\
n        }\n    }\n    print_debug_message('_job_l\
ist_poll', 'Num jobs: ' . scalar(@$jobid_list),\n \
       11);\n    print_debug_message('_job_list_po\
...
_file_submit_job', 'Begin', 1);\n    my $filename \
= shift;\n\n    # Open the file of identifiers.\n \
   my $LISTFILE;\n    if ($filename eq '-') { # ST\
DIN.\n        open($LISTFILE, '<-')\n            o\
r die 'Error: unable to STDIN (' . $! . ')';\n    \
}\n    else { # File.\n        open($LISTFILE, '<'\
, $filename)\n            or die 'Error: unable to\
 open file ' . $filename . ' (' . $! . ')';\n    }\
\n\n    # Job identifier tracking for parallel exe\
cution.\n    my @jobid_list = ();\n    my $job_num\
...
my $seq_id = $line;\n                print STDERR \
\"Submitting job for: $seq_id\\n\"\n              \
      if ($outputLevel > 0);\n                $job\
_number++;\n                my $job_id = &submit_j\
ob($seq_id, $seq_id);\n                my $job_inf\
o_str =\n                    sprintf('%s %s %d %d'\
, $job_id, $seq_id, 0, $job_number);\n            \
    push(@jobid_list, $job_info_str);\n           \
 }\n            else {\n                print STDE\
RR\n                    \"Warning: line \\\"$line\\
\\" is not recognised as an identifier\\n\";\n    \
        }\n\n            # Parallel mode, wait for\
 job(s) to finish to free slots.\n            whil\
e ($params{'maxJobs'} > 1\n                && scal\
ar(@jobid_list) >= $params{'maxJobs'}) {\n        \
        &_job_list_poll(\\@jobid_list);\n         \
       print_debug_message('list_file_submit_job',\
\n                    'Remaining jobs: ' . scalar(\
@jobid_list), 1);\n            }\n        }\n    }\
\n    close $LISTFILE;\n\n    # Parallel mode, wai\
t for remaining jobs to finish.\n    while ($param\
s{'maxJobs'} > 1 && scalar(@jobid_list) > 0) {\n  \
      &_job_list_poll(\\@jobid_list);\n        pri\
nt_debug_message('list_file_submit_job',\n        \
    'Remaining jobs: ' . scalar(@jobid_list), 1);\\
...
ass default values and fix bools (without default \
...
essage('client_poll', 'Begin', 1);\n    my $jobid \
= shift;\n    my $status = 'PENDING';\n\n    # Che\
ck status and wait if not finished. Terminate if t\
hree attempts get \"ERROR\".\n    my $errorCount =\
...
essage('get_results', 'Begin', 1);\n    my $jobid \
= shift;\n    print_debug_message('get_results', '\
jobid: ' . $jobid, 1);\n    my $seq_id = shift;\n \
   print_debug_message('get_results', 'seq_id: ' .\
 $seq_id, 1) if ($seq_id);\n\n    my $output_basen\
ame = $jobid;\n\n    # Verbose\n    if ($outputLev\
el > 1) {\n        print 'Getting results for job \
', $jobid, \"\\n\";\n    }\n\n    # Check status, \
and wait if not finished\n    client_poll($jobid);\
\n\n    # Default output file names use JobId, how\
ever the name can be specified...\n    if (defined\
($params{'outfile'})) {\n        $output_basename \
= $params{'outfile'};\n    }\n    # Or use sequenc\
e identifer.\n    elsif (defined($params{'useSeqId\
'} && defined($seq_id) && $seq_id ne '')) {\n     \
   $output_basename = $seq_id;\n\n        # Make s\
afe to use as a file name.\n        $output_basena\
me =~ s/\\W/_/g;\n    }\n\n    # Use JobId if outp\
ut file name is not defined\n    else {\n        u\
nless (defined($params{'outfile'})) {\n           \
 #$params{'outfile'} = $jobid;\n            $outpu\
t_basename = $jobid;\n        }\n    }\n\n    # Ge\
t list of data types\n    my (@resultTypes) = rest\
_get_result_types($jobid);\n\n\n    # Get the data\
 and write it to a file\n    if (defined($params{'\
outformat'})) {\n        # Specified data type\n  \
      # check to see if there are multiple formats\
 (comma separated)\n        my $sep = \",\";\n    \
    my (@multResultTypes);\n        if ($params{'o\
utformat'} =~ /$sep/) {\n            @multResultTy\
pes = split(',', $params{'outformat'});\n        }\
\n        else {\n            $multResultTypes[0] \
= $params{'outformat'};\n        }\n        # chec\
k if the provided formats are recognised\n        \
foreach my $inputType (@multResultTypes) {\n      \
      my $expectation = 0;\n            foreach my\
 $resultType (@resultTypes) {\n                if \
($resultType->{'identifier'} eq $inputType && $exp\
ectation eq 0) {\n                    $expectation\
 = 1;\n                }\n            }\n         \
   if ($expectation ne 1) {\n                die '\
Error: unknown result format \"' . $inputType . '\\
"';\n            }\n        }\n        # if so get\
 the files\n        my $selResultType;\n        fo\
reach my $resultType (@resultTypes) {\n           \
 if (grep {$_ eq $resultType->{'identifier'}} @mul\
tResultTypes) {\n                $selResultType = \
$resultType;\n                my $result = rest_ge\
t_result($jobid, $selResultType->{'identifier'});\\
n                if (defined($params{'outfile'}) &\
& $params{'outfile'} eq '-') {\n                  \
...
esult($jobid, $resultType->{'identifier'});\n     \
       if (defined($params{'outfile'}) && $params{\
'outfile'} eq '-') {\n                write_file($\
...
{\n        open(my $FILE, '>', $filename)\n       \
     or die \"Error: unable to open output file $f\
ilename ($!)\";\n        syswrite($FILE, $data);\n\
        close($FILE);\n    }\n    print_debug_mess\
age('write_file', 'End', 1);\n}\n\n=head2 usage()\\
n\nPrint program usage message.\n\n  &usage();\n\n\
=cut\n\nsub usage {\n    print STDERR <<EOF\nEMBL-\
EBI NCBI Blast Perl Client:\n\nSequence similarity\
 search with NCBI Blast.\n\n[Required (for job sub\
mission)]\n  --email               E-mail address.\
\n  --program             The BLAST program to be \
used for the Sequence Similarity\n                \
        Search.\n  --stype               Indicates\
 if the sequence is protein or DNA/RNA.\n  --seque\
nce            The query sequence can be entered d\
irectly into this form.\n                        T\
he sequence can be in GCG, FASTA, EMBL (Nucleotide\
 only),\n                        GenBank, PIR, NBR\
F, PHYLIP or UniProtKB/Swiss-Prot (Protein\n      \
                  only) format. A partially format\
ted sequence is not\n                        accep\
ted. Adding a return to the end of the sequence ma\
y\n                        help certain applicatio\
ns understand the input. Note that\n              \
          directly using data from word processors\
 may yield\n                        unpredictable \
results as hidden/control characters may be\n     \
                   present.\n  --database         \
   Database.\n\n[Optional]\n  --task              \
  Task option (only selectable for blastn).\n  --m\
atrix              (Protein searches) The substitu\
tion matrix used for scoring\n                    \
    alignments when searching the database.\n  --a\
lignments          Maximum number of match alignme\
nts reported in the result\n                      \
  output.\n  --scores              Maximum number \
of match score summaries reported in the\n        \
                result output.\n  --exp           \
      Limits the number of scores and alignments r\
eported based on\n                        the expe\
ctation value. This is the maximum number of times\
\n                        the match is expected to\
 occur by chance.\n  --dropoff             The amo\
unt a score can drop before gapped extension of wo\
rd\n                        hits is halted.\n  --m\
atch_scores        (Nucleotide searches) The match\
 score is the bonus to the\n                      \
  alignment score when matching the same base. The\
 mismatch is\n                        the penalty \
when failing to match.\n  --gapopen             Pe\
nalty taken away from the score when a gap is crea\
ted in\n                        sequence. Increasi\
ng the gap openning penalty will decrease\n       \
                 the number of gaps in the final a\
lignment.\n  --gapext              Penalty taken a\
way from the score for each base or residue\n     \
                   in the gap. Increasing the gap \
extension penalty favors\n                        \
short gaps in the final alignment, conversly decre\
asing the\n                        gap extension p\
enalty favors long gaps in the final\n            \
            alignment.\n  --filter              Fi\
lter regions of low sequence complexity. This can \
avoid\n                        issues with low com\
plexity sequences where matches are found\n       \
                 due to composition rather than me\
aningful sequence\n                        similar\
ity. However in some cases filtering also masks\n \
                       regions of interest and so \
should be used with caution.\n  --seqrange        \
    Specify a range or section of the input sequen\
ce to use in\n                        the search. \
Example: Specifying '34-89' in an input sequence\n\
                        of total length 100, will \
tell BLAST to only use residues 34\n              \
          to 89, inclusive.\n  --gapalign         \
   This is a true/false setting that tells the pro\
gram the\n                        perform optimise\
d alignments within regions involving gaps.\n     \
                   If set to true, the program wil\
l perform an alignment using\n                    \
    gaps. Otherwise, if it is set to false, it wil\
l report only\n                        individual \
HSP where two sequence match each other, and thus\\
n                        will not produce alignmen\
ts with gaps.\n  --wordsize            Word size f\
or wordfinder algorithm.\n  --compstats           \
Use composition-based statistics.\n  --align      \
         Formating for the alignments.\n  --transl\
table         Query Genetic code to use in transla\
tion.\n\n[General]\n  -h, --help            Show t\
...
available result types for job.\n  --polljob      \
       Poll for the status of a job.\n  --pollFreq\
            Poll frequency in seconds (default 3s)\
.\n  --jobid               JobId that was returned\
 when an asynchronous job was submitted.\n  --outf\
ile             File name for results (default is \
JobId; for STDOUT).\n  --multifasta          Treat\
 input as a set of fasta formatted sequences.\n  -\
-useSeqId            Use sequence identifiers for \
output filenames.\n                        Only av\
ailable in multi-fasta and multi-identifier modes.\
\n  --maxJobs             Maximum number of concur\
rent jobs. Only\n                        available\
 in multifasta or list file modes.\n  --outformat \
          Result format(s) to retrieve. It accepts\
 comma-separated values.\n  --params              \
List input parameters.\n  --paramDetail         Di\
splay details for input parameter.\n  --quiet     \
          Decrease output.\n  --verbose           \
  Increase output.\n  --version             Prints\
 out the version of the Client and exit.\n  --base\
Url             Base URL. Defaults to:\n          \
              https://www.ebi.ac.uk/Tools/services\
/rest/ncbiblast\n\nSynchronous job:\n  The results\
/errors are returned as soon as the job is finishe\
d.\n  Usage: perl $scriptName --email <your\\@emai\
l.com> [options...] <SeqFile|SeqID(s)>\n  Returns:\
 results as an attachment\n\nAsynchronous job:\n  \
Use this if you want to retrieve the results at a \
later time. The results\n  are stored for up to 24\
 hours.\n  Usage: perl $scriptName --asyncjob --em\
ail <your\\@email.com> [options...] <SeqFile|SeqID\
(s)>\n  Returns: jobid\n\nCheck status of Asynchro\
nous job:\n  Usage: perl $scriptName --status --jo\
bid <jobId>\n\nRetrieve job data:\n  Use the jobid\
 to query for the status of the job. If the job is\
 finished,\n  it also returns the results/errors.\\
n  Usage: perl $scriptName --polljob --jobid <jobI\
d> [--outfile string]\n  Returns: string indicatin\
g the status of the job and if applicable, results\
\n  as an attachment.\n\nFurther information:\n  h\
ttps://www.ebi.ac.uk/Tools/webservices and\n    ht\
tps://github.com/ebi-wp/webservice-clients\n\nSupp\
ort/Feedback:\n  https://www.ebi.ac.uk/support/\nE\
OF\n}\n\n=head1 FEEDBACK/SUPPORT\n\nPlease contact\
 us at L<https://www.ebi.ac.uk/support/> if you ha\
ve any\nfeedback, suggestions or issues with the s\
ervice or this client.\n\n=cut\n","\n=head1 NAME\n\
\nwublast_lwp.pl\n\n=head1 DESCRIPTION\n\nWU-BLAST\
 (REST) web service Perl client using L<LWP>.\n\nT\
ested with:\n\n=over\n\n=item *\nL<LWP> 5.79, L<XM\
L::Simple> 2.12 and Perl 5.8.3\n\n=item *\nL<LWP> \
5.808, L<XML::Simple> 2.18 and Perl 5.8.8 (Ubuntu \
8.04 LTS)\n\n=item *\nL<LWP> 5.834, L<XML::Simple>\
 2.18 and Perl 5.10.1 (Ubuntu 10.04 LTS)\n\n=item \
*\nL<LWP> 6.03, L<XML::Simple> 2.18 and Perl 5.14.\
2 (Ubuntu 12.04 LTS)\n\n=back\n\nFor further infor\
mation see:\n\n=over\n\n=item *\nL<http://www.ebi.\
ac.uk/Tools/webservices/services/sss/wu_blast_rest\
>\n\n=item *\nL<http://www.ebi.ac.uk/Tools/webserv\
ices/tutorials/perl>\n\n=back\n\n=head1 LICENSE\n\\
nCopyright 2012-2013 EMBL - European Bioinformatic\
...
rvices/rest/wublast';\n\nmy $checkInterval = 3;\n\\
nmy $outputLevel = 1;\n\nmy $numOpts = scalar(@ARG\
V);\nmy %params = ( 'debugLevel' => 0 );\n\nmy %to\
ol_params = ();\nGetOptions(\n\n	# Tool specific o\
ptions\n	'program|p=s'     => \\$tool_params{'prog\
ram'},      # BLAST program\n	'database|D=s'    =>\
 \\$params{'database'},     # Search database\n	'm\
atrix|m=s'      => \\$tool_params{'matrix'},      \
 # Scoring matrix\n	'exp|E=f'         => \\$tool_p\
arams{'exp'},          # E-value threshold\n	'view\
filter|e'    => \\$tool_params{'viewfilter'},   # \
Display filtered sequence\n	'filter|f=s'      => \\
\$tool_params{'filter'},       # Low complexity fi\
lter name\n	'alignments|n=i'  => \\$tool_params{'a\
lignments'},   # Number of alignments\n	'scores|s=\
i'      => \\$tool_params{'scores'},       # Numbe\
r of scores\n	'sensitivity|S=s' => \\$tool_params{\
'sensitivity'},  # Search sensitivity\n	'sort|t=s'\
        => \\$tool_params{'sort'},         # Sort \
hits by...\n	'stats|T=s'       => \\$tool_params{'\
stats'},        # Scoring statistic to use\n	'stra\
nd|d=s'      => \\$tool_params{'strand'},       # \
Strand to use\n	'topcombon|c=i'   => \\$tool_param\
s{'topcombon'},    # Consistent sets of HSPs\n	'al\
ign|A=i'       => \\$tool_params{'align'},   # Pai\
rwise alignment format\n	'stype=s' => \\$tool_para\
ms{'stype'},    # Sequence type 'protein' or 'dna'\
\n	'sequence=s' => \\$params{'sequence'},         \
# Query sequence file or DB:ID\n	'multifasta' => \\
\$params{'multifasta'},       # Multiple fasta inp\
ut\n\n	# Compatability options, old command-line.\\
n	'echofilter|e'    => \\$params{'echofilter'},   \
# Display filtered sequence\n	'b=i'  => \\$params{\
'numal'},        # Number of alignments\n	'appxml=\
s'        => \\$params{'appxml'},       # Applicat\
ion XML\n\n	# Generic options\n	'email=s'       =>\
 \\$params{'email'},          # User e-mail addres\
...
d( $params{'sequence'} ) )\n  )\n{\n\n	# Bad argum\
ent combination, so print error message and usage\\
n	print STDERR 'Error: bad option combination', \"\
\\n\";\n	&usage();\n	exit(1);\n}\n\nelsif ( $param\
s{'params'} ) {\n	&print_tool_params();\n}\n\nelsi\
f ( $params{'paramDetail'} ) {\n	&print_param_deta\
ils( $params{'paramDetail'} );\n}\n\nelsif ( $para\
ms{'status'} && defined( $params{'jobid'} ) ) {\n	\
&print_job_status( $params{'jobid'} );\n}\n\nelsif\
 ( $params{'resultTypes'} && defined( $params{'job\
...
'} || $ARGV[0];\n		$list_filename =~ s/^\\@//;\n		\
&list_file_submit_job($list_filename);\n	}\n\n	# D\
efault: single sequence/identifier.\n	else {\n\n		\
# Load the sequence data and submit.\n		&submit_jo\
b( &load_data() );\n	}\n}\n\n=head1 FUNCTIONS\n\n=\
cut\n\n\n=head2 rest_user_agent()\n\nGet a LWP Use\
rAgent to use to perform REST requests.\n\n  my $u\
a = &rest_user_agent();\n\n=cut\n\nsub rest_user_a\
gent() {\n	print_debug_message( 'rest_user_agent',\
 'Begin', 21 );\n	# Create an LWP UserAgent for ma\
king HTTP calls.\n	my $ua = LWP::UserAgent->new();\
\n	# Set 'User-Agent' HTTP header to identifiy the\
 client.\n	'$Revision: 2560 $' =~ m/(\\d+)/;\n	$ua\
->agent(\"EBI-Sample-Client/$1 ($scriptName; $OSNA\
ME) \" . $ua->agent());\n	# Configure HTTP proxy s\
upport from environment.\n	$ua->env_proxy;\n	print\
_debug_message( 'rest_user_agent', 'End', 21 );\n	\
return $ua;\n}\n\n=head2 rest_error()\n\nCheck a R\
EST response for an error condition. An error is m\
apped to a die.\n\n  &rest_error($response, $conte\
nt_data);\n\n=cut\n\nsub rest_error() {\n	print_de\
bug_message( 'rest_error', 'Begin', 21 );\n	my $re\
sponse = shift;\n	my $contentdata;\n	if(scalar(@_)\
 > 0) {\n		$contentdata = shift;\n	}\n	if(!defined\
($contentdata) || $contentdata eq '') {\n		$conten\
...
ge::decodable();\n	};\n	$can_accept = '' unless de\
fined($can_accept);\n	# Perform the request\n	my $\
response = $ua->get($requestUrl,\n		'Accept-Encodi\
ng' => $can_accept, # HTTP compression.\n	);\n	pri\
nt_debug_message( 'rest_request', 'HTTP status: ' \
. $response->code,\n		11 );\n	print_debug_message(\
 'rest_request',\n		'response length: ' . length($\
response->content()), 11 );\n	print_debug_message(\
 'rest_request',\n		'request:' .\"\\n\" . $respons\
e->request()->as_string(), 32 );\n	print_debug_mes\
sage( 'rest_request',\n		'response: ' . \"\\n\" . \
$response->as_string(), 32 );\n	# Unpack possibly \
compressed response.\n	my $retVal;\n	if ( defined(\
$can_accept) && $can_accept ne '') {\n	    $retVal\
 = $response->decoded_content();\n	}\n	# If unable\
 to decode use orginal content.\n	$retVal = $respo\
nse->content() unless defined($retVal);\n	# Check \
for an error.\n	&rest_error($response, $retVal);\n\
	print_debug_message( 'rest_request', 'retVal: ' .\
...
aram_list) = &rest_get_parameters();\n\n=cut\n\nsu\
b rest_get_parameters {\n	print_debug_message( 're\
st_get_parameters', 'Begin', 1 );\n	my $url       \
         = $baseUrl . '/parameters/';\n	my $param_\
list_xml_str = rest_request($url);\n	my $param_lis\
t_xml     = XMLin($param_list_xml_str);\n	my (@par\
am_list)       = @{ $param_list_xml->{'id'} };\n	p\
rint_debug_message( 'rest_get_parameters', 'End', \
1 );\n	return (@param_list);\n}\n\n=head2 rest_get\
_parameter_details()\n\nGet details of a tool para\
meter.\n\n  my $paramDetail = &rest_get_parameter_\
details($param_name);\n\n=cut\n\nsub rest_get_para\
meter_details {\n	print_debug_message( 'rest_get_p\
arameter_details', 'Begin', 1 );\n	my $parameterId\
 = shift;\n	print_debug_message( 'rest_get_paramet\
er_details',\n		'parameterId: ' . $parameterId, 1 \
);\n	my $url                  = $baseUrl . '/param\
eterdetails/' . $parameterId;\n	my $param_detail_x\
ml_str = rest_request($url);\n	my $param_detail_xm\
l     = XMLin($param_detail_xml_str);\n	print_debu\
g_message( 'rest_get_parameter_details', 'End', 1 \
);\n	return ($param_detail_xml);\n}\n\n=head2 rest\
_run()\n\nSubmit a job.\n\n  my $job_id = &rest_ru\
n($email, $title, \\%params );\n\n=cut\n\nsub rest\
_run {\n	print_debug_message( 'rest_run', 'Begin',\
 1 );\n	my $email  = shift;\n	my $title  = shift;\\
n	my $params = shift;\n	print_debug_message( 'rest\
_run', 'email: ' . $email, 1 );\n	if ( defined($ti\
tle) ) {\n		print_debug_message( 'rest_run', 'titl\
e: ' . $title, 1 );\n	}\n	print_debug_message( 're\
st_run', 'params: ' . Dumper($params), 1 );\n\n	# \
...
efined($paramDetail->{'values'}->{'value'})) {\n		\
if(ref($paramDetail->{'values'}->{'value'}) eq 'AR\
RAY') {\n			foreach my $value ( @{ $paramDetail->{\
'values'}->{'value'} } ) {\n				&print_param_value\
($value);\n			}\n		}\n		else {\n				&print_param_v\
alue($paramDetail->{'values'}->{'value'});\n		}\n	\
}\n	print_debug_message( 'print_param_details', 'E\
nd', 1 );\n}\n\n=head2 print_param_value()\n\nPrin\
t details of a tool parameter value.\n\n  &print_p\
aram_details($param_value);\n\nUsed by print_param\
_details() to handle both singluar and array value\
s.\n\n=cut\n\nsub print_param_value {\n	my $value \
= shift;\n	print $value->{'value'};\n	if ( $value-\
...
value->{'label'}, \"\\n\";\n	if ( defined( $value-\
>{'properties'} ) ) {\n		foreach\n		  my $key ( so\
rt( keys( %{ $value->{'properties'}{'property'} } \
) ) )\n		{\n			if ( ref( $value->{'properties'}{'p\
roperty'}{$key} ) eq 'HASH'\n				&& defined( $valu\
e->{'properties'}{'property'}{$key}{'value'} )\n		\
	  )\n			{\n				print \"\\t\", $key, \"\\t\",\n			\
...
ARGV[0] eq '-' ) {    # File\n			push( @filename_l\
ist, $ARGV[0] );\n		}\n		else {\n			warn 'Warning:\
 Input file \"' . $ARGV[0] . '\" does not exist'\n\
		}\n	}\n	if ( $params{'sequence'} ) {            \
       # Via --sequence\n		if ( -f $params{'sequen\
...
n\n	$/ = '>';\n	foreach my $filename (@filename_li\
st) {\n		my $INFILE;\n		if($filename eq '-') { # S\
TDIN.\n			open( $INFILE, '<-' )\n			  or die 'Erro\
r: unable to STDIN (' . $! . ')';\n		} else { # Fi\
le.\n			open( $INFILE, '<', $filename )\n			  or d\
ie 'Error: unable to open file ' . $filename . ' (\
' . $! . ')';\n		}\n		while (<$INFILE>) {\n			my $\
seq = $_;\n			$seq =~ s/>$//;\n			if ( $seq =~ m/(\
...
ubmit_job', $seq, 11 );\n				&submit_job($seq);\n	\
			$params{'outfile'} = undef if ( $jobIdForFilena\
me == 1 );\n			}\n		}\n		close $INFILE;\n	}\n	prin\
t_debug_message( 'multi_submit_job', 'End', 1 );\n\
}\n\n=head2 list_file_submit_job()\n\nSubmit multi\
ple jobs using a file containing a list of entry i\
dentifiers as \ninput.\n\n  &list_file_submit_job(\
$list_filename)\n\n=cut\n\nsub list_file_submit_jo\
b {\n	print_debug_message( 'list_file_submit_job',\
 'Begin', 11 );\n	my $filename         = shift;\n	\
my $jobIdForFilename = 1;\n	$jobIdForFilename = 0 \
if ( defined( $params{'outfile'} ) );\n\n	# Iterat\
e over identifiers, submitting each job\n	my $LIST\
FILE;\n	if($filename eq '-') { # STDIN.\n		open( $\
...
\n	while (<$LISTFILE>) {\n		my $line = $_;\n		chom\
p($line);\n		if ( $line ne '' ) {\n			&print_debug\
_message( 'list_file_submit_job', 'line: ' . $line\
, 2 );\n			if ( $line =~ m/\\w:\\w/ ) {    # Check\
 this is an identifier\n				print STDERR \"Submitt\
ing job for: $line\\n\"\n				  if ( $outputLevel >\
 0 );\n				&submit_job($line);\n			}\n			else {\n	\
			print STDERR\n\"Warning: line \\\"$line\\\" is \
not recognised as an identifier\\n\";\n			}\n		}\n\
		$params{'outfile'} = undef if ( $jobIdForFilenam\
e == 1 );\n	}\n	close $LISTFILE;\n	print_debug_mes\
sage( 'list_file_submit_job', 'End', 11 );\n}\n\n=\
head2 load_data()\n\nLoad sequence data from file \
or option specified on the command-line.\n\n  &loa\
d_data();\n\n=cut\n\nsub load_data {\n	print_debug\
_message( 'load_data', 'Begin', 1 );\n	my $retSeq;\
\n\n	# Query sequence\n	if ( defined( $ARGV[0] ) )\
 {    # Bare option\n		if ( -f $ARGV[0] || $ARGV[0\
] eq '-' ) {    # File\n			$retSeq = &read_file( $\
ARGV[0] );\n		}\n		else {                         \
...
tSeq = $params{'sequence'};\n		}\n	}\n	print_debug\
_message( 'load_data', 'End', 1 );\n	return $retSe\
q;\n}\n\n=head2 load_params()\n\nLoad job paramete\
rs from command-line options.\n\n  &load_params();\
\n\n=cut\n\nsub load_params {\n	print_debug_messag\
e( 'load_params', 'Begin', 1 );\n\n	# Database(s) \
to search\n	my (@dbList) = split /[ ,]/, $params{'\
database'};\n	$tool_params{'database'} = \\@dbList\
;\n\n	# Compatability options, old command-line.\n\
	if(!$tool_params{'viewfilter'} && $params{'echofi\
lter'}) {\n		$tool_params{'viewfilter'} = 'true';\\
n	}\n	if(!$tool_params{'alignments'} && $params{'n\
umal'}) {\n		$tool_params{'alignments'} = $params{\
'numal'};\n	}\n	# TODO: set alignment format optio\
n to get NCBI BLAST XML.\n	if($params{'appxml'}) {\
\n		$tool_params{'align'} = '';\n	}\n\n	print_debu\
g_message( 'load_params', 'End', 1 );\n}\n\n=head2\
 client_poll()\n\nClient-side job polling.\n\n  &c\
lient_poll($job_id);\n\n=cut\n\nsub client_poll {\\
n	print_debug_message( 'client_poll', 'Begin', 1 )\
;\n	my $jobid  = shift;\n	my $status = 'PENDING';\\
n\n	my $errorCount = 0;\n	while ($status eq 'RUNNI\
NG'\n		|| $status eq 'PENDING'\n		|| ( $status eq \
'ERROR' && $errorCount < 2 ) )\n	{\n		$status = re\
st_get_status($jobid);\n		print STDERR \"$status\\\
n\" if ( $outputLevel > 0 );\n		if ( $status eq 'E\
...
\n	return $status;\n}\n\n=head2 get_results()\n\nG\
et the results for a job identifier.\n\n  &get_res\
ults($job_id);\n\n=cut\n\nsub get_results {\n	prin\
t_debug_message( 'get_results', 'Begin', 1 );\n	my\
 $jobid = shift;\n	print_debug_message( 'get_resul\
ts', 'jobid: ' . $jobid, 1 );\n\n	# Verbose\n	if (\
 $outputLevel > 1 ) {\n		print 'Getting results fo\
r job ', $jobid, \"\\n\";\n	}\n\n	# Check status, \
and wait if not finished\n	client_poll($jobid);\n\\
n	# Use JobId if output file name is not defined\n\
...
ultType->{'identifier'} eq $params{'outformat'} ) \
{\n				$selResultType = $resultType;\n			}\n		}\n	\
...
{'outfile'} . '.'\n					  . $selResultType->{'iden\
tifier'} . '.'\n					  . $selResultType->{'fileSuf\
fix'},\n					$result\n				);\n			}\n		}\n		else {\\
n			die 'Error: unknown result format \"' . $param\
s{'outformat'} . '\"';\n		}\n	}\n	else {    # Data\
 types available\n		      # Write a file for each \
output type\n		for my $resultType (@resultTypes) {\
\n			if ( $outputLevel > 1 ) {\n				print STDERR '\
Getting ', $resultType->{'identifier'}, \"\\n\";\n\
			}\n			my $result = rest_get_result( $jobid, $re\
sultType->{'identifier'} );\n			if ( $params{'outf\
...
e->{'identifier'} . '.'\n					  . $resultType->{'f\
ileSuffix'},\n					$result\n				);\n			}\n		}\n	}\\
n	print_debug_message( 'get_results', 'End', 1 );\\
n}\n\n=head2 read_file()\n\nRead a file into a sca\
lar. The special filename '-' can be used to read \
from \nstandard input (STDIN).\n\n  my $data = &re\
ad_file($filename);\n\n=cut\n\nsub read_file {\n	p\
rint_debug_message( 'read_file', 'Begin', 1 );\n	m\
...
$content;\n}\n\n=head2 write_file()\n\nWrite data \
to a file. The special filename '-' can be used to\
 write to \nstandard output (STDOUT).\n\n  &write_\
file($filename, $data);\n\n=cut\n\nsub write_file \
{\n	print_debug_message( 'write_file', 'Begin', 1 \
);\n	my ( $filename, $data ) = @_;\n	print_debug_m\
essage( 'write_file', 'filename: ' . $filename, 2 \
);\n	if ( $outputLevel > 0 ) {\n		print STDERR 'Cr\
eating result file: ' . $filename . \"\\n\";\n	}\n\
	if ( $filename eq '-' ) {\n		print STDOUT $data;\\
n	}\n	else {\n		open( my $FILE, '>', $filename )\n\
		  or die \"Error: unable to open output file $fi\
lename ($!)\";\n		syswrite( $FILE, $data );\n		clo\
se($FILE);\n	}\n	print_debug_message( 'write_file'\
, 'End', 1 );\n}\n\n=head2 usage()\n\nPrint progra\
m usage message.\n\n  &usage();\n\n=cut\n\nsub usa\
ge {\n	print STDERR <<EOF\nWU-BLAST\n========\n   \
\nRapid sequence database search programs utilizin\
g the BLAST algorithm\n    \n[Required]\n\n  -p, -\
-program      : str  : BLAST program to use, see -\
-paramDetail program\n  -D, --database     : str  \
: database(s) to search, space separated. See\n   \
                           --paramDetail database\\
n      --stype        : str  : query sequence type\
, see --paramDetail stype\n  seqFile            : \
file : query sequence (\"-\" for STDIN, \\@filenam\
e for\n                              identifier li\
st file)\n\n[Optional]\n\n  -m, --matrix       : s\
tr  : scoring matrix, see --paramDetail matrix\n  \
-e, --exp          : real : 0<E<= 1000. Statistica\
l significance threshold \n                       \
       for reporting database sequence matches.\n \
 -e, --viewfilter   :      : display the filtered \
query sequence\n  -f, --filter       : str  : filt\
er the query sequence for low complexity \n       \
                       regions, see --paramDetail \
filter\n  -A, --align        : int  : pairwise ali\
gnment format, see --paramDetail align\n  -s, --sc\
ores       : int  : number of scores to be reporte\
d\n  -b, --alignments   : int  : number of alignme\
nts to report\n  -S, --sensitivity  : str  : sensi\
tivity of the search, \n                          \
    see --paramDetail sensitivity\n  -t, --sort	  \
   : str  : sort order for hits, see --paramDetail\
 sort\n  -T, --stats        : str  : statistical m\
odel, see --paramDetail stats\n  -d, --strand     \
...
nous job \n                              was submi\
tted.\n      --outfile      : str  : file name for\
 results (default is jobid;\n                     \
         \"-\" for STDOUT)\n      --outformat    :\
 str  : result format to retrieve\n      --params \
      :      : list input parameters\n      --para\
mDetail  : str  : display details for input parame\
...
for the status of the job. If the job is finished,\
 \n  it also returns the results/errors.\n  Usage:\
 $scriptName --polljob --jobid <jobId> [--outfile \
string]\n  Returns: string indicating the status o\
f the job and if applicable, results \n  as an att\
achment.\n\nFurther information:\n\n  http://www.e\
bi.ac.uk/Tools/webservices/services/sss/wu_blast_r\
est\n  http://www.ebi.ac.uk/Tools/webservices/tuto\
rials/perl\n\nSupport/Feedback:\n\n  http://www.eb\
i.ac.uk/support/\nEOF\n}\n\n=head1 FEEDBACK/SUPPOR\
T\n\nPlease contact us at L<http://www.ebi.ac.uk/s\
upport/> if you have any \nfeedback, suggestions o\
r issues with the service or this client.\n\n=cut\\
n","\n\n\nmy $PROBTRESH = 0.3;# base pairs below t\
his prob threshold will be ignored\nmy $WEIGHT = 1\
...
$;$)\n{\n    my ($nseq, $fd) = @_;\n    if (! defi\
ned($fd)) {\n        $fd = *STDOUT;\n    }\n    pr\
intf $fd \"! TC_LIB_FORMAT_01\\n\";\n    printf $f\
d \"%d\\n\", $nseq;\n}\n\n\nsub tcoffeelib_header_\
addseq($$;$)\n{\n    my ($id, $seq, $fd) = @_;\n  \
  if (! defined($fd)) {\n        $fd = *STDOUT;\n \
   }\n    printf $fd \"%s %d %s\\n\", $id, length(\
...
, $fd) = @_;\n\n    if (! defined($fd)) {\n       \
 $fd = *STDOUT;\n    }\n\n    # output basepair in\
dices with fixed weight\n    printf $fd \"#%d %d\\\
n\", $nseq, $nseq;\n    # output basepairs (only o\
nce) and with unit-offset\n    for (my $i=0; $i<$l\
...
my (@struct);# return\n    my ($templ, $fhtmp, $fn\
ametmp, $cmd, $ctr, $window_size);\n    our $ntemp\
++;\n    \n    $templ = $myname . \".pid-\" . $$ .\
$ntemp .\".XXXXXX\";\n    ($fhtmp, $fnametmp) = te\
mpfile($templ, UNLINK => 1); \n    print $fhtmp \"\
>$id\\n$seq\\n\";\n\n    # --- init basepair array\
...
($seq):70;\n    $cmd = \"RNAplfold -W $window_size\
 < $fnametmp >/dev/null\";\n    system($cmd);\n   \
 \n    if ($? != 0) {\n        printf STDERR \"ERR\
OR: RNAplfold ($cmd) exited with error status %d\\\
n\", $? >> 8;\n        return;\n    }\n    #unlink\
...
\\n\");\n	    return;\n	  }\n      }\n    }\n\n   \
 \n    # --- read base pairs from created postscri\
pt\n    #\n    open(FH, $fps);\n    while (my $lin\
e = <FH>) {\n        my ($nti, $ntj, $prob);\n    \
    chomp($line);        \n        # line: bp bp s\
qrt-prob ubox\n        my @match = ($line =~ m/^([\
...
red as square root\n\n            if ($prob>$probt\
resh) {\n                #printf STDERR \"\\$struc\
t[$nti][$ntj] sqrtprob=$3 prob=$prob > $probtresh\\
\n\";\n                $struct[$nti-1][$ntj-1] = $\
WEIGHT\n            }\n            # store with ze\
ro-offset\n        }\n    }\n    close(FH);\n\n   \
...
s);\n    #\n    # or gzip\n    #$cmd = \"gzip -qf \
$fps\";\n    #system($cmd);\n    #if ($? != 0) {\n\
    #    printf STDERR \"ERROR: gzip ($cmd) exited\
 with error status %d\\n\", $? >> 8;\n    #}\n\n  \
  return \\@struct;\n}\n\n\n\n\n\nsub rnaseqfmt($)\
\n{\n    my ($seq) = @_;\n    # remove gaps\n    $\
seq =~ s/-//g;\n    # uppercase RNA\n    $seq = uc\
...
\n\";\n    }\n    print STDERR << \"EOF\";\n$mynam\
e:\n Creates a T-Coffee RNA structure library from\
 RNAplfold prediction.\n See FIXME:citation\nUsage\
...
$fmsq\\\"\")\n}\nif (length($flib)==0) {\n    usag\
e(\"empty out-filename\")\n}\n\n\n\n\n\n\n%seq=rea\
d_fasta_seq($fmsq);\n\n\n@nl=keys(%seq);\n\n$nseq=\
$#nl+1;\nopen FD_LIB, \">$flib\" or die \"can't op\
en $flib!\";\ntcoffeelib_header($nseq, *FD_LIB);\n\
foreach $id (keys (%seq))\n  {\n    my ($seq, $fmt\
seq);\n    \n    $seq = $seq{$id}{seq};\n    \n   \
 $fmtseq = rnaseqfmt($seq);# check here, formattin\
g for folding important later\n    if (length($seq\
)!=length($fmtseq)) {\n        print STDERR \"ERRO\
R: invalid sequence $id is not an RNA sequence. re\
ad seq is: $seq\\n\";\n        exit\n      }\n   \\
n    tcoffeelib_header_addseq($id, uc($seq), *FD_L\
IB);\n  }\ntcoffeelib_comment(\"generated by $myna\
me on \" . localtime(), *FD_LIB);\n\n\n\n$i=0;\nfo\
reach $id (keys (%seq))\n  {\n    my ($cleanid, $s\
eq, $bpm);\n    $seq=$seq{$id}{seq};\n    $cleanid\
 = $id;\n    $cleanid =~ s,[/ ],_,g;# needed for r\
naplfold\n    $seq = rnaseqfmt($seq);\n    \n    $\
bpm = plfold($cleanid, rnaseqfmt($seq), $PROBTRESH\
);       \n    \n    tcoffeelib_struct($i+1, lengt\
h($seq), $bpm, *FD_LIB);\n    $i++;\n}\n\n\ntcoffe\
elib_footer(*FD_LIB);\nclose FD_LIB;\nexit (0);\n\\
n","\n\n\n\n\n$cmd=join ' ', @ARGV;\nif ($cmd=~/-i\
nfile=(\\S+)/){ $seqfile=$1;}\nif ($cmd=~/-outfile\
...
\");\n    seq2RNAplfold2tclib ($name, $s{$name}{se\
q}, $tclib);\n  }\nclose (F);\nexit (EXIT_SUCCESS)\
...
pen (RF, \">$tmp\");\n    print (RF \">$name\\n$se\
q\\n\");\n    close (RF);\n    \n    system \"t_co\
ffee -other_pg RNAplfold2tclib.pl -in=$tmp -out=$t\
clib\";\n    \n    unlink ($tmp);\n    return $tcl\
ib;\n  }\n    \n    \nsub read_fasta_seq \n  {\n  \
  my $f=@_[0];\n    my %hseq;\n    my (@seq, @com,\
 @name);\n    my ($a, $s,$nseq);\n\n    open (F, $\
...
";\nour $CL=$PROGRAM;\n\nour $CLEAN_EXIT_STARTED;\\
nour $debug_lock=$ENV{\"DEBUG_LOCK\"};\nour $LOCKD\
IR=$ENV{\"LOCKDIR_4_TCOFFEE\"};\nif (!$LOCKDIR){$L\
OCKDIR=getcwd();}\nour $ERRORDIR=$ENV{\"ERRORDIR_4\
_TCOFFEE\"};\nour $ERRORFILE=$ENV{\"ERRORFILE_4_TC\
OFFEE\"};\n&set_lock ($$);\nif (isshellpid(getppid\
())){lock4tc(getppid(), \"LLOCK\", \"LSET\", \"$$\\
\n\");}\n      \nour $print;\nmy ($fmsq1, $fmsq2, \
$output, $outfile, $arch, $psv, $hmmtop_home, $tri\
m, $cov, $sample, $mode, $gor_home, $gor_seq, $gor\
_obs);\n\nGetOptions(\"-in=s\" => \\$fmsq1,\"-outp\
ut=s\" =>\\$output ,\"-out=s\" => \\$outfile, \"-a\
rch=s\" => \\$arch,\"-psv=s\" => \\$psv, \"-hmmtop\
_home=s\", \\$hmmtop_home,\"-trim=s\" =>\\$trim ,\\
"-print=s\" =>\\$print,\"-cov=s\" =>\\$cov , \"-sa\
mple=s\" =>\\$sample, \"-mode=s\" =>\\$mode, \"-go\
r_home=s\"=>\\$gor_home, \"-gor_seq=s\"=>\\$gor_se\
q,\"-gor_obs=s\"=>\\$gor_obs);\n\n\nif (!$mode){$m\
ode = \"hmmtop\"}\nelsif ($mode eq \"hmmtop\"){;}\\
nelsif ($mode eq \"gor\"){;}\nelse {myexit(flush_e\
rror (\"-mode=$mode is unknown\"));}\n\n\nour $HOM\
E=$ENV{\"HOME\"};\nour $MCOFFEE=($ENV{\"MCOFFEE_4_\
TCOFFEE\"})?$ENV{\"MCOFFEE_4_TCOFFEE\"}:\"$HOME/.t\
_coffee/mcoffee\";\n\nif ($mode eq \"hmmtop\")\n  \
{\n    \n    check_configuration (\"hmmtop\");\n  \
  if (-e $arch){$ENV{'HMMTOP_ARCH'}=$arch;}\n    e\
lsif (-e $ENV{HMMTOP_ARCH}){$arch=$ENV{HMMTOP_ARCH\
};}\n    elsif (-e \"$MCOFFEE/hmmtop.arch\"){$arch\
=$ENV{'HMMTOP_ARCH'}=\"$MCOFFEE/hmmtop.arch\";}\n \
   elsif (-e \"$hmmtop_home/hmmtop.arch\"){$arch=$\
ENV{'HMMTOP_ARCH'}=\"$hmmtop_home/hmmtop.arch\";}\\
n    else {myexit(flush_error ( \"Could not find A\
RCH file for hmmtop\"));}\n    \n    \n    if (-e \
$psv){$ENV{'HMMTOP_PSV'}=$psv;}\n    elsif (-e $EN\
V{HMMTOP_PSV}){$psv=$ENV{HMMTOP_PSV};}\n    elsif \
(-e \"$MCOFFEE/hmmtop.psv\"){$psv=$ENV{'HMMTOP_PSV\
'}=\"$MCOFFEE/hmmtop.psv\";}\n    elsif (-e \"$hmm\
top_home/hmmtop.psv\"){$psv=$ENV{'HMMTOP_PSV'}=\"$\
hmmtop_home/hmmtop.psv\";}\n    else {myexit(flush\
_error ( \"Could not find PSV file for hmmtop\"));\
}\n\n  }\nelsif ($mode eq \"gor\")\n  {\n    our $\
GOR_SEQ;\n    our $GOR_OBS;\n    \n    check_confi\
guration (\"gorIV\");\n    if (-e $gor_seq){$GOR_S\
...
\nelse\n  {\n   \n    %pred=fasta2pred(\\%seq, $mo\
de);\n    %predA=pred2aln (\\%pred, \\%seq);\n    \
\n    \n    if (!$output || $output eq \"predictio\
n\"){output_fasta_seq (\\%predA, $outfile);}\n    \
elsif ($output eq \"color_html\"){pred2color (\\%p\
red,\\%seq, $outfile);}\n    elsif ($output eq \"c\
ons\"){pred2cons($outfile,\\%predA);}\n    else {f\
lush_error (\"$output is an unknown output mode\")\
;}\n  }\n\nsub fasta2sample\n  {\n    my $SR=shift\
;\n    my $it=shift;\n    my %S=%$SR;\n    \n    m\
y $seq=index2seq_name (\\%S, 1);\n    my $l=length\
($S{$seq}{seq});\n    my @sl=keys(%S);\n    my $ns\
eq=$#sl+1;\n    my $index=$nseq;\n  \n    if (!$sa\
mple) {return %S;}\n    for (my $a=0; $a<$it; $a++\
)\n      {\n	my $newseq=\"\";\n	my $nname=\"$seq\\\
...
\n	      \nsub fasta2pred\n  {\n    my $s=shift;\n\
    my $mode=shift;\n\n    if ( $mode eq \"hmmtop\\
"){return fasta2hmmtop_pred($s);}\n    elsif ($mod\
e eq \"gor\"){return fasta2gor_pred ($s);}\n  }\ns\
ub fasta2hmmtop_cons\n  {\n    my $outfile=shift;\\
n    my $SR=shift;\n    \n    my $o = new FileHand\
le;\n    my $i = new FileHandle;\n    my $tmp_in =\
vtmpnam();\n    my $tmp_out=vtmpnam();\n    my %se\
q=%$SR;\n    my %pred;\n    my $N=keys(%seq);\n   \
 \n    output_fasta_seq (\\%seq,$tmp_in, \"seq\");\
\n    `hmmtop -pi=mpred -if=$tmp_in -sf=FAS -pl 2>\
/dev/null >$tmp_out`;\n    open ($o, \">$outfile\"\
);\n    open ($i, \"$tmp_out\");\n    while (<$i>)\
\n      {\n	my $l=$_;\n	if (($l=~/>HP\\:\\s+(\\d+)\
\\s+(.*)/)){my $line=\">$2 NSEQ: $N\\n\";print $o \
...
m();\n    my %seq=%$SR;\n    my %pred;\n    \n\n  \
  output_fasta_seq (\\%seq,$tmp_in, \"seq\");\n\n \
   \n    `hmmtop -if=$tmp_in -sf=FAS -pl 2>/dev/nu\
ll >$tmp_out`;\n    \n\n    \n    \n    open ($o, \
\">$tmp\");\n    open ($i, \"$tmp_out\");\n    whi\
le (<$i>)\n      {\n	my $l=$_;\n	if (($l=~/>HP\\:\\
\s+(\\d+)\\s+(.*)/)){my $line=\">$2\\n\";print $o \
...
\n    my %pred;\n    \n\n    output_fasta_seq (\\%\
seq,$tmp_in, \"seq\");\n    `gorIV -prd $tmp_in -s\
eq $GOR_SEQ -obs $GOR_OBS >$tmp_out`;\n    open ($\
o, \">$tmp\");\n    open ($i, \"$tmp_out\");\n    \
while (<$i>)\n      {\n	my $l=$_;\n\n	\n	if ( $l=~\
/>/){print $o \"$l\";}\n	elsif ( $l=~/Predicted Se\
c. Struct./){$l=~s/Predicted Sec. Struct\\.//;prin\
t $o \"$l\";}\n      }\n    close ($o);\n    close\
...
eHandle;\n    my %P=%$PR;\n    my %A=%$AR;\n    my\
 %PA;\n    my $tmp=vtmpnam();\n    my $f= new File\
Handle;\n    \n    open ($f, \">$tmp\");\n    fore\
ach my $s (sort{$A{$a}{index}<=>$A{$b}{index}}(key\
...
=shift;\n    my $aln= shift;\n    my ($j,$a,$b);\n\
 ...
 $aln);\n    for ($a=$b=0;$a<=$#A; $a++)\n      {\\
n	if ($A[$a] ne \"-\"){$A[$a]=$P[$b++];}\n      }\\
n    if ($b!= ($#P+1)){add_warning (\"Could not th\
read sequence: $b $#P\");}\n    \n    $j= join (''\
, @A);\n    return $j;\n  }\nsub pred2color\n  {\n\
    my $predP=shift;\n    my $alnP=shift;\n    my \
$out=shift;\n    my $F=new FileHandle;\n    my $st\
...
eq_reformat -in $aln -struc_in $struc -struc_in_f \
number_fasta -output color_html -out $out\");\n   \
...
my $to=shift;\n    my $from_type=shift;\n    my $t\
o_type=shift;\n    my $action=shift;\n    my $msg;\
\n    \n    if (!&lock4tc($from, $from_type, \"LCH\
ECK\", \"\")){return 0;}\n    $msg=&lock4tc ($from\
, $from_type, \"LREAD\", \"\");\n    &lock4tc ($fr\
om, $from_type,\"LRELEASE\", $msg);\n    &lock4tc \
($to, $to_type, $action, $msg);\n    return;\n  }\\
nsub isshellpid\n  {\n    my $p=shift;\n    if (!l\
ock4tc ($p, \"LLOCK\", \"LCHECK\")){return 0;}\n  \
  else\n      {\n	my $c=lock4tc($p, \"LLOCK\", \"L\
READ\");\n	if ( $c=~/-SHELL-/){return 1;}\n      }\
\n    return 0;\n  }\nsub isrootpid\n  {\n    if(l\
ock4tc (getppid(), \"LLOCK\", \"LCHECK\")){return \
0;}\n    else {return 1;}\n  }\nsub lock4tc\n	{\n	\
  my ($pid,$type,$action,$value)=@_;\n	  my $fname\
;\n	  my $host=hostname;\n	  \n	  if ($type eq \"L\
LOCK\"){$fname=\"$LOCKDIR/.$pid.$host.lock4tcoffee\
\";}\n	  elsif ( $type eq \"LERROR\"){ $fname=\"$L\
OCKDIR/.$pid.$host.error4tcoffee\";}\n	  elsif ( $\
type eq \"LWARNING\"){ $fname=\"$LOCKDIR/.$pid.$ho\
st.warning4tcoffee\";}\n	  \n	  if ($debug_lock)\n\
	    {\n	      print STDERR \"\\n\\t---lock4tc(tcg\
): $action => $fname =>$value (RD: $LOCKDIR)\\n\";\
\n	    }\n\n	  if    ($action eq \"LCHECK\") {retu\
rn -e $fname;}\n	  elsif ($action eq \"LREAD\"){re\
turn file2string($fname);}\n	  elsif ($action eq \\
"LSET\") {return string2file ($value, $fname, \">>\
\");}\n	  elsif ($action eq \"LRESET\") {return st\
...
\n      $SIG{'SIGFPE'}='signal_cleanup';\n      \n\
      $SIG{'SIGKILL'}='signal_cleanup';\n      $SI\
G{'SIGPIPE'}='signal_cleanup';\n      $SIG{'SIGSTO\
P'}='signal_cleanup';\n      $SIG{'SIGTTIN'}='sign\
al_cleanup';\n      $SIG{'SIGXFSZ'}='signal_cleanu\
p';\n      $SIG{'SIGINFO'}='signal_cleanup';\n    \
  \n      $SIG{'SIGBUS'}='signal_cleanup';\n      \
$SIG{'SIGALRM'}='signal_cleanup';\n      $SIG{'SIG\
TSTP'}='signal_cleanup';\n      $SIG{'SIGTTOU'}='s\
ignal_cleanup';\n      $SIG{'SIGVTALRM'}='signal_c\
leanup';\n      $SIG{'SIGUSR1'}='signal_cleanup';\\
n\n\n      $SIG{'SIGSEGV'}='signal_cleanup';\n    \
  $SIG{'SIGTERM'}='signal_cleanup';\n      $SIG{'S\
IGCONT'}='signal_cleanup';\n      $SIG{'SIGIO'}='s\
ignal_cleanup';\n      $SIG{'SIGPROF'}='signal_cle\
anup';\n      $SIG{'SIGUSR2'}='signal_cleanup';\n\\
n      $SIG{'SIGSYS'}='signal_cleanup';\n      $SI\
G{'SIGURG'}='signal_cleanup';\n      $SIG{'SIGCHLD\
'}='signal_cleanup';\n      $SIG{'SIGXCPU'}='signa\
l_cleanup';\n      $SIG{'SIGWINCH'}='signal_cleanu\
p';\n      \n      $SIG{'INT'}='signal_cleanup';\n\
...
t;\n    my $ppid=shift;\n    my $type=shift;\n    \
my $com=shift;\n    \n    $ERROR_DONE=1;\n    lock\
4tc ($rpid, \"LERROR\",\"LSET\",\"$pid -- ERROR: $\
type\\n\");\n    lock4tc ($$, \"LERROR\",\"LSET\",\
 \"$pid -- COM: $com\\n\");\n    lock4tc ($$, \"LE\
RROR\",\"LSET\", \"$pid -- STACK: $ppid -> $pid\\n\
\");\n   \n    return $code;\n  }\nsub add_warning\
 \n  {\n    my $rpid=shift;\n    my $pid =shift;\n\
    my $command=shift;\n    my $msg=\"$$ -- WARNIN\
G: $command\\n\";\n    print STDERR \"$msg\";\n   \
...
{\n	my $ppid=getppid();\n	if (!$ERROR_DONE) \n	  {\
\n	    &lock4tc($$,\"LERROR\", \"LSET\", \"$$ -- S\
TACK: $p -> $$\\n\");\n	    &lock4tc($$,\"LERROR\"\
, \"LSET\", \"$$ -- COM: $CL\\n\");\n	  }\n      }\
\n    my $warning=&lock4tc($$, \"LWARNING\", \"LRE\
AD\", \"\");\n    my $error=&lock4tc($$,  \"LERROR\
\", \"LREAD\", \"\");\n    #release error and warn\
ing lock if root\n    \n    if (isrootpid() && ($w\
arning || $error) )\n      {\n	\n	print STDERR \"*\
*************** Summary *************\\n$error\\n$\
warning\\n\";\n\n	&lock4tc($$,\"LERROR\",\"RELEASE\
...
,\"LERROR\", \"LSET\"))\n	    {\n	      myexit(add\
_error ($EXIT_FAILURE,$$,$pid,getppid(), \"UNSPECI\
FIED system\", $com));\n	    }\n	}\n    }\n  retur\
n $?;\n}\n\nsub check_configuration \n    {\n     \
 my @l=@_;\n      my $v;\n      foreach my $p (@l)\
\n	{\n	  \n	  if   ( $p eq \"EMAIL\")\n	    { \n	 \
...
_installed (\"wget\") && !&pg_is_installed (\"curl\
\"))\n		{\n		  myexit(add_error ($EXIT_FAILURE,$$,\
...
$tmp;\n    &check_configuration ( \"wget\"); \n   \
 \n    $tmp=&vtmpnam ();\n    \n    if     (&pg_is\
_installed    (\"wget\")){`wget www.google.com -O$\
tmp >/dev/null 2>/dev/null`;}\n    elsif  (&pg_is_\
installed    (\"curl\")){`curl www.google.com -o$t\
...
ml);\n    if (!$r && $p=~/::/)\n      {\n	print ST\
DERR \"\\nYou Must Install the perl package $p on \
your system.\\nRUN:\\n\\tsudo perl -MCPAN -e 'inst\
all $pg'\\n\";\n      }\n    elsif (!$r)\n      {\\
n	myexit(flush_error(\"\\nProgram $p Supported but\
...
myname = basename($0);\n\n    if ($errmsg) {\n    \
    print STDERR \"ERROR: $errmsg\\n\";\n    }\n\n\
    print STDERR << \"EOF\";\n    \n$myname: align\
 two sequences by means of consan\\'s sfold\nUsage\
...
ning stuff is stockholm formatted\n    $cmd = \"se\
d -e '1d' $frawout\";\n    system(\"$cmd > $fstk\"\
);\n    if ($? != 0) {\n        $errmsg = \"comman\
d failed with exit status $?.\";\n        $errmsg \
.=  \"Command was \\\"$cmd\\\"\";\n        return \
-1;\n    }\n\n    # this gives an error message. j\
...
$)\n{\n    \n    my ($fs1, $fs2, $fmodel, $foutfa)\
 = @_;\n    \n\n    my ($cmd, $frawout, $ferrlog, \
$freadme, $ftimelog, $fstk);\n\n    # add  basenam\
e($fmsqin) (unknown here!)\n    $frawout = \"sfold\
.log\";\n    $ferrlog = \"sfold.err\";\n    $ftime\
log = \"sfold.time\";\n    $freadme =  \"sfold.REA\
DME\";\n    $fstk = \"sfold.stk\";\n    \n    # pr\
epare execution...\n    #\n    # ./tmp is essentia\
l for dswpalign\n    # otherwise you'll get a segf\
ault\n    mkdir \"./tmp\";\n    \n    $cmd = \"sfo\
ld -m $fmodel $fs1 $fs2\";\n    open(FREADME,\">$f\
readme\");\n    print FREADME \"$cmd\\n\"; \n    c\
lose(FREADME);\n\n    # and go\n    #\n    system(\
\"/usr/bin/time -p -o $ftimelog $cmd >$frawout 2>$\
ferrlog\");\n    if ($? != 0) {\n        $errmsg =\
 \"command failed with exit status $?\";\n        \
$errmsg .= \"command was \\\"$cmd\\\". See \" . ge\
tcwd . \"\\n\";\n        return -1;\n    }\n\n    \
...
msqin)) {\n    usage('missing input filename');\n \
   exit(1);\n}\nif (! defined($fmsaout)) {\n    us\
age('missing output filename');\n    exit(1);\n\n}\
\nif (scalar(@ARGV)) {\n    usage('Unknown remaini\
ng args');\n    exit(1);\n}\n\n$FMODEL = \"$ref_di\
r/mix80.mod\";\nif (! -e \"$FMODEL\") {\n    die(\\
"couldn't find sfold grammar model file. Expected \
$FMODEL\\n\");\n}\n\n\nmy %hseq=read_fasta_seq ($f\
msqin);\nmy $id;\n\nforeach $id (keys(%hseq))\n  {\
\n    push(@seq_array, $hseq{$id});\n  }\n\nif ( s\
calar(@seq_array) != 2 ) {\n    die(\"Need *exactl\
y* two sequences as input (pairwise alignment!).\"\
)\n}\n\n\n\nmy ($sec, $min, $hour, $mday, $mon, $y\
ear, $wday, $yday, $isdst) = localtime(time);\nmy \
$datei = sprintf(\"%4d-%02d-%02d\", $year+1900, $m\
on+1, $mday);\nmy $templ = basename($0) . \".\" . \
$datei . \".pid-\" . $$ . \".XXXXXX\";\nmy $wd = t\
empdir ( $templ, DIR => $TMPDIR);\n\ncopy($fmsqin,\
 \"$wd/\" . basename($fmsqin) . \".org\"); # for r\
eproduction\ncopy($FMODEL, \"$wd\");\nmy $fmodel =\
 basename($FMODEL);\nmy $orgwd = getcwd;\nchdir $w\
d;\n\n\n\nmy @sepseqfiles;\nforeach $id (keys(%hse\
q)) {\n    my ($seq, $orgseq, $fname, $sout);\n   \
 $seq=$hseq{$id}{'seq'};\n    \n    $fname = basen\
ame($fmsqin) . \"_$id.fa\";\n    # replace funnies\
 in file/id name (e.g. \"/\" \" \" etc)\n    $fnam\
e =~ s,[/ ],_,g;\n    open (PF, \">$fname\");\n   \
 print (PF \">$id\\n$seq\\n\");\n    close (PF);\n\
\n    push(@sepseqfiles, $fname);\n}\n\nmy ($f1, $\
f2, $fout);\n$f1 = $sepseqfiles[0];\n$f2 = $sepseq\
files[1];\n$fout = $wd . basename($fmsqin) . \".ou\
t.fa\";\nif (sfold_wrapper($f1, $f2, $fmodel, \"$f\
out\") != 0) {\n    printf STDERR \"ERROR: See log\
s in $wd\\n\";\n    exit(1);\n} else {\n    chdir \
$orgwd;\n    copy($fout, $fmsaout);\n    rmtree($w\
d);\n   exit(0);\n}\n","\nuse Env qw(HOST);\nuse E\
nv qw(HOME);\nuse Env qw(USER);\n\n\n$tmp=clean_cr\
...
n$c=0;\n\nforeach $l (@lines)\n  {\n    if (!($l=~\
/\\S/)){next;}\n    elsif ($stockholm && ($l=~/^\\\
/\\// || $l=~/STOCKHOLM/)){next;}#get read of STOC\
HOLM Terminator\n   \n    $l=~/(\\S+)\\s+(\\S*)/g;\
\n    $n=$1; $s=$2;\n    \n    $seq[$c].=$s;\n    \
$name[$c]=$n;\n    $c++;\n    \n    if ( $c==$nseq\
){$c=0;}\n    \n  } \n\nif ( $c!=0)\n      {\n	pri\
nt STDERR \"ERROR: $ARGV[0] is NOT an MSA in Clust\
alw format: make sure there is no blank line withi\
n a block [ERROR]\\n\";\n	exit (EXIT_FAILURE);\n  \
    }\n\nfor ($a=0; $a< $nseq; $a++)\n  {\n    $na\
...
ST);\nuse Env qw(HOME);\nuse Env qw(USER);\n\nmy $\
format=file2format ($ARGV[0]);\n\nif    ($format e\
q \"clustalw\"){clustalw2fasta($ARGV[0]);}\nelsif \
($format eq \"fasta\")   {fasta2fasta($ARGV[0]);}\\
nelsif ($format eq \"msf\")   {msf2fasta($ARGV[0])\
;}\nelsif ($format eq \"phylip\")   {phylip2fasta(\
$ARGV[0]);}\nelsif ($format eq \"nameseq\") {displ\
ay_file ($ARGV[0]);}\n \nexit (0);\n\nsub file2for\
mat\n  {\n    my $f=shift;\n    \n    my $l=file2n\
_lines($f,2);\n    \n    if ( $l=~/^CLUSTAL/){retu\
rn \"clustalw\";}\n    elsif ($l=~/^SAGA/){return \
...
n    elsif ($l=~/\\s+\\d+\\s+\\d+\\s/){return \"ph\
ylip\";}\n    elsif ($l=~/\\#NAMESEQ_01/){return \\
"nameseq\";}\n    else \n      {\n	print STDERR \"\
ERROR: $f FILE is NOT a supported format [ERROR]\\\
n\";\n	system (\"cp $f /Users/cnotredame/cedric1.t\
xt\");\n	exit (1);\n      }\n  }\nsub display_file\
\n    {\n       my $file=shift;\n       my $F= new\
 FileHandle;\n       open ($F, $file);\n       whi\
le (<$F>){print \"$_\";}\n       close ($F);\n    \
...
& /(\\S+)\\s+(.*)/)\n	    {\n	      $name=$1;\n	  \
    $seq=$2;\n	      $seq=~s/\\s//g;\n	      $seq=\
~s/\\~/\\-/g;\n	      $seq=~s/\\./\\-/g;\n	      i\
f ( $list{$n}{'name'} && $list{$n}{'name'} ne $nam\
e)\n		{\n		  print \"$list{$n}{'name'} Vs $name\";\
\n		  \n		  exit (1);\n		}\n	      else\n		{\n		  \
$list{$n}{'name'}= $name;\n		}\n	      \n	      $l\
ist{$n}{'seq'}=$list{$n}{'seq'}.$seq;\n	      \n	 \
     $nseq=++$n;\n	      \n	    }\n	  else\n	    {\
$n=0;}\n	}\n      close ($F);\n      \n      for (\
my $a=0; $a<$nseq; $a++)\n	{\n	  my $nl=length ($l\
ist{$a}{'name'});\n	  my $sl=length ($list{$a}{'se\
q'});\n	  print \">$list{$a}{'name'}\\n$list{$a}{'\
seq'}\\n\";\n	}\n    }\n    \nsub fasta2fasta\n   \
 {\n      my $file=shift;\n      my $F= new FileHa\
ndle;\n      my ($seq, $name,$n,$l,%len);\n      m\
...
robase_symbol_/g;\n    \n    \n    #Remove annotat\
ion\n    $file=~s/\\n[\\.:*\\s]+\\n/\\n\\n/g;\n   \
 \n    #Remove White spaces before the sequence na\
me\n    $file=~s/\\n[ \\t\\r\\f]+(\\b)/\\n\\1/g;\n\
...
e;\n    shift (@blocks);\n    @s=split /\\@/, $blo\
cks[0];\n    $nseq=$#s+1;\n    \n    #Merge all th\
e sequences and split every Nseq\n    \n    \n    \
$file=join '@', @blocks;\n    @lines=split /\\@/,$\
file;\n    \n    $c=0;\n    \n    foreach my $l (@\
lines)\n      {\n	my ($n, $s);\n	\n	if (!($l=~/\\S\
/)){next;}\n	elsif ($stockholm && ($l=~/^\\/\\// |\
| $l=~/STOCKHOLM/)){next;}#get read of STOCHOLM Te\
rminator\n	\n	$l=~/(\\S+)\\s+(\\S*)/g;\n	$n=$1; $s\
=$2;\n	\n	$seq[$c].=$s;\n	$name[$c]=$n;\n	$c++;\n	\
\n	if ( $c==$nseq){$c=0;}\n	\n      } \n    \n    \
if ( $c!=0)\n      {\n	print STDERR \"ERROR: $fnam\
e is NOT an MSA in Clustalw format: make sure ther\
...
\S+)/)\n      {\n	$s=$seqname{$nseq++}=$1;\n	print\
 OUT \"$l\";\n	\n      }\n    else \n      {\n	$l=\
uc($l);\n	print OUT \"$l\";\n      }\n  }\n\nclose\
 (F);\nclose(OUT);\n\nsystem (\"msa $msaf > $msaou\
tf 2>$err\");\nopen (F, \"$msaoutf\");\n$read=0;\n\
$cn=0;\nwhile (<F>)\n  {\n    $l=$_;\n    if ($rea\
d)\n      {\n	if ($l=~/End gaps not penalized/){$r\
ead=0;}\n	elsif (!($l=~/\\S/))\n	  {\n	    $cn=0;\\
n	  }\n	else\n	  {\n	    \n	    chomp ($l);\n	    \
$seqal{$cn++}.=$l;\n	    $tot++;\n	  }\n      }\n \
 ...
 {\n	$read=1;\n      }\n  }\nclose (F);\n\nif ($to\
t<1 && $run_anyway==1)\n  {\n    print STDERR \"\\\
nWarning: MSA returned a NULL file -- Use T-Coffee\
 instead\\n\";\n    open (F,$err);\n    while (<F>\
){print \"$_\";}\n      \n    system (\"t_coffee -\
seq $msaf -outfile $ARGV[1]  -quiet\");\n  }\nelsi\
f ($tot<1 && $run_anyway==2)\n  {\n    \n    \n   \
 $nseq/=2;\n    $nseq=int ($nseq);\n    if ($nseq<\
2){$nseq=2;}\n    print \"RUN MSA with NSeq=$nseq\\
\n\";\n    #print (\"t_coffee -dpa -dpa_nseq $nseq\
 -seq $ARGV[0] -dpa_tree codnd -outfile $ARGV[1] -\
dpa_method msa_msa\");\n    system (\"t_coffee -dp\
a -dpa_nseq $nseq -seq $ARGV[0] -dpa_tree codnd -o\
utfile $ARGV[1] -dpa_method msa_msa>/dev/null\");\\
n\n  }\nelsif ($tot<1)\n  {\n    exit (EXIT_FAILUR\
E);\n  }\nelse\n  {\n    open (OUT, \">$ARGV[1]\")\
;\n    for ($a=0; $a<$nseq;$a++)\n      {\n	print \
OUT \">$seqname{$a}\\n$seqal{$a}\\n\";\n      }\n \
...
$h{$n}{NFout}=\"$dir/$h{$n}{name}.aln\";\n\n      \
      $h{$n}{out}=$f[1];\n\n            fasta2fast\
aupp ($h{$n}{in}, $h{$n}{NFin});\n            $n++\
;\n          }\n      }\n    close (F);\n    chdir\
 ($dir);\n    \n    if (!$test)\n      {\n	system \
(\"fbname=\\$(basename `ls *.seq` .seq); \\\n     \
        run_upp.py -s \\${fbname}.seq -m amino --c\
pu 1 -d outdir -o \\${fbname}.aln; \\\n           \
  mv outdir/\\${fbname}.aln_alignment.fasta \\${fb\
name}.aln;\");\n      }\n    \n    foreach my $n (\
keys (%h))\n      {\n	if ($test)\n	  {\n	    syste\
m (\"cp $h{$n}{NFin} $h{$n}{NFout}\");\n	    print\
 \"$h{$n}{NFin} $h{$n}{NFout} $h{$n}{out}\\n\";\n	\
  }\n        fastaupp2fasta ($h{$n}{NFout},$h{$n}{\
out});\n      }\n    chdir ($cdir);\n  }\n\nsub se\
q2msa\n    {\n      my ($in, $out)=@_;\n      my $\
cdir=getcwd;\n      \n      \n      if (!($in=~/\\\
//)){$in=$cdir.\"/\".$in;}\n      if (!($out=~/\\/\
...
n	    $h{$n}{NFin}= \"$dir/$h{$n}{name}.seq4nf\";\\
n	    $h{$n}{NFout}=\"$dir/$h{$n}{name}.aln\";\n	 \
   \n	    $h{$n}{out}=$f[1];\n	    \n	    translat\
e_fasta_seq (\"uU\", \"X\",$h{$n}{in}, $h{$n}{NFin\
});\n	    $n++;\n	  }\n      }\n    close (F);\n  \
  \n    \n    chdir ($dir);\n    dump_nf (\"nf\");\
\n    dump_config ();\n   \n    #system (\"nextflo\
w run nf  --name \\'*.seq4nf\\' >/dev/null 2>/dev/\
...
y $seq=random_string(10);\n    $seq.=\".fa\";\n   \
 my $aln=$seq;\n    $aln.=\".aln\";\n    \n    mkd\
ir ($dir);\n    translate_fasta_seq (\"uU\", \"X\"\
,$in, \"$dir/$seq\");\n    chdir ($dir);\n    \n  \
  dump_nf (\"nf\");\n    dump_config ();\n    prin\
t \"IN: $in OUT: $cdir/$out\\nDIR: $dir\\nnextflow\
 run nf  --name \\'*.fa\\' \\n\";\n    system (\"n\
extflow run nf  --name \\'*.fa\\' \");\n    print \
\"$dir/$aln $cdir/$out\\n\";\n    translate_fasta_\
seq (\"xX\", \"U\",$aln, \"$cdir/$out\");\n    chd\
...
n\n      print F \"docker.enabled = true\\n\";\n  \
    print F \"process.container = \\'cbcrg/benchfa\
m_large_scale\\'\\n\";\n      close (F);\n    }\n\\
nsub dump_nf\n  {\n    my $nff=shift;\n    open (F\
,\">$nff\");\n    print F \"#!/usr/bin/env nextflo\
w\\n\";\n    print F \"params.base_dir=\\\"./\\\"\\
\n\";\n    print F \"params.out_dir=\\\"./\\\"\\n\\
";\n    print F \"Channel.fromPath(params.name)\\n\
\";\n    print F \"\\t.map{ tuple(it.baseName, it)\
 }\\n\";\n    \n    print F \"\\t.set{ file_names_\
1 }\\n\";\n    print F \"process clustalo_align{\\\
n\";\n    print F \"\\tpublishDir params.out_dir, \
mode: \\\"copy\\\"\\n\";\n    print F \"tag \\\"\\\
${name}\\\"\";\n    print F \"\\n\";\n    print F \
\"\\tinput:\\n\";\n    print F \"\\tset name, file\
(seq_file) from file_names_1\\n\";\n    print F \"\
\\toutput:\\n\";\n    print F \"\\tfile \\\"\\${na\
me}.aln\\\"\\n\";\n    print F \"\\n\";\n    print\
 F \" \\\"\\\"\\\"\\n\";\n    print F \" clustalo \
-i \\$seq_file -o \\${name}.aln\\n\";\n    print F\
 \"\\\"\\\"\\\"\\n\\n\";\n    print F \"}\\n\";\n \
   close (F);\n  }\n\nsub random_string\n    {\n  \
    my $len=shift;\n      my @chars = (\"A\"..\"Z\\
", \"a\"..\"z\");\n      my $string;\n      $strin\
g .= $chars[rand @chars] for 1..$len;\n      retur\
n $string;\n    }\n\nsub mymkdir\n      {\n	my $d=\
shift;\n	my $cd='/';\n	\n	foreach my $e (split (/\\
\//, $d))\n	  {\n	    $cd.=\"$e/\";\n	    if ( !-d\
 $cd){mkdir ($cd);}\n	  }\n	return;\n      }\n    \
...
T);\nuse Env qw(HOME);\nuse Env qw(USER);\n\n     \
 ...
 \nuse strict;                                    \
         \nuse warnings;\nuse diagnostics;\n\nmy $\
in_hit_list, my $in_aln=0, my(%name_list)=(),my (%\
list)=(),my $n_seq=0; my $test=0;\nmy($j)=0, my $n\
=0, my $nom, my $lg_query, my %vu=();\n\nopen (F, \
\">tmp\");\n\n$/=\"\\n\";\nwhile (<>)\n{\n    prin\
t F $_;\n    if($_ =~ /Query=\\s*(.+?)\\s/i) { $no\
m=$1;}\n\n    if ( /Sequences producing significan\
t alignments/){$in_hit_list=1;}\n    \n    if ($_=\
~ /^pdb\\|/i) { $_=~ s/pdb\\|//g; }\n    if ($_=~ \
...
s+)/ && ($in_aln == 1))\n    {\n	my $name=$1;\n	my\
 $start=$2;\n	my $seq=$3;\n	my $end=$4;\n		\n	if (\
$name =~ /QUERY/i) { $lg_query=length($seq); }\n\n\
	unless ($test > $n) #m\n	{\n	    my(@seqq)= split\
('',$seq);\n	    my($gap_missing)= scalar(@seqq);\\
n	    \n	    while ($gap_missing != $lg_query)  { \
unshift (@seqq,\"-\"); $gap_missing= scalar(@seqq)\
; }\n	    $seq=join('',@seqq);  #m\n	}\n	\n	if ($n\
ame =~ /QUERY/i)\n	{\n	    $n=0; %vu=(); $j=0;\n	 \
...
n	$list{$n}{'name'}=$name;\n\n	$seq=~tr/a-z/A-Z/;\\
n	$list{$n}{'seq'}=$list{$n}{'seq'};\n	$list{$n}{'\
seq'}.=$seq;\n\n	$n++;\n	$vu{$name}++;\n	$test++;\\
n   } \n    \n}\n\nmy @numero=();\n\nfor (my $a=0;\
 $a<$n; $a++) #m\n{\n    my $long=length($list{0}{\
'seq'});  \n    my $long1= length($list{$a}{'seq'}\
);\n  \n    while ($long1 ne $long)\n    {\n	$list\
{$a}{'seq'}.=\"-\";\n	$long1= length ($list{$a}{'s\
eq'});\n    } \n \n    push (@numero,\"$list{$a}{'\
name'} $list{$a}{'real_name'}\\n\");\n}\n\nmy %dej\
avu=();\n\n\nfor (my $i=0; $i<=$#numero; $i++)\n{\\
n    my $s=\">$list{$i}{'real_name'}\\n$list{$i}{'\
seq'}\\n\";\n    my $k=0;\n    \n    if (exists $d\
ejavu{$numero[$i]}) {next;}\n    else\n    {	\n	fo\
r ($j=0; $j<$n ; $j++)\n	{\n	    if (\"$numero[$i]\
\" eq \"$numero[$j]\" && $j != $i )\n	    {\n		++$\
k;\n		$s .=\">$list{$j}{'real_name'}\\n$list{$j}{'\
seq'}\\n\";\n	    }\n	}	\n    }\n    \n    if ($k>\
0) \n    {\n	my $cons;\n	open (SOR,\">tempo_aln2co\
ns\"); print SOR $s;  close SOR ;\n	open (COM,\"t_\
coffee -other_pg seq_reformat -in tempo_aln2cons -\
action +aln2cons +upper |\") ; \n     	while (<COM\
>)\n	{	\n	    if (/^>/) { $cons =\">$list{$i}{'rea\
l_name'}\\n\"; next;}\n	    $_=~ s/\\n//g;\n	    $\
cons .=$_;\n	}\n	close COM; unlink (\"tempo_aln2co\
ns\");\n	print $cons,\"\\n\"; print F $cons,\"\\n\\
";\n    }	\n    else  { print $s;  print F $s; }\n\
    \n    $dejavu{$numero[$i]}++;\n} #m\n\nexit;\n\
\n\n\n\n\n\n\n\n\n\n\n","use Env;\n\n\n$tmp_dir=\"\
\";\n$init_dir=\"\";\n$program=\"tc_generic_method\
.pl\";\n\n$blast=@ARGV[0];\n\n$name=\"query\";$seq\
=\"\";\n%p=blast_xml2profile($name,$seq,100, 0, 0,\
 $blast);\n&output_profile (%p);\n\n\nsub output_p\
rofile\n  {\n    my (%profile)=(@_);\n    my ($a);\
\n    for ($a=0; $a<$profile{n}; $a++)\n      {\n	\
\n	print \">$profile{$a}{name} $profile{$a}{commen\
t}\\n$profile{$a}{seq}\\n\";\n      }\n    return;\
\n  }\nsub file_contains \n  {\n    my ($file, $ta\
g, $max)=(@_);\n    my ($n);\n    $n=0;\n    \n   \
...
=\"EBI\";}\n    elsif (&file_contains ($file,\"NCB\
I_BlastOutput\",100)) {$BLAST_TYPE=\"NCBI\";}\n   \
 else\n      {\n	$BLAST_TYPE=\"\";\n      }\n    r\
...
\",100)){%p=ncbi_blast_xml2profile(@_);}\n    else\
 \n      {\n	print \"************ ERROR: Blast Ret\
urned an unknown XML Format **********************\
\";\n	die;\n      }\n    for ($a=0; $a<$p{n}; $a++\
)\n      {\n	my $name=$p{$a}{name};\n	$p{$name}{se\
q}=$p{$a}{seq};\n      }\n    return %p;\n  }\nsub\
 ncbi_blast_xml2profile \n  {\n    my ($name,$seq,\
...
,%HSP);\n	\n	$ldb=\"\";\n\n	%ID=&xml2tag_list ($hi\
t{$a}{body}, \"Hit_id\");\n	$identifyer=$ID{0}{bod\
y};\n	\n	%DE=&xml2tag_list ($hit{$a}{body}, \"Hit_\
def\");\n	$definition=$DE{0}{body};\n	\n	%HSP=&xml\
2tag_list ($hit{$a}{body}, \"Hsp\");\n	for ($b=0; \
$b<$HSP{n}; $b++)\n	  {\n	    my (%START,%END,%E,%\
I,%Q,%M);\n\n	 \n	    %START=&xml2tag_list ($HSP{$\
b}{body}, \"Hsp_query-from\");\n	    %HSTART=&xml2\
tag_list ($HSP{$b}{body}, \"Hsp_hit-from\");\n	   \
 \n	    %LEN=  &xml2tag_list ($HSP{$b}{body}, \"Hs\
p_align-len\");\n	    %END=  &xml2tag_list ($HSP{$\
b}{body}, \"Hsp_query-to\");\n	    %HEND=  &xml2ta\
g_list ($HSP{$b}{body}, \"Hsp_hit-to\");\n	    %E=\
&xml2tag_list     ($HSP{$b}{body}, \"Hsp_evalue\")\
;\n	    %I=&xml2tag_list     ($HSP{$b}{body}, \"Hs\
p_identity\");\n	    %Q=&xml2tag_list     ($HSP{$b\
}{body}, \"Hsp_qseq\");\n	    %M=&xml2tag_list    \
 ($HSP{$b}{body}, \"Hsp_hseq\");\n	    \n	    for \
($e=0; $e<$Q{n}; $e++)\n\n	      {\n		$qs=$Q{$e}{b\
ody};\n		$ms=$M{$e}{body};\n		if ($seq eq\"\"){$se\
q=$qs;$L=length($seq);}\n		\n		$expectation=$E{$e}\
{body};\n		$identity=($LEN{$e}{body}==0)?0:$I{$e}{\
body}/$LEN{$e}{body}*100;\n		$start=$START{$e}{bod\
y};\n		$end=$END{$e}{body};\n		$Hstart=$HSTART{$e}\
{body};\n		$Hend=$HEND{$e}{body};\n	\n		$coverage=\
(($end-$start)*100)/$L;\n\n	\n		if ($identity>$max\
...
hits]=$identifyer;\n		$comment[$nhits]=\"$ldb|$ide\
ntifyer [Eval=$expectation][id=$identity%][start=$\
Hstart end=$Hend]\";\n		$nhits++;\n	      }\n	  }\\
n      }\n    \n    $profile{n}=0;\n    $profile{$\
profile{n}}{name}=$name;\n    $profile{$profile{n}\
}{seq}=$seq;\n    $profile {n}++;\n    \n    for (\
$a=0; $a<$nhits; $a++)\n      {\n	$n=$a+1;\n	\n	$p\
rofile{$n}{name}=\"$name\\_$a\";\n	$profile{$n}{se\
q}=\"\";\n	$profile{$n}{Qseq}=$Qseq[$a];\n	$profil\
e{$n}{Hseq}=$Hseq[$a];\n	$profile{$n}{Qstart}=$Qst\
artL[$a];\n	$profile{$n}{Hstart}=$HstartL[$a];\n	$\
...
\n	my (%Q,%M,%E,%I);\n	\n	$ldb=&tag2value ($hit{$a\
}{open}, \"database\");\n	$identifyer=&tag2value (\
$hit{$a}{open}, \"id\");\n\n	$description=&tag2val\
ue ($hit{$a}{open}, \"description\");\n	\n	%Q=&xml\
2tag_list ($hit{$a}{body}, \"querySeq\");\n	%M=&xm\
l2tag_list ($hit{$a}{body}, \"matchSeq\");\n	%E=&x\
ml2tag_list ($hit{$a}{body}, \"expectation\");\n	%\
I=&xml2tag_list ($hit{$a}{body}, \"identity\");\n	\
\n\n	for ($b=0; $b<$Q{n}; $b++)\n	  {\n	    \n	   \
 \n	    $qs=$Q{$b}{body};\n	    $ms=$M{$b}{body};\\
n	    if ($seq eq\"\"){$seq=$qs;$L=length($seq);}\\
n\n	    $expectation=$E{$b}{body};\n	    $identity\
=$I{$b}{body};\n	    \n	    	    \n	    $start=&ta\
g2value ($Q{$b}{open}, \"start\");\n	    $end=&tag\
2value ($Q{$b}{open}, \"end\");\n	    $startM=&tag\
2value ($M{$b}{open}, \"start\");\n	    $endM=&tag\
2value ($M{$b}{open}, \"end\");\n	    $coverage=((\
$end-$start)*100)/$L;\n	    \n	   # print \"$id: I\
D: $identity COV: $coverage [$start $end]\\n\";\n	\
    \n	    \n	    if ($identity>$maxid || $identit\
y<$minid || $coverage<$mincov){next;}\n	    # prin\
t \"KEEP\\n\";\n\n	    \n	    @lr1=(split (//,$qs)\
);\n	    @lr2=(split (//,$ms));\n	    $l=$#lr1+1;\\
n	    for ($c=0;$c<$L;$c++){$p[$nhits][$c]=\"-\";}\
\n	    for ($d=0,$c=0; $c<$l; $c++)\n	      {\n		$\
r=$lr1[$c];\n		if ( $r=~/[A-Za-z]/)\n		  {\n		    \
