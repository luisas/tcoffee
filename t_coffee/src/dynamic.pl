...
if ($cmethod eq "tcoffee"|| $cmethod eq "t_coffee" )
...
elsif ($cmethod eq "mafftginsi")
  {
        ...
        my_system ("ginsi --treein ${id}.mafftbinary ${seqs} > ${id}.prog.${align_method}.with.${tree_method}.tree.aln");
        print "\n\n![dynamic.pl][--------------MAFFTGINSI TESTING -------]ginsi --treein ${id}.mafftbinary ${seqs} > ${id}.prog.${align_method}.with.${tree_method}.tree.aln
";
