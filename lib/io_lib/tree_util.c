...
NT_node tree2collapsed_tree(NT_node T, int n, char **string)
{
  ...
  else if (n > 1)
  {
    ...
    for (a = 0; a < n; a++)
    {
      buf = strcat(buf, string[a]), buf = strcat(buf, " ");
    }
...
char *prune_treeF(NT_node T, Sequence *S, char *tmp)
{
  ...
  {
    if (R[a]->parent)
      r[a][0] = (R[a]->parent)->order;
    else
      r[a][0] = -1;

    if (R[a]->left)
      r[a][1] = (R[a]->left)->order;
    else
      r[a][1] = -1;

    if (R[a]->right)
      r[a][2] = (R[a]->right)->order;
    else
      r[a][2] = -1;
...
int compare_aln_tree(NT_node T, Alignment *A, int *n, FILE *fp)
{
  ...
  else
  {
    char *seq1, *seq2;
    if (!(T->left)->seqal)
      compare_aln_tree(T->left, A, n, fp);
    if (!(T->right)->seqal)
      compare_aln_tree(T->right, A, n, fp);
...
NT_node tree_scan(Alignment *A, NT_node RT, char *pscan, char *ptree)
{
  ...
  else if (strm(mode, "single"))
  {
    for (b = 0, ax = 0; ax < nl; ax++)
    {
      ...
      if (pstart < 1 || pstart > A->len_aln)
        continue;
      if (pend < 1 || pend > A->len_aln)
        continue;
      ...
      for (cx = 0; cx < nl; cx++)
      {
        ...
        for (d = start; d <= w; d++)
        {
          for (b = start; b <= w; b++)
          {
            ...
            if (pend < 1 || pend > A->len_aln)
              continue;
            if (p2start < 1 || p2start > A->len_aln)
              continue;
            if (p2end < 1 || p2end > A->len_aln)
              continue;
            if (pstart <= p2start && pend >= p2start)
              continue;
            if (pstart <= p2end && pend >= p2end)
              continue;
            ...
            if (TS->uw >= best_score)
            {
              ...
              best_start = pstart;
              best_end = pend;
              best_pos2 = c, best_w2 = d, best_start2 = p2start, best_end2 = p2end;
              set = 1;
            }
            vfree(TS);
          }
...
Tree_sim *tree_scan_multiple_pos(int *poslist, int *wlist, int nl, Alignment *A, char *ptree, NT_node RT)
{
  ...
  for (s = 0; s < A->nseq; s++)
  {
    ...
    {
      if (pos[a])
        B->seq_al[s][n++] = A->seq_al[s][a - 1];
    }
  }

  B->len_aln = n;
  for (s = 0; s < A->nseq; s++)
    B->seq_al[s][B->len_aln] = '\0';
...
Tree_sim *tree_scan_pos(Alignment *A, int start, int end, char *ptree, NT_node RT)
{
  ...
  if (start < 1 || start > A->len_aln)
    return NULL;
  if (end < 1 || end > A->len_aln)
    return NULL;
...
int print_node_list(NT_node T, Sequence *RS)
{
  ...
  while (L[0])
  {
    ...
    for (b = 0; b < S->nseq; b++)
    {
      ...
      if (p != -1)
        nlseq2[p] = (L[0])->lseq2[b] + '0';
    }
    for (b = 0; b < RS->nseq; b++)
      fprintf(stdout, "%c", nlseq2[b]);
    fprintf(stdout, "\n");
    L++;
  }
  return 1;
...
Alignment *phylotrim2(Alignment *A, char *reftree, int *l, int ph, int narg, char **argl, char *name)
{
  ...
  for (a = 0; a < A->nseq; a++)
  {
    fprintf(stdout, "%d", (ph == 1) ? l[a] : 1 - l[a]);
  }
...
NT_node new_search_split(NT_node T, NT_node B, int nseq)
{
  ...
  if (!T || !B)
    return NULL;
  else if (new_compare_split(T->lseq2, B->lseq2, nseq) == 1)
    return B;
  else if ((N = new_search_split(T, B->right, nseq)))
    return N;
  else
    return new_search_split(T, B->left, nseq);
}
int new_compare_split(int *b1, int *b2, int n)
{
  int a, flag;
...
float compare_trees(NT_node T1, NT_node T2, int nseq, int mode)
{
  ...
  if (getenv4debug("DEBUG_TREE_COMPARE"))
    display_node(T1, "\nNODE ", nseq);

  if (T1->parent && T1->nseq > 1)
...
float search_node(NT_node B, NT_node T, int nseq, int mode)
{
  ...
  {
    if (getenv4debug("DEBUG_TREE_COMPARE"))
      fprintf(stderr, "[1][%d]", (int)evaluate_node_similarity(B, T, nseq, mode));
    if (mode == RECODE)
      B->dist = B->leaf;
...
NT_node main_aln2tree(Alignment *A, int n, char **arglist, char *name)
{
  ...
  else
  {
    ...
    if (!file_exists(NULL, CacheAln) || !file_exists(NULL, CacheTree) || file2diff(CacheAln, tmpA))
    {
      ...
      T = tree_compute(A, n, arglist);
      if (T)
        vfclose(print_tree(T, "newick", vfopen(CacheTree, "w")));
      free_tree(T);
    }
    else
    {
...
NT_node tree_compute(Alignment *A, int n, char **arg_list)
{
  ...
  else if (strm(arg_list[0], "kmeans"))
    return aln2km_tree(A, NULL, 1);
  else if (strm(arg_list[0], "trmsd"))
    return aln2trmsd_tree(A, arg_list[1]);
...
NT_node reset_dist_tree(NT_node R, float n)
{
  ...
  if (R->parent && !(R->parent)->parent && !(R->parent)->bot)
    R->dist = n / 2;
  else
    R->dist = n;
...
int decode_seq_in_tree(NT_node R, char **name)
{
  ...
  else
  {
    int s = atoi(R->name);
    vfree(R->name);
    R->name = (char *)vcalloc(strlen(name[s - 1]) + 1, sizeof(char));
...
NT_node rename_seq_in_tree(NT_node R, char ***list)
{
  ...
  else
  {
    ...
    while (list[n][0][0])
    {
      if (strm(list[n][0], R->name))
        sprintf(R->name, "%s", list[n][1]);
      n++;
    }
  }
  return R;
...
Sequence *tree2seq(NT_node p, Sequence *S)
{
  ...
  while (p)
  {
    ...
    if (x >= 3 || p->isseq)
    {
      ...
      {
        if (p->isseq)
          sprintf(S->name[S->nseq++], "%s", p->name);
      }
      if (x > 3)
        p = NULL;
      else if (p)
  ...
  while (current != NULL)
  {
    if (!current->left)
    {
      if (current && current->isseq && !current->visited)
      {
        ...
        if (i < S->nseq)
          current->name = csprintf(current->name, "%s", S->name[i]);
        else
    ...
    else
    {
      ...
      if (!pre->visited)
      {
        if (pre && !pre->visited && pre->isseq)
        {
          ...
          if (i < S->nseq)
            current->name = csprintf(pre->name, "%s", S->name[i]);
          else
      ...
      while (pre->right && pre->right != current)
      {
        ...
        if (pre)
        {
          if (pre && !pre->visited && pre->isseq)
          {
            int i = atoi(pre->name) - 1;
            if (i < S->nseq)
              pre->name = csprintf(pre->name, "%s", S->name[i]);
            else
              printf_exit(EXIT_FAILURE, stderr, "\nERROR: Sequence %s is in tree but not in source sequence file [FATAL]", pre->name);
            rv++;
          }
  ...
  while (current != NULL)
  {
    if (!current->left)
    {
      if (current && current->isseq && !current->visited)
      {
        ...
        if (i != -1)
          current->name = csprintf(current->name, "%d", i + 1);
        else
          printf_exit(EXIT_FAILURE, stderr, "\nERROR: Sequence %s is in tree but not in source sequence file [FATAL]", current->name);
        rv++;
      }
    ...
    else
    {
      ...
      if (!pre->visited)
      {
        if (pre && !pre->visited && pre->isseq)
        {
          ...
          if (i != -1)
            current->name = csprintf(pre->name, "%d", i + 1);
          else
            printf_exit(EXIT_FAILURE, stderr, "\nERROR: Sequence %s is in tree but not in source sequence file [FATAL]", pre->name);
          rv++;
        }
      ...
      while (pre->right && pre->right != current)
      {
        ...
        if (pre)
        {
          if (pre && !pre->visited && pre->isseq)
          {
            int i = name_is_in_hlist(pre->name, S->name, S->nseq);
            if (i != -1)
              pre->name = csprintf(pre->name, "%d", i + 1);
            else
              printf_exit(EXIT_FAILURE, stderr, "\nERROR: Sequence %s is in tree but not in source sequence file [FATAL]", pre->name);
  ...
  while (current != NULL)
  {
    if (!current->left)
    {
      if (current && current->isseq && !current->visited)
      {
        int i = name_is_in_hlist(current->name, S->name, S->nseq);
        if (i != -1)
          current->name = csprintf(current->name, "%d_%s", i + 1, current->name);
        else
          printf_exit(EXIT_FAILURE, stderr, "\nERROR: Sequence %s is in tree but not in source sequence file [FATAL]", current->name);
        rv++;
      }
    ...
    else
    {
      ...
      if (!pre->visited)
      {
        if (pre && !pre->visited && pre->isseq)
        {
          int i = name_is_in_hlist(pre->name, S->name, S->nseq);
          if (i != -1)
            current->name = csprintf(pre->name, "%d_%s", i + 1, pre->name);
          else
            printf_exit(EXIT_FAILURE, stderr, "\nERROR: Sequence %s is in tree but not in source sequence file [FATAL]", pre->name);
          rv++;
        }
      ...
      while (pre->right && pre->right != current)
      {
        ...
        if (pre)
        {
          if (pre && !pre->visited && pre->isseq)
          {
            int i = name_is_in_hlist(pre->name, S->name, S->nseq);
            if (i != -1)
              pre->name = csprintf(pre->name, "%d_%s", i + 1, pre->name);
            else
              printf_exit(EXIT_FAILURE, stderr, "\nERROR: Sequence %s is in tree but not in source sequence file [FATAL]", pre->name);
...
for ( a=0; a< R->nseq; a++) fprintf ( stderr, "[%d]", R->lseq[a]);
    */
    fprintf(fp, "\n %10s N ", R->name);
    for (a = 0; a < nseq; a++)
      fprintf(fp, "%d", R->lseq2[a]);
    fprintf(fp, "\n %10s D ", R->name);
    for (a = 0; a < nseq; a++)
      fprintf(fp, "%d", R->idist[a]);
...
int tree2nleaf(NT_node root)
{
  ...
  while (current)
  {
    if (!current->left)
    {
      if (!current->visited && current->isseq)
        nleaf++;
      current->visited = 1;
      current = current->right;
    }
...
int rec_tree2nleaf(NT_node R)
{
  ...
  else if (R->right == NULL && R->left == NULL && R->bot == NULL)
  {
    R->leaf = 1;
    return 1;
  }
...
NT_node reroot_tree(NT_node TREE, NT_node Right)
{
  ...
  if (!EMPTY)
    EMPTY = (NT_node)vcalloc(1, sizeof(NT_node));
  if (!Right->parent)
    return Right;
  ...
  if ((NR->left)->right == Right)
    (NR->left)->right = EMPTY;
  else if ((NR->left)->left == Right)
    (NR->left)->left = EMPTY;
...
FILE *no_rec_print_tree(NT_node p, FILE *fp)
{
  ...
  if (ns == 1)
  {
    ...
    if (p->name)
      fprintf(fp, "(%s:1.000)", p->name);
    else if (p->right)
      fprintf(fp, "(%s:1.000)", (p->right)->name);
    else if (p->left)
      fprintf(fp, "(%s:1.000)", (p->left)->name);
    return fp;
  }
  ...
  while (p)
  {
    ...
    if (x >= 3 || p->isseq)
    {
      ...
      {
        if (p->isseq)
          fprintf(fp, "%s:%.5f", p->name, p->dist);
        else
        {
          if (p->bootstrap != 0)
            fprintf(fp, "%d", (int)p->bootstrap);
...
int *aln2seq_chain(Alignment *A, int **sim, int seq1, int seq2, int limit, int max_chain)
{
  ...
  if (find_seq_chain(A, sim, used, seq1, seq1, seq2, 1, limit, max_chain, &nseq))
  {
    ...
    sprintf(output2, "#%s %s N: %d Lower: %d Sim: %d DELTA: %d\n", A->name[list[0]], A->name[list[n - 1]], n, limit, sim[list[0]][list[n - 1]], limit - sim[list[0]][list[n - 1]]);
    ...
    for (a = 0; a < n - 1; a++)
    {
      sprintf(output2, "%s -->%d -->", A->name[chain[a][1]], sim[chain[a][1]][chain[a + 1][1]]);
      strcat(output1, output2);
    }
    sprintf(output2, "%s\n", A->name[chain[n - 1][1]]);
    strcat(output1, output2);
...
Sequence *get_fasta_tree(char *fname, char *comment_out)
{
  ...
  while (c != EOF)
  {
    if (c == '>')
    {
      ...
      if (LS->name[current][l - 1] == ',' || LS->name[current][l - 1] == ';')
        LS->name[current][l - 1] = '\0';
...
void output_fasta_tree(char *fname, Alignment *A)
{
  ...
  if (A->Tree)
    return output_fasta_tree(fname, A->Tree);
...
int output_treelist(char *fname, Alignment *A)
{
  ...
  if (A->Tree)
    return output_treelist(fname, A->Tree);
...
NT_node find_node_in_tree(int *key, int nseq, NT_node T)
{
  ...
  else
  {
    ...
    for (yes = 1, a = 0; a < nseq; a++)
    {
      ...
      else if (!key[a] && T->lseq2[a])
        yes = 0;
    }

    if (yes)
      return T;
    else if ((C = find_node_in_tree(key, nseq, T->right)))
      return C;
    else if ((C = find_node_in_tree(key, nseq, T->left)))
      return C;
    else
    {
...
NT_node recode_tree(NT_node T, Sequence *S)
{
  ...
  vfree(T->lseq);
  T->lseq = (int *)vcalloc(S->nseq, sizeof(int));
  vfree(T->lseq2);
  T->lseq2 = (int *)vcalloc(S->nseq, sizeof(int));
  vfree(T->idist);
  T->idist = (int *)vcalloc(S->nseq, sizeof(int));
  vfree(T->ldist);
  T->ldist = (int *)vcalloc(S->nseq, sizeof(int));
  T->nseq = 0;
  ...
  else
  {
    ...
    for (a = 0; a < S->nseq; a++)
    {
      ...
      if (T->lseq2[a])
        T->lseq[T->nseq++] = a;
      if (T->lseq2[a])
        T->idist[a] = (!R) ? 0 : (R->idist[a] + ((!L) ? 0 : L->idist[a]) + d);
      if (T->lseq2[a])
        T->ldist[a] = (!R) ? 0 : R->ldist[a] + ((!L) ? 0 : L->ldist[a]) + (int)(T->dist * 10000);
    }
  }
...
NT_node node2master(NT_node T, Sequence *S, float *w)
{
  ...
  else if (!T->left && !T->right)
  {
    ...
    if (i == -1)
      HERE("Could Not Find %s in Tree", T->name);
...
int tree2split_list(NT_node T, int ns, int **sl, int *n)
{
  ...
  else
  {
    ...
    for (a = 0; a < ns; a++)
      sl[n[0]][a] = (c == 0) ? T->lseq2[a] : (1 - T->lseq2[a]);
    n[0]++;
  }
  return 1;
}
...
NT_node display_code(NT_node T, int n, FILE *fp)
{
  ...
  if (!T->parent)
    return T;
  else if (!(T->parent)->parent && root4dc == 1)
    return T;
  else if (!(T->parent)->parent && root4dc == 0)
    root4dc = 1;
...
NT_node file2tree(char *fname)
{
  ...
  if (!fname || !check_file_exists(fname))
    return NULL;
...
float tree2tot_dist(NT_node T, int mode)
{
  ...
  {
    if (mode == BOOTSTRAP && T->bootstrap != 0)
      t += T->bootstrap;
    else
      t += T->dist;
  }
...
int node_sort(char *name, NT_node T)
{
  ...
  if (N == NULL)
    printf_exit(EXIT_FAILURE, stderr, "ERROR: %s is not in the tree [FATAL:%s]\n", name, PROGRAM);
...
NT_node tree2node(char *name, NT_node T)
{
  ...
  if (!T)
    return T;
  else if (T->leaf && strm(T->name, name))
    return T;
  else
  {
...
NT_node *tree2seqnode_list(NT_node p, NT_node *L)
{
  ...
  if (!L)
  {
    L = (NT_node *)vcalloc(nn + 1, sizeof(NT_node));
  }
  while (p)
  {
    if (!p->visited && p->isseq)
      L[n++] = p;
...
NT_node duplicate_tree(NT_node T)
{
  ...
  while (L1[a])
  {
    ...
    if (R->parent)
      N->parent = L2[(R->parent)->order];
    if (R->left)
      N->left = L2[(R->left)->order];
    if (R->right)
      N->right = L2[(R->right)->order];
...
NT_node *tree2node_list(NT_node p, NT_node *L)
{
  ...
  if (!L)
  {
    L = (NT_node *)vcalloc(nn + 2, sizeof(NT_node));
  }
  while (p)
  {
    if (!p->visited)
...
int cmp_tree_array(const void *p, const void *q)
{
  ...
  if (vp[0][1] > vq[0][1])
    return 1;
  else if (vp[0][1] < vq[0][1])
    return -1;
  else if (vp[0][2] > vq[0][2])
    return 1;
  else if (vp[0][2] < vq[0][2])
    return -1;
  else
...
int count_tree_groups(Sequence *LIST, char *group_file)
{
  ...
  if (group_file)
  {
    ...
    while (l[ng])
    {
      ...
      {
        if ((i = name_is_in_list(l[ng][b], S->name, S->nseq, 100)) != -1)
          rlist[g][gs[g]][i] = 1;
      }
      gs[g]++;
      ng++;
...
Split **count_splits(NT_node RT, Sequence *LIST, char *param)
{
  ...
  if (!def_param)
    def_param = (char *)vcalloc(10, sizeof(char));
  ...
  for (a = 0; a < n1; a++)
  {
    ...
    for (b = 0; b < n1; b++)
    {
      ...
      if (d <= nb && ((s1 == s2) || ((S->nseq - s1) == s2)))
        print_split(b, list1, LIST, S, main_buf, (tlist == 1) ? in2 : NULL);
    }
  }
  a = 0;
  vfree(cache);
...
Split *print_split(int a, int **list1, Sequence *LIST, Sequence *S, char *buf, char *split_file)
{
  ...
  fprintf(stdout, "\n>");
  for (t = 0, b = 0; b < S->nseq; b++)
  {
    fprintf(stdout, "%d", list1[a][b]);
    t += list1[a][b];
    SP->split[b] = '0' + list1[a][b];
  }
...
int **treelist2lti(Sequence *S, Sequence *TS, int ngb, FILE *out)
{
  ...
  for (a = 0; a < TS->nseq; a++)
  {
    ...
    for (b = 0; b < TS->nseq; b++)
    {
      ...
      {
        if (!dist[c])
          dist[c] = tree2dist(T[c], TS, NULL);
        for (d = 0; d < S->nseq; d++)
        {
          float score, d1, d2;

          if (!dist[d])
            dist[d] = tree2dist(T[d], TS, NULL);
          d1 = dist[c][0][a][b];
          d2 = dist[d][0][a][b];
          score = FABS((d1 - d2));
          if (d1 > ngb || d2 > ngb)
...
int ***tree2dist(NT_node T, Sequence *S, int ***d)
{
  ...
  for (a = 0; a < S->nseq; a++)
    ...
    {
      if (l0[a] > 0 && r0[b] > 0)
        d[0][a][b] = d[0][b][a] = l0[a] + r0[b];
      if (l0[a] > 0 && r0[b] > 0)
        d[1][a][b] = d[1][b][a] = l1[a] + r1[b];
    }
...
int **treelist2groups(Sequence *S, Sequence *TS, char *star_node, FILE *out)
{
  ...
  for (n = 0, a = 0; a < S->nseq; a++)
  {
    if (used[a] > 1)
    {
      ...
      if (out)
        print_tree(T[a], "newick_tree", out);
      results[n][0] = a;
...
int **treelist2avg_treecmp(NT_node *L, char *file)
{
  ...
  if (file)
    L = read_tree_list(main_read_seq(file));
  n = treelist2n(L);
...
int treelist_file2consense(char *tree_file, char *outtree, char *outfile)
{
  ...
  if (!check_program_is_installed("consense", NULL, NULL, "www.phylip.com", NO_REPORT))
    return 0;

  fp = vfopen(command, "w");
  fprintf(fp, "%s\nY\n", tree_file);
  fclose(fp);
  if (check_file_exists("outtree"))
  {
    flag1 = 1;
    printf_system("mv outtree %s::IGNORE_FAILURE::", tmp_outtree);
  }
  if (check_file_exists("outfile"))
  {
    flag2 = 1;
    printf_system("mv outfile %s::IGNORE_FAILURE::", tmp_outfile);
  }
  printf_system("consense <%s > /dev/null 2>/dev/null::IGNORE_FAILURE::", command);

  if (outtree)
    printf_system("mv outtree %s::IGNORE_FAILURE::", outtree);
  remove("outtree");
  if (outfile)
    printf_system("mv outfile %s::IGNORE_FAILURE::", outfile);
  remove("outfile");
  if (flag1)
    printf_system("mv %s outtree::IGNORE_FAILURE::", tmp_outtree);
  if (flag2)
    printf_system("mv %s outfile::IGNORE_FAILURE::", tmp_outfile);
  return 1;
}
...
NT_node treelist2filtered_bootstrap(NT_node *L, char *file, int **score, float t)
{
  ...
  if (t == 1 || t == 0 || !score)
    return treelist2bootstrap(L, file);
...
Alignment *treelist2node_support_best(Alignment *T)
{
  ...
  if (T->Tree)
    return treelist2node_support_best(T->Tree);
...
Alignment *treelist2node_support(Alignment *T)
{
  if (!T)
    ...
    return treelist2node_support(T->Tree);
...
float update_node_support(NT_node T1, NT_node T2, int nseq)
{
  ...
  while (L1 && L1[0])
  {
    ...
    while (L2 && L2[0])
    {
      ...
      if (memcmp(rl1seq, l2seq, nseq * sizeof(int)) && memcmp(l1seq, l2seq, sizeof(int) * nseq))
        L2++;
      else
      {
        sc++;
...
int **treelist2ns(NT_node T, Sequence *B, char *ref)
{
  ...
  for (r = 1; r <= B->nseq; r++)
  {
    ...
    if (list == NULL)
      myexit(fprintf_error(stderr, "File %s is empty", B->name[r - 1]));
  ...
  fprintf(stdout, "#AVG: ");
  for (r = 1; r <= B->nseq; r++)
    fprintf(stdout, "%6.4f ", ((float)tot[r] / (float)max[r]) / (float)nr[r]);
  fprintf(stdout, "\n");
...
Sequence *treelist2sub_seq(Sequence *S, int f)
{
  ...
  for (a = 0; a < maxnseq; a++)
  {
    count[a][2] = 0;
    for (b = 0; b < S->nseq; b++)
      grid[b][maxnseq] = 1; // prepare to keep everything
...
KT_node *pool(KT_node *K1, int n1, int *n2in, int N)
{
  ...
  for (a = 0; a < n1; a++)
    K2[a] = (KT_node)vcalloc(1, sizeof(KTreenode));
...
char *tree2msa4dpa(NT_node T, Sequence *S, int N, char *method)
{
  ...
  if (getenv("DUMP_SEQ_BUCKETS") || getenv("DUMP_SEQ_BUCKETS_ONLY"))
  {
    ...
    if (getenv("DUMP_SEQ_BUCKETS_ONLY"))
      exit(0);
  }
...
ALNcol *msa2graph_new(Alignment *A, Sequence *S, ALNcol ***S2, ALNcol *msa, int seq)
{
  ...
  else
  {
    ...
    for (c = 0; c < S->len[seq]; c++)
      (S2[seq][c])->aa = 1; // tag every msa column that is already assigned to seq
    start = msa;
    while (start)
    {
      HERE("Pa %d %d", start->id, start->aa);
      start = start->next;
    }
    for (c = 0; c < A->len_aln; c++)
      HERE("Ch: %d %d %d %d", graph[c]->id, graph[c]->aa, (graph[c]->next2) ? 1 : 0, (graph[c]->next) ? 1 : 0);

    start = msa;
    while (start->next)
      if (!start->next2)
      {
        HERE("8: %d %d", start->id, start->aa);
        start = start->next;
        if (count == 20)
          exit(0);
        count++;
      }                      // No insert in Child, INCREMENT
...
ALNcol * msa2graph_old2 (Alignment *A, Sequence *S, ALNcol***S2,ALNcol*msa,int seq)
{
  //Thread msa in the child section
  int s, c,ir, subseq, a;
  int * lu =(int*) vcalloc (A->nseq, sizeof (int));
 ...
 printf(" - Create pos indexes from alignment \n");
  int i;  
  char filename[] = "/home/luisasantus/Desktop/crg_cluster/projects/tcoffee/merge_test/data_regressive_standard/intermediate_alignment_";
  for (i = 0; i < A->nseq; i++)
  {
    printf("%s\n", A->name[i]);
    sprintf(filename, "%s%s", filename, A->name[i]);
  }
  ...
  for (s = 0; s < A->nseq; s++)
  {
    ...
    for (r = 0, c = 0; c < A->len_aln; c++)
    {
      ...
      // printf("pos[%d][%d] = %d\n", s,c,pos[s][c]);
      //  printf("%d", pos[s][c]);
      // Save input alignment

      fprintf(fp, "%c", A->seq_al[s][c]);
    }
    fprintf(fp, "\n");
    // printf("\n");
    // This matches the index of seq in S - the complete sequence dataset and in A, the subMSA
...
ALNcol *msa2graph_old(Alignment *A, Sequence *S, ALNcol ***S2, ALNcol *msa, int seq)
{
  // The alignment is an intermediate alignment (like A,B)
  // Sequence contains all the sequences to be aligned
  // S2
  // MSA is the actual final alignment. It is a linked list.
  // Why do we have seq?

  // We want to integrate A into msa.
  // When we do that

  // Thread msa in the child section
  printf("------------------------- \n\n");
  // printf("!!![TEST_LUISA] seq -----------> %d\n",seq );
  // printf("!!![TEST_LUISA] A-> nseq -----------> %d\n", A-> nseq );
  // printf("!!![TEST_LUISA] A-> len_aln -----------> %d\n", A-> len_aln );
  // printf("!!![TEST_LUISA] S-> nseq -----------> %d\n", S-> nseq );

  // printf("!!![TEST_LUISA] msa-> aa -----------> %s\n", msa-> aa );

  int s, c, ir, subseq, a, i;
  int *lu = (int *)vcalloc(A->nseq, sizeof(int));
  int **pos = (int **)declare_int(A->nseq, A->len_aln);
  ALNcol **graph = (ALNcol **)vcalloc(A->len_aln, sizeof(ALNcol *));
  ALNcol *lp, *p, *test, **test2, ***test3;
  ALNcol *start, *end, *parent, *child, *lchild, *lparent;
  int compact = 1;
  int check_homoplasy = 1;
  int *rescount;
  ...
  // If there is nothing to add, we just return the msa.
  if (A->len_aln == 0)
    return msa;
  nnseq = (msa) ? (A->nseq + (msa->next)->nseq - 1) : A->nseq;

  // Quick tets to explore what is Sequence S
  // In Sequence S we have all the sequences to be aligned
  // Explorative, ot be removed
  for (i = 0; i < S->nseq; i++)
  {
    // printf("%s\n", S->name[i]);
  }
  ...
  // 1 -Fill up the look up section: pos
  //  It is a matrix with the summary of the input alignment
  //  index to the position or -1 if gap
  //  It also stores in which position in the list of all the sequences each
  //  of the sequences in the alignment are ( is S has A,B,C and alignment has A,C
  //  in lu i will have 0,2 )
  subseq = -1;
  // printf("Pos matrix\n");
  ...
  //                                                    READ IN ALIGNMENT
  // Luisa debug:
  // Here collect the name of the sequences in the alignment
  ...
  // Here it counts the gaps that are NOT in in the shared sequence
  // This is the stuf we want to align and add as infos, we do not want ot add the info about the shared sequence twice (as already in msa)
  for (s = 0; s < A->nseq; s++)
    if (s != subseq)
      for (c = 0; c < A->len_aln; c++)
      {
  ...
  // -----------------------------------------------------------------------------------------------------------------------------
  // Here only if homoplasy
  // Here also collects counts for homoplasy
  if (msa && check_homoplasy)
  {
    int len, r;
    int *rpos = (int *)vcalloc(A->len_aln, sizeof(int));
  ...
  // I think here the mergin is happening
  // Map new columns
  // printf(" lenght alignment: %d\n", A->len_aln);

  // Iterates each column of the alignment
  //printf(" - Find if there is a reference sequence already, if yes map!\n");
  ...
  for (c = 0; c < A->len_aln; c++)
  {
    ...
    // scan A->nseq to check if one of the residue is already mapped onto MSA
    // If such a residue is mapped then the whole column is mapped
    // here the mapped one is the reference sequence, right?
  ...
  // Case 1: we don't have an MSA yet
  //
  if (!msa)
  {
    // create msa object from graph 
    msa = start = declare_alncol();
    end = declare_alncol();
    start->aa = -1;
    end->aa = -1;
  ...
  else if (compact == 1) // stack
  {
    ...
    for (c = 0; c < A->len_aln; c++)
    {
      // Insering an aminoacid
      if (graph[c]->aa)
        last = graph[c];
      // If the last pos in the msa is a gap 
      else if (last->next && !(last->next)->aa)
      {
        for (s = 0; s < A->nseq; s++)
        {
  ...
  else
  {
    ...
    // Fill in S2 with aa==1
    for (c = 0; c < S->len[seq]; c++)
      (S2[seq][c])->aa = 1;
    for (c = 0; c < A->len_aln; c++)
    {
      // Here is where they add the whole graph in the last column
      // printf(" graph: %d\n", graph[c]->aa);
      if (graph[c]->aa){
        //printf("inserting pos! \n"); 
        last = graph[c];
      }
        
      else
      {
        printf("inserting gap! \n"); 
        // This is where new columns of gaps are inserted in the MSA
        graph[c]->next = last->next;
        last->next = graph[c];
        last = last->next;
...
int kseq2kmsa_thread(NT_node T, KT_node *K, int n, char *method)
{
  ...
  for (a = 0; a < n; a++)
    failed += (check_file_exists(K[a]->msaF)) ? 0 : 1;
  if (failed > 0)
...
char *reg_seq_file2msa_file(char *method, int nseq, char *seqF, char *msaF, char *treeF)
{
  ...
  if (nseq == 1)
    com = csprintf(NULL, "cp %s %s", seqF, msaF);
  else if (use_old_constraint_list)
    return seq_file2msa_file(method, seqF, msaF);
  else
    com = csprintf(NULL, "dynamic.pl -method %s -seq %s -outfile %s -tree %s", method, seqF, msaF, treeF);
  ...
  for (a = 0; a < K->nc; a++)
  {
    ...
    while (lu0[i0][n])
    {
      ...
      if (lm > 0 || ls > 0)
      {
        ...
        {
          if (A->seq_al[0][c] == '-')
            tm = insert_gap_in_graph(tm);
          else
            tm = tm->r;
          if (A->seq_al[1][c] == '-')
            ts = insert_gap_in_graph(ts);
          else
            ts = ts->r;
        }
        vfree(master);
...
ALN_node *kmsa2graph_seq(Sequence *S, KT_node K, Alignment *A0, ALN_node **lu0, ALN_node *list, int *ns, int *done, int max)
{
  ...
  for (a = 0; a < K->nc; a++)
  {
    ...
    {
      if (A->seq_al[0][b] == '-')
        m = insert_gap_in_graph(m);
      else
        m = m->r;

      if (A->seq_al[1][b] == '-')
        s = insert_gap_in_graph(s);
      else
        s = s->r;
    }
...
char **graph2master_seq(ALN_node n, char **seq)
{
  ...
  if (!seq)
    seq = (char **)vcalloc(2, sizeof(char *));
...
int check_graph_integrity(ALN_node g)
{
  ...
  while (t)
  {
    ...
    while (col)
    {
      if (n2top(col) != top)
      {
        HERE("column pb with top");
        exit(0);
      }
      else if (n2bot(col) != bot)
      {
        HERE("column pb with bot");
        exit(0);
      }
      col = col->c;
    }
  ...
  for (a = 0; a < K->nc; a++)
  {
    ...
    while (lu0[i0][n])
    {
      ...
      if (lm > 0 || ls > 0)
      {
        ...
        {
          if (A->seq_al[0][c] == '-')
            tm = insert_gap_in_graph_d(tm);
          else
            tm = rn_r(tm);
          if (A->seq_al[1][c] == '-')
            ts = insert_gap_in_graph_d(ts);
          else
            ts = rn_r(ts);
        }
        vfree(master);
