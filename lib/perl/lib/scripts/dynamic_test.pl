...
#  Error message if tree is not a file.
if ($tree eq "list")
  {
    my $f="$tmpdir/f";
    # The command will create a dummy fasta to run tcoffee
...
# ERROR message also in case methods are passed as a list as normally in T-coffee
# Parse alignment methods, in case they are given as a list
...
# -------------------------------------------------------
#     1.    COPY (if given) OR COMPUTE GUIDE TREE
# -------------------------------------------------------
...
# -------------------------------------------------------
#     2.    COPY (if given) OR COMPUTE GUIDE TREE
# -------------------------------------------------------
...
if ($cmethod eq "tcoffee"|| $cmethod eq "t_coffee" )
  { my $string=file2string($infile);
    if ($VERBOSE){print "\n![dynamic.pl] --- RUNNING ALIGNMENT \n $string\n\n";}
    print("t_coffee -seq $infile -outfile $outfile -output fasta_aln $CL4tc>/dev/null  $QUIET\n\n");
    print("---------------------------------------------------------------------------------------");
    my_system ("t_coffee -seq $infile -outfile $outfile -quiet -output fasta_aln $CL4tc>/dev/null  $QUIET");
  }
...
elsif ($method2use=~/famsa/)
  {
    ...
    my $string=file2string($infile);
    if ($VERBOSE){print "\n![dynamic.pl] --- RUNNING ALIGNMENT \n $string\n\n";}
    print("famsa $treeFlag $threadFlag4famsa $infile $outfile >/dev/null $QUIET\n\n");
    print("---------------------------------------------------------------------------------------");
...
# Returns number of sequences in fasta
sub file2nseq
  {
    # Input: fasta file
    # Output: number of sequences in file
    my ($f)=@_;
    my $n=`grep -c '>' $f`;
    return $n;
  }

# Given a file, returns its absolute path.
# If the file does not already present its absolute path,
# It assumes its in the current directory
sub file2abs
    {
      my ($f, $mode)=@_;
      # Case 1: file already has absolute path
      if (!$f || $f=~/^\//){return $f;}
...
# returns content of file in a string format,
